...
Grok 可玩路径是**完整 plugin**（与 Codex 同构的 `skills/` 树，加上同一个
toolbox MCP gateway），不是单独的薄路由 skill，也不是 Grok 专用运行时。
...
安装脚本会失败关闭，除非 `grok plugin details coc-keeper` 确认可信插件同时
发现 skill 和 MCP server。然后在本仓库开 Grok 会话，用 `grok inspect` 确认
`plugin: coc-keeper` 下能看到 `coc-main`、`coc-keeper-play` 等 skill 以及
`coc-keeper` MCP server，再说：
...
`coc-story-director` 加载完整 craft，并优先通过插件 MCP 调用同一
`coc_toolbox.py`；MCP 不可用时才走等价 shell fallback，同一次状态变更不能
混用两条路径。必须原样保存玩家输入，禁止手改 `.coc`、日志或 toolbox
evidence；finalization 顺序/完整性失败时只能使用 typed/idempotent operation
修复，否则停止并报告 blocker。立绘使用
...
Cursor、Grok、Kimi、ZCode 的 MCP gateway 都只转发到现有
