...
- `authority.clue_here: false` is no longer read as an instruction to rebind. It is a fact about where the carrier currently is: prepare a reviewed `source_rebinding` only when the fiction is genuinely reaching for that carrier, and otherwise play the chosen action and leave the thread standing. The compressed `brief.md` reminder is corrected in the same way; it previously read `authority.clue_here=false: reviewed source_rebinding first` as a bare imperative, which is what the Keeper acted on.
- `mode` is no longer permanent. A delivered bridge closes `clarify_known` only until a later assessment records the player `aligned` on that same worldline/loop/thread; after that, held evidence may be clarified again. The `relation`-direction rule (1.2.6) is unchanged, and `bridge_offer`, `bridge_receipt`, `acquired_clarification` and `player_discharge` keep their meanings and authority rules exactly.
- Retained blocking evidence: campaign `game-83177d61-ab11-4d58-b8ec-cf8b9c98d5a4` on `the-haunting`, turns 87-91. Turn 47 had delivered a bridge on `house-haunted-by-corbitt`; turns 62, 63, 64 and 80 assessed `aligned` on that same thread; turn 87 assessed a pure movement sentence as `detached` while the player was walking to the newspaper to publish the story that thread is about, holding four of its five supporting clues. The permanent prior-delivery flag forced `introduce_evidence`, the one unacquired clue was authored at a sanitarium so `authority.clue_here` was false everywhere, and with no `preparation_wait` and no `rebinding_refused` the only lawful sub-review was `none`/`revise`. Turns 88, 89 and 90 published nothing at all despite settled receipts (a hard-success Persuade, a move, an NPC arrival) while the Keeper spent `max_rewrites` on `source_rebinding` attempts that failed on anchors. Turn 91 passed first try only because the player happened to state the core claim verbatim. No new lane, verb, counter, semantic regex/list, physical-place list or automatic adaptation is added.
...
## 1.2.6
- Makes the selected evidence's `relation` explicit for reentry clarification. Under `mode` `clarify_known`, the one acquired `known` row the Keeper uses carries a `relation` (`supports` | `contradicts`); the Keeper must state how it supports or contradicts the selected core thread's claim **with that row's own relation**, and the exact quote must use the evidence in that direction. A `supports` row may not be used to argue against the selected thread (for example, to argue the house is a manufactured scare or bad luck), and a `contradicts` row may not be used to argue for it. Under `mode` `introduce_evidence` the supplied `bridge` already carries its `relation`; the Keeper realizes the bridge in that same direction. The checked audit (`narration-audit` 1.2.16) now carries `relation` in `reentry_review` and enforces the same rule; no new lane, verb, counter, semantic regex/list, physical-place list or automatic adaptation is added.
- Retained failed evidence: `midgame-reentry-live-20-run` turn 5. Mode was `clarify_known` with known row `globe-unpublished-story` whose relation was `supports`, and the player was still explicitly `misframed` about `house-haunted-by-corbitt`. The candidate used that supporting article to argue the opposite direction — that the unrelated fates looked like a manufactured scare rather than Corbitt's will — and the audit passed `acquired_clarification` anyway. That is a wrong-direction clarification, not a pass: a `supports` row must be argued in the supporting direction. P5 remains open pending retest.
- Raw `midgame-reentry-live-18` evidence was accidentally deleted by a worktree closeout and is now `invalid-for-acceptance`; it was never retained in the evidence worktree. The `midgame-reentry-live-19` and `midgame-reentry-live-20` campaigns and playtest runs are retained there.
