...
def write_sanity_gain_pending(
    ...
    *,
    ...
    write_json_atomic(path, { "schema_version": 1, "investigator_id": investigator_id, "san_gain": san_gain, "gain_source": source, })
    return path
...
def _load_phobia_mania_table(tables: RuleTables, name: str) -> dict[str, Any]:
    """Load phobias.json or manias.json as a flat {Name: {trigger,...}} dict.

    Returns {} if the table is missing so the module degrades gracefully.
    """
    data = tables.load(name) if tables.exists(name) else {}
...
class SanitySession:
    ...
    def __init__(self, investigator_id: str, san_max: int, int_value: int,
                 ...
                 cm_value: int = 0,
                 awfulness_caps: dict[str, int] | None = None, *,
                 tables: RuleTables,
                 clock_minutes: int | None = None) -> None:
        self.investigator_id = investigator_id
        self._tables = tables
        self._roll = RollApi(tables)
        # The campaign clock (world.json clock.minutes) at load time; the time layer
        # of the old repo is re-cut onto it: due triggers are stored on the snapshot
        # and compared against this clock by the session layer.
        self.clock_minutes = clock_minutes
        self.recovery_trigger: dict[str, Any] | None = None
        self.treatment_trigger: dict[str, Any] | None = None
        ...
        # / clear clock triggers and align with day anchors.
    ...
    def apply_direct_loss(
        ...
        *,
        ...
        if parsed["kind"] == "constant":
            ...
            rolled = percentile.roll_expression(loss_expr, self._rng)
    ...
    def sanity_check(self, source: str, san_loss_success: int | str,
        ...
        san_before = self.san_current
        res = self._roll.percentile_check(san_before, rng=self._rng)
        roll_id = self._roll_id()
    ...
    def _check_temporary_insanity(self, source: str, alone: bool,
        ...
        """
        int_res = self._roll.percentile_check(self.int_value, rng=self._rng)
    ...
    def _schedule_monthly_treatment_trigger(self) -> str | None:
        """Record when the next monthly Psychoanalysis treatment is due (p.164).

        Due at ``clock_minutes + 30 days``; the session layer publishes
        ``sanity.treatment.due`` once the campaign clock reaches it. Returns the
        trigger id, or None when no clock is attached.
        """
    ...
    def _resolve_bout_result(self, table_key: str, bout_roll: int) -> dict[str, Any]:
        ...
        try:
            if table_key == "summary":
                rows = self._tables.bout_summary_table()
            else:
                rows = self._tables.bout_realtime_table()
        except Exception:
            return {}
        for row in rows or []:
    ...
    def _roll_phobia(self) -> str | None:
        ...
        """
        table = _load_phobia_mania_table(self._tables, "phobias")
        if not table:
            return None
    ...
    def _roll_mania(self) -> str | None:
        ...
        """
        table = _load_phobia_mania_table(self._tables, "manias")
        if not table:
            return None
        names = list(table.keys())
        roll = self._rng.randint(1, 100)
    ...
    # ------------------------------------------------------------------ #
    # Clock integration: schedule recovery / treatment triggers on the snapshot
    # ------------------------------------------------------------------ #
    def _time_layer_ready(self) -> bool:
        """True when the session was loaded against the campaign clock."""
        return self.clock_minutes is not None

    def _record_time_layer_san_loss(self, amount: int) -> None:
        """The old time layer mirrored daily SAN loss into a period ledger; the kernel
        keeps ``daily_san_lost`` on the snapshot itself, so nothing else to record."""
        return None

    def _schedule_recovery_trigger(self, remaining_hours: int) -> str | None:
        """Record when temporary insanity may recover (p.176: 1D10 hours, in a safe place).

        Due at ``clock_minutes + remaining_hours*60``; the session layer publishes
        ``sanity.recovery.due`` once the campaign clock reaches it. Returns the
        trigger id, or None when no clock is attached.
        """
        if not self._time_layer_ready():
            return None
    ...
    def recover_temporary(self) -> bool:
        """p.176: temporary insanity ends after 1D10 hours (in a safe place).

        Clears the condition, drops the recovery trigger and emits a recovery event.
        """
    ...
    def snapshot(self) -> dict[str, Any]:
        return {
            ...
            "recovery_trigger": dict(self.recovery_trigger) if self.recovery_trigger else None,
            "treatment_trigger": dict(self.treatment_trigger) if self.treatment_trigger else None,
            "events": list(self.events),
        }

    def save(self, campaign_dir: Path) -> Path:
        """Write the per-investigator snapshot (``save/sanity-state/<inv>.json``).

        The old engine also kept a legacy single-owner mirror and pushed SAN into an
        investigator-state document; the kernel mirrors ``san_current`` into the party
        sheet through the settlement context instead.
        """
    ...
    def load(cls, campaign_dir: Path, investigator_id: str,
             ...
             cm_value: int = 0, *, tables: RuleTables,
             clock_minutes: int | None = None) -> "SanitySession":
        """Reconstruct a session from the per-investigator snapshot, or start a fresh
        one (``san_max`` 99 until the caller seeds it from the sheet) when none exists."""
        canonical_path = sanity_snapshot_path(campaign_dir, investigator_id)
        if not canonical_path.is_file():
            return cls(investigator_id, san_max=99, int_value=int_value,
                       rng=rng or random.Random(), cm_value=cm_value,
                       campaign_dir=campaign_dir, tables=tables, clock_minutes=clock_minutes)
        snap = read_json(canonical_path)
        ...
        recovery = snap.get("recovery_trigger")
        sess.recovery_trigger = dict(recovery) if isinstance(recovery, dict) else None
        treatment = snap.get("treatment_trigger")
        sess.treatment_trigger = dict(treatment) if isinstance(treatment, dict) else None
