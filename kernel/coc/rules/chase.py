...
opposed Drive Auto. Persists via fileio.write_json_atomic.
...
def _load_chase_rules(tables: RuleTables) -> dict[str, Any]:
    return tables.load("chase") if tables.exists("chase") else {}


def get_vehicle_stats(tables: RuleTables, vehicle_name: str) -> dict[str, Any]:
    ...
    """
    rules = _load_chase_rules(tables)
...
def vehicle_collision(tables: RuleTables, severity: str, rng: random.Random | None = None) -> dict[str, Any]:
...
class ChaseSession:
    ...
    def add_participant(
        ...
        if vehicle_key and is_vehicle:
            stats = get_vehicle_stats(self._tables, vehicle_key)
    ...
    def establish(self) -> dict[str, Any]:
        ...
        for aid, p in self.participants.items():
            ...
            if p["is_vehicle"] and p.get("drive_auto") is not None:
                ...
                continue
            res = self._roll.percentile_check(int(target), rng=self._rng)
            delta = 0
    ...
    def _resolve_dex_tie(
        ...
        for participant in participants:
            result = self._roll.percentile_check(int(participant["dex"]), rng=self._rng)
    ...
    def _negotiate_hazard(
        ...
        *,
        ...
        res = self._roll.percentile_check(
    ...
    def _resolve_barrier_skill(
        ...
        self._spend_actions(p, 1)
        res = self._roll.percentile_check(target, difficulty=difficulty, rng=self._rng)
        rid = self._roll_id()
        self.pending_rolls.append({
            "roll_id": rid, "actor_id": actor_id, "skill": skill,
            "target": target, "roll": res["roll"], "outcome": res["outcome"],
    ...
    def initiate_melee_conflict(
        ...
        *,
        ...
        a CombatSession (``combat.CombatSession``) — we add participants if
    ...
    def vehicle_conflict(
        ...
        *,
        ...
        atk_res = self._roll.percentile_check(atk_skill, penalty=atk_penalty, rng=self._rng)
        dfn_res = self._roll.percentile_check(dfn_skill, rng=self._rng)
        ...
        opposed = CombatSession._resolve_opposed(
        ...
        if damage >= 20:
            ...
            sev = "minor"
        coll = vehicle_collision(self._tables, sev, rng=self._rng)
        out["collision"] = coll
    ...
    def apply_vehicle_collision(
        ...
        *,
        ...
        p = self.participants[actor_id]
        coll = vehicle_collision(self._tables, severity, rng=self._rng)
    ...
    def _resolve_legacy_conflict(
        ...
        res = self._roll.percentile_check(int(ft), rng=self._rng)
        rid = self._roll_id()
        self.pending_rolls.append({
    ...
    def _resolve_hide(
        ...
        res = self._roll.percentile_check(stealth, rng=self._rng)
        rid = self._roll_id()
        self.pending_rolls.append({
    ...
    def passenger_action(
        ...
        if atype == "assist_driver":
            ...
            res = self._roll.percentile_check(target, rng=self._rng)
            rid = self._roll_id()
            self.pending_rolls.append({
                "roll_id": rid, "actor_id": actor_id, "skill": skill,
                "target": target, "roll": res["roll"], "outcome": res["outcome"],
    ...
    def sudden_hazard(
        ...
        *,
        ...
        res = self._roll.percentile_check(luck_target, rng=self._rng)
        rid = self._roll_id()
        self.pending_rolls.append({
    ...
    def roll_random_hazard(self, *, environment: str = "normal") -> dict[str, Any]:
        ...
        if environment == "hazardous":
            ...
            bonus = 1
        res = self._roll.percentile_check(100, bonus=bonus, penalty=penalty, rng=self._rng)
    ...
    def _validate_genesis_evidence(cls, data: dict[str, Any], evidence: Any) -> None:
        ...
        for origin in genesis_participants:
            ...
            if (not isinstance(origin.get("conditions"), list)
                    ...
                    or any(value not in VALID_CONDITIONS
    ...
    @classmethod
    def load(
        cls, path: Path, rng: random.Random | None = None, *, tables: RuleTables,
    ...
    def _validate_snapshot(data: Any) -> None:
        ...
        for index, round_row in enumerate(rounds, start=1):
            ...
            for turn in round_row["turns"]:
                ...
                # The budget is recomputed every round (debt, a captured mover changing the
                # slowest MOV), so a historical turn's budget is its own record, not the
                # live participant's; only DEX is constant across the chase.
                budget = turn.get("movement_actions")
                if turn.get("dex") != participant["dex"] or isinstance(budget, bool) or not isinstance(budget, int) or budget < 0:
