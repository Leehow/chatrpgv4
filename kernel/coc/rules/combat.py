...
# --------------------------------------------------------------------------- #
def load_weapon_catalog(tables: RuleTables) -> dict[str, dict[str, Any]]:
    """Load the canonical weapon catalog from the ruleset's weapons.json.
    ...
    """
    data = tables.load("weapons") if tables.exists("weapons") else {}
...
def resolve_module_weapons(
    ...
    """
    base = dict(catalog) if catalog else load_weapon_catalog(tables)
...
class CombatSession:
    ...
    def __init__(self, combat_id: str, scene_ref: str, started_at_turn: int,
        ...
        self._weapon_catalog = resolve_module_weapons(tables, module_weapons)
    ...
    def _percentile(self, actor_id: str, skill: str, target: int, goal: str,
        ...
        res = self._roll.percentile_check(target, difficulty, bonus=bonus,
        ...
        if (
            ...
            if unmodified == 0:
                unmodified = 100
            unmodified_settlement = self._roll.resolve_percentile_roll(
                unmodified,
                int(res["base_target"]),
        ...
        record["player_projection"] = self._roll.build_player_projection(record)
    ...
    def _apply_luck_to_roll(
        ...
        *,
        ...
        adjusted = self._roll.spend_luck(
            {
                "roll": original_roll,
                **{
                    field: record[field]
    ...
    def _apply_opposed_luck_precommit(
        ...
        *,
        ...
        for points in range(1, max_points + 1):
            candidate_settlement = self._roll.resolve_percentile_roll(
    ...
    @classmethod
    def load(
        cls, campaign_dir: Path, *, rng: random.Random, tables: RuleTables,
        ...
        session = cls(
            ...
            strict_int(data["started_at_turn"], "started_at_turn"), rng=rng, tables=tables,
        )
