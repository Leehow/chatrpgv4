...
export interface ProductRunSummary {
  run: number; fixture: string; admission: string; keeper: string; thinking: string; wall_ms: number; status: string | null; reason: string | null;
...
export async function productReplayOnce(name: string, run: number, outDir: string, admission: 'lane' | 'jev', options: ProductRunOptions = {}): Promise<ProductRunSummary> {
  ...
  const summary: ProductRunSummary = {run, fixture: name, admission, keeper: live ? `live ${liveProvider}/${liveModel}` : 'replay', thinking, wall_ms: wall,
...
export async function main(argv: string[]): Promise<void> {
  ...
  for (let run = 1; run <= runs; run++) {
    const summary = await productReplayOnce(name, run, outDir, admission, {keeper, ...(thinking ? {thinking} : {}), ...(model ? {model} : {}), ...(then.length ? {then} : {})});
    summaries.push(summary);
    console.log(JSON.stringify({run, fixture: name, admission, keeper: summary.keeper, thinking: summary.thinking, wall_ms: summary.wall_ms,
