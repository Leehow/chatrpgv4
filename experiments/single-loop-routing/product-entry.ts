 ...
 */
function admissionReplay(baseline: Row, log: (row: Row) => void, liveKeeper = false) {
...
export interface ProductRunSummary {
  run: number; fixture: string; admission: string; keeper: string; thinking: string; seed: string | null; wall_ms: number; status: string | null; reason: string | null;
...
export async function productReplayOnce(name: string, run: number, outDir: string, admission: 'lane' | 'jev', options: ProductRunOptions = {}): Promise<ProductRunSummary> {
  ...
  const judge = admissionReplay(baseline, log, live);
  ...
  const summary: ProductRunSummary = {run, fixture: name, admission, keeper: live ? `live ${liveProvider}/${liveModel}` : 'replay', thinking, seed: options.seed ?? null, wall_ms: wall,
...
export async function main(argv: string[]): Promise<void> {
  ...
  for (let run = 1; run <= runs; run++) {
    const summary = await productReplayOnce(name, run, outDir, admission, {keeper, ...(thinking ? {thinking} : {}), ...(model ? {model} : {}), ...(then.length ? {then} : {}), ...(seed ? {seed} : {})});
    summaries.push(summary);
    console.log(JSON.stringify({run, fixture: name, admission, keeper: summary.keeper, thinking: summary.thinking, seed: summary.seed, wall_ms: summary.wall_ms,
