...
import { sourceInfo, sourcePage, sourceSearch, sourceText, sourceWindow, closeSourceDocuments } from '../extensions/module/source.ts';

export async function sourceMain(args: string[]): Promise<number> {
  try {
    if (args.length !== 2 || !['info', 'page', 'search', 'text', 'window'].includes(args[0])) throw new Error('Unknown source operation');
