...
/**
 * The worker roles live in a package now, not the runtime tree, so a test that
 * wants a catalog installs the package that owns it — App profile, then on.
 */
async function installRolesPack(agent: string): Promise<void> {
  const dest = join(agent, "extensions", "roles-pack");
  await mkdir(dest, { recursive: true });
  await cp(new URL("../../../packs/agent-orchestration/agents", import.meta.url).pathname, join(dest, "agents"), { recursive: true });
...
describe("explicit sidebar project persistence", () => {
  ...
  it("persists ordered subagent model chains without replacing other settings", async () => {
    const fixture = await setup();
    await installRolesPack(fixture.agent);
    const first = backend(fixture.agent, fixture.sessions);
    await first.handle("setExtensionEnabled" as never, ["roles-pack", true, "app"]);
    expect(await first.handle("setSubagentModel", ["explore", [{ model: "openai/gpt-5", thinking: "high" }, { model: "anthropic/claude-sonnet-4", thinking: "medium" }]])).toEqual({
      explore: [{ model: "openai/gpt-5", thinking: "high" }, { model: "anthropic/claude-sonnet-4", thinking: "medium" }],
    });
    expect(await first.handle("listAgentDefinitions", [])).toEqual(expect.arrayContaining([
      expect.objectContaining({ name: "explore" }),
    ]));
    const listed = await first.handle("listAgentDefinitions", []) as Array<{ name: string; description?: string }>;
    expect(listed.map((a) => a.name)).toEqual(expect.arrayContaining(["explore", "general-purpose", "reviewer", "secretary"]));
    expect(listed.map((a) => a.name)).not.toContain("operator");
    expect(listed.map((a) => a.name)).not.toContain("computer-use");
    expect(listed.every((a) => typeof a.name === "string" && typeof a.description === "string")).toBe(true);
    const fresh = backend(fixture.agent, fixture.sessions);
