...
describe("PiHostBackend message queue integration", () => {
  it("queues busy sends, emits snapshots, preserves attachments, and drains FIFO only after settle", async () => {
    ...
    // Post-Stop nothing automatic redelivers; items are held, not lost.
    await eventually(() => (backend as any).queue.isBusy("s1") === false);
    expect((await backend.handle("listQueue", ["s1"]) as any[]).map(item => item.text)).toEqual(["first", "second"]);
    // A deliberate user cut-in re-engages delivery and resumes the FIFO.
    await backend.handle("cutInQueuedMessage", ["s1", first.message.id]);
  ...
  it("supports edit, promote, steer, remove, failed retention, and retry through the real RPC path", async () => {
    ...
    // The steer ack never produced a user echo; stop recycles "B edited" to
    // the FIFO head. Release the held turn naturally so the drain delivers
    // the recycled item before the `__queue_fail__` probe.
    const failed = await backend.handle("enqueueMessage", ["s1", "__queue_fail__"]) as any;
    expect(failed.outcome).toBe("queued");
    await backend.handle("stop", ["s1"]);
    await eventually(() => (backend as any).queue.isBusy("s1") === false);
    expect((await backend.handle("listQueue", ["s1"]) as any[]).map(item => item.text)).toEqual(["B edited", "__queue_fail__"]);
    // User Stop must freeze automatic redelivery; an explicit user cut-in of
    // the recycled head re-engages the FIFO chain through the same drain path.
    // Recycled payloads mint fresh ids, so address the head by position.
    const recycledHead = (await backend.handle("listQueue", ["s1"]) as any[])[0];
    await backend.handle("cutInQueuedMessage", ["s1", recycledHead.id]);
  ...
  it("requeues a steered message whose turn settled without echoing it", async () => {
    const setup = await fixture();
    const backend = setup.create();
    await backend.handle("sendPrompt", ["s1", "__hold__"]);
    const item = await backend.handle("enqueueMessage", ["s1", "interrupt lost"]) as any;
    const steered = await backend.handle("steerQueuedMessage", ["s1", item.message.id]) as any;
    expect(steered.state).toBe("sending");
    expect(await backend.handle("listQueue", ["s1"])).toEqual([]);

    // fake-pi acks the steer but never echoes it, like pi when the running
    // turn is aborted/compacted before its steering queue drains. The settle
    // must recycle the payload to the FIFO head instead of losing it.
    await backend.handle("stop", ["s1"]);
    await eventually(() => (backend as any).queue.isBusy("s1") === false);
    expect(await backend.handle("listQueue", ["s1"])).toEqual([
      expect.objectContaining({ text: "interrupt lost", state: "queued" }),
    ]);

    // The next deliberate user action delivers it as a normal prompt. The
    // recycled payload carries a fresh id, so address the head by position.
    const recycledHead = (await backend.handle("listQueue", ["s1"]) as any[])[0];
    await backend.handle("cutInQueuedMessage", ["s1", recycledHead.id]);
    await eventually(() => (backend as any).queue.listQueue("s1").length === 0);
    await backend.close();
  ...
  it("does not redeliver a steered message once pi echoes it as a user message", async () => {
    const setup = await fixture();
    const backend = setup.create();
    const userEchoes: any[] = [];
    const off = backend.subscribe(event => { if (event.channel === "stream" && event.event.type === "user_message") userEchoes.push(event.event); });
    await backend.handle("sendPrompt", ["s1", "__hold__"]);
    const item = await backend.handle("enqueueMessage", ["s1", "__steer_echo__ hello"]) as any;
    const steered = await backend.handle("steerQueuedMessage", ["s1", item.message.id]) as any;
    expect(steered.state).toBe("sending");
    await eventually(() => userEchoes.some(event => event.content === "__steer_echo__ hello"));

    // The echo is the delivery receipt: settle recycles nothing when it lands
    // first. If stop beat a very late ack/recycle path, the payload is instead
    // preserved queued and suppression must hold it until an explicit user
    // resend — either way nothing is lost and nothing is redelivered twice.
    await backend.handle("stop", ["s1"]);
    await eventually(() => (backend as any).queue.isBusy("s1") === false);
    const remain = await backend.handle("listQueue", ["s1"]) as any[];
    if (remain.length > 0) {
      expect(remain).toEqual([
        expect.objectContaining({ text: "__steer_echo__ hello", state: "queued" }),
      ]);
      await backend.handle("cutInQueuedMessage", ["s1", remain[0].id]);
    }
    expect(await backend.handle("listQueue", ["s1"])).toEqual([]);
    off();
    await backend.close();
  ...
  it("announces a drained queue prompt on the next started so the UI does not treat it as a ghost", async () => {
    ...
    await backend.handle("stop", ["s1"]);
    // Only a user-driven resend may start the next turn: a cut-in announces
    // the delivery on `started` exactly like an idle drain used to.
  ...
  it("user stop unwinds a parked cut-in when Pi never settles", async () => {
    ...
    const cutIn = backend.handle("cutInQueuedMessage", ["s1", queued.message.id]);
    await eventually(async () => (await backend.handle("listQueue", ["s1"]) as any[]).some(item =>
      item.id === queued.message.id && item.text === "unstick me" && item.state === "sending"));
    ...
    await expect(cutIn).rejects.toThrow(/remains queued/);
    off();
    await backend.close();
  });
  ...
  it("treats cutIn after an idle enqueue was already accepted as idempotent", async () => {
    const setup = await fixture();
    const backend = setup.create();
    const direct = await backend.handle("enqueueMessage", ["s1", "already accepted"]) as any;
    expect(direct.outcome).toBe("direct");
    expect(await backend.handle("listQueue", ["s1"])).toEqual([]);

    await expect(backend.handle("cutInQueuedMessage", ["s1", direct.message.id])).resolves.toMatchObject({
  ...
  it("stop after exiting does not spawn a replacement or abort a new pid", async () => {
    ...
    const afterStop = (await backend.handle("listAgents", ["s1"]) as any[]).find(agent => agent.agentId === "agent-1");
    expect(afterStop).toMatchObject({ agentId: "agent-1", state: "running" });
    ...
    await backend.close();
    await sweeps.waitForIdle();
    const afterClose = [...(backend as any).agents.values()].find((agent: any) => agent.agentId === "agent-1");
