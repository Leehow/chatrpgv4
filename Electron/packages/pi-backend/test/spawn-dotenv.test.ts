...
describe("pi spawn env injects the configured profile .env (T17 parity)", () => {
  ...
  async function fixture(dotEnv: string | undefined, parentEnv: NodeJS.ProcessEnv = {}, piCommand?: PiCommand, resourceMode?: "default" | "explicit") {
    ...
    if (resourceMode === "explicit") {
      const projectAgent = join(cwd, ".pi", "agent");
      await mkdir(projectAgent, { recursive: true });
      await writeFile(join(projectAgent, "ext-enabled.json"), "{}\n");
    }
  ...
  it("strips stale PIPIUI_* from .env and parent; the host contract wins", async () => {
    const { backend, env } = await fixture(
      "PIPIUI_SESSION_KEY=stale-dotenv\nPIPIUI_BRIDGE_PORT=9999\nPIPIUI_EXT_STALE_PROBE=/stale/dotenv.ts\n",
      { PIPIUI_SESSION_KEY: "stale-parent", PIPIUI_EXT_STALE_PROBE: "/stale/parent.ts" },
    );
    await backend.handle("sendPrompt", ["session-1", "go"]);
    const actual = env();
    ...
    expect(actual.PIPIUI_EXT_STALE_PROBE).toBeUndefined();
  });
  ...
  it("does not import the host-global legacy project registry into a HOME-less sandbox", async () => {
    // The host env below deliberately has no HOME. The legacy Swift registry is
    // user-scoped, so nothing may be imported from the real homedir: a leaked
    // import used to register the developer's real projects in this sandbox and
    // then repoint their on-disk credential files at this temp fixture.
    const { backend } = await fixture(undefined, {});
    const projects = await backend.handle("listProjects", []) as Array<{ path: string }>;
    for (const project of projects) {
      expect(project.path.startsWith(root)).toBe(true);
    }
  });
