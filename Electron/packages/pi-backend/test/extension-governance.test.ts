...
import { createExtensionRegistry, type ExtensionRegistry } from "../src/extension-registry.js";
...
async function backendFor(dirs: { runtime?: string; agent?: string; extensionRegistry?: ExtensionRegistry }) {
  ...
  return createPiHostBackend({
    ...
    vaultDir: agent,
    ...(dirs.extensionRegistry ? { extensionRegistry: dirs.extensionRegistry } : {}),
  });
}
...
describe("settings migrations", () => {
  ...
  it("applies key maps on load and writes the new version once", async () => {
    ...
    await writeManifest(join(agent, "extensions", "quota"), quotaV2);
    await mkdir(agent, { recursive: true });
    await writeFile(
...
describe("uninstallExtension", () => {
  ...
  it("refuses to uninstall builtin packages", async () => {
    ...
    // The base registers no builtin at all, so the rule is exercised against an
    // injected one: a host that does ship a package still cannot have it removed.
    ...
    await expect(backend.handle("uninstallExtension" as never, ["host-builtin"])).rejects.toThrow(/cannot be uninstalled/);
    const listed = (await backend.handle("listExtensions" as never, [])) as Array<{ id: string }>;
    expect(listed.find((item) => item.id === "host-builtin")).toBeTruthy();
    await backend.close();
  });
