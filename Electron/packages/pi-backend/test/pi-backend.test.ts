...
const waitForNoPrompt=async(path:string,needle:string,timeoutMs=250)=>{const deadline=Date.now()+timeoutMs;for(;;){const log=await readFile(path,"utf8").catch(()=>"");if(log.includes(needle))throw new Error(`prompt log unexpectedly contained ${needle}: ${log}`);if(Date.now()>deadline)return log;await new Promise(r=>setTimeout(r,10));}};
const backendWithPromptLog=async(label:string)=>{root=await mkdtemp(join(tmpdir(),label));const cwd=join(root,"project");const dir=join(root,"sessions","project");await mkdir(dir,{recursive:true});await mkdir(cwd,{recursive:true});const promptLog=join(root,"prompts.log");await writeFile(join(dir,"session.jsonl"),JSON.stringify({type:"session",version:3,id:"session-1",timestamp:"2026-08-10T00:00:00.000Z",cwd})+"\n");const backend=createPiHostBackend({agentDir:join(root,"agent"),sessionsRoot:join(root,"sessions"),runtimeRoot:join(root,"runtime"),piPath:"node",spawn:(_bin,_args,options)=>spawn("/usr/local/bin/node",[new URL("./fake-pi.mjs",import.meta.url).pathname],{...options,env:{...options.env,PATH:"/usr/local/bin:/usr/bin:/bin",FAKE_PI_PROMPT_LOG:promptLog}}) as any});await backend.handle("addProject",[cwd]);return {backend,promptLog};};
it("stop aborts only the Boss turn and leaves running background subagents",async()=>{const {backend,promptLog}=await backendWithPromptLog("pipi-pi-stop-keep-");await backend.handle("sendPrompt",["session-1","__agent_running__"]);const before=await waitForAgentState(backend,"agent-1","running");expect(before?.state).toBe("running");await backend.handle("stop",["session-1"]);await waitForNoPrompt(promptLog,"/subagent_abort_all");const after=await waitForAgentState(backend,"agent-1","running");expect(after?.state).toBe("running");await backend.close();});
it("cut-in aborts only the Boss turn and does not send /subagent_abort_all",async()=>{const {backend,promptLog}=await backendWithPromptLog("pipi-pi-cutin-keep-");await backend.handle("sendPrompt",["session-1","__hold_agent_running__"]);const before=await waitForAgentState(backend,"agent-1","running");expect(before?.state).toBe("running");const queued=await backend.handle("enqueueMessage",["session-1","cut-in-keep"]) as any;await backend.handle("cutInQueuedMessage",["session-1",queued.message.id]);await waitForNoPrompt(promptLog,"/subagent_abort_all");const after=await waitForAgentState(backend,"agent-1","running");expect(after?.state).toBe("running");await backend.close();});
it("close still sweeps running background subagents",async()=>{const {backend,promptLog}=await backendWithPromptLog("pipi-pi-close-sweep-");await backend.handle("sendPrompt",["session-1","__agent_running__"]);const before=await waitForAgentState(backend,"agent-1","running");expect(before?.state).toBe("running");await backend.close();await waitForPrompt(promptLog,"/subagent_abort_all");});
...
await expect(backend.handle("mergeWorktree",["session-1","agent-1","run-1"])).rejects.toThrow(/not implemented in this host yet/);
...
const fresh=createPiHostBackend({agentDir:join(root,"agent"),sessionsRoot:join(root,"sessions"),runtimeRoot:join(root,"runtime"),...spawnPi()});await expect(fresh.handle("getSessionStats",[])).rejects.toThrow(/no active session/);});
...
const statuses=events.filter(e=>e.channel==="stream"&&e.event.type==="status").map(e=>e.event);expect(statuses).toEqual([
  expect.objectContaining({status:"started",turnEpoch:expect.any(Number)}),
  expect.objectContaining({status:"stopped",turnEpoch:statuses[0].turnEpoch}),
]);});
