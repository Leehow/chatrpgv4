...
import firecrawlAnydoc, {
  ...
  resolveAnydocSource,
} from "../../../packs/document-workbench/agent/pipiui-firecrawl-anydoc.ts";
import {
  ANYDOC_VERSION,
  anydocEngineAvailability,
  anydocNativePackageName,
  convertBytesWithAnydoc,
} from "../../../packs/document-workbench/agent/anydoc-local.ts";
...
describe("pipiui-firecrawl-anydoc local extraction", () => {
  ...
  it("rejects uploads, PDFs, relative paths, and unknown formats", () => {
    ...
    expect(resolveAnydocSource("/abs/scan.pdf")).toMatchObject({ error: expect.stringContaining("不解析 PDF") });
  ...
  it("imports the official anydoc wasm adapter", async () => {
    const manifest = JSON.parse(await readFile(new URL("../../../packs/document-workbench/anydoc/node_modules/@firecrawl/anydoc/package.json", import.meta.url), "utf8"));
    ...
    const shipped = new URL("../../../packs/document-workbench/anydoc/node_modules", import.meta.url);
