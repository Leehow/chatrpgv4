...
import { MANAGED_PACKAGES, managedPackageRoot } from "../src/spawn-assembly.js";

const temp = () => mkdtemp(join(tmpdir(), "managed-npm-"));
const web = MANAGED_PACKAGES[0];

/** Lays out exactly what `managedPackageRoot` looks for, so both halves are checked against one shape. */
...
describe("ensureManagedPackage", () => {
  it("treats an already-installed pinned version as present without invoking npm", async () => {
    ...
    try {
      ...
      // The root lookup finds the same tree the installer just accepted.
      expect(managedPackageRoot(root, web)).toBe(join(root, "managed-npm", `${web.name}-${web.version}`, "node_modules", web.name));
