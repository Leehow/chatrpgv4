...
type RecordedCall = { sessionId: string; payload: QueuedDispatchPayload; behavior: DispatchBehavior };
...
describe("SessionMessageQueue", () => {
  it("disposes one session idempotently without resurrecting it from a late dispatch", async () => {
    ...
    queue.markBusy("s1");
    queue.enqueue("s1", { text: "delete me" });
    queue.markBusy("s2");
    queue.enqueue("s2", { text: "keep me" });

    const idle = queue.notifyIdle("s1");
    expect(host.calls).toHaveLength(1);
    expect(queue.sessionCount()).toBe(2);

    queue.disposeSession("s1");
    queue.disposeSession("s1");
    expect(queue.sessionCount()).toBe(1);
    expect(queue.hasSession("s1")).toBe(false);
    expect(queue.listQueue("s1")).toEqual([]);
    expect(queue.listQueue("s2")).toEqual([expect.objectContaining({ text: "keep me" })]);
    ...
    expect(queue.listQueue("s1")).toEqual([]);
    expect(queue.listQueue("s2")).toEqual([expect.objectContaining({ text: "keep me" })]);
    queue.finalizeSessionDisposal("s1");
  });
  ...
  it("discards stale renderer timing after queued content edits", async () => {
    const host = recordingHost();
    const queue = new SessionMessageQueue({ dispatch: host.dispatch });
    queue.markBusy("s1");
    const original = queue.enqueue("s1", {
      text: "original",
      attachments: [{ dataBase64: "aGVsbG8=", mimeType: "image/png" }],
      turnTelemetry: { turnId: "stale", submittedAt: 1, promptBytes: 8, attachmentBytes: 5 },
    }).message;
    const edited = queue.updateMessage("s1", original.id, {
      text: "edited",
      attachments: [{ dataBase64: "d29ybGQ=", mimeType: "image/jpeg" }],
    });
    expect(edited.turnTelemetry).toBeUndefined();

    const idle = queue.notifyIdle("s1");
    expect(host.calls[0]?.payload.turnTelemetry).toBeUndefined();
    host.pending[0].resolve(undefined);
    await idle;
  });

  it("refreshes renderer timing when an edited queued item supplies a new sample", async () => {
    const host = recordingHost();
    const queue = new SessionMessageQueue({ dispatch: host.dispatch });
    queue.markBusy("s1");
    const original = queue.enqueue("s1", {
      text: "original",
      turnTelemetry: { turnId: "old", submittedAt: 1, promptBytes: 8 },
    }).message;
    const refreshed = { turnId: "fresh", submittedAt: 10, promptBytes: 12 } as const;
    const edited = queue.updateMessage("s1", original.id, { text: "edited", turnTelemetry: refreshed });
    expect(edited.turnTelemetry).toEqual(refreshed);

    const idle = queue.notifyIdle("s1");
    expect(host.calls[0]?.payload.turnTelemetry).toEqual(refreshed);
    host.pending[0].resolve(undefined);
    await idle;
  });
  ...
  it("requeueAtHead restores payloads at the FIFO head without dispatching", () => {
    const host = recordingHost();
    const queue = new SessionMessageQueue({ dispatch: host.dispatch });
    queue.markBusy("s1");
    queue.enqueue("s1", { text: "existing" });
    const restored = queue.requeueAtHead("s1", [{ text: "first" }, { text: "second" }]);
    expect(restored.map((item) => item.text)).toEqual(["first", "second"]);
    expect(queue.listQueue("s1").map((item) => item.text)).toEqual(["first", "second", "existing"]);
    expect(queue.listQueue("s1").every((item) => item.state === "queued")).toBe(true);
    expect(host.calls).toHaveLength(0); // the caller owns the drain
    expect(queue.requeueAtHead("s1", [{ text: "   " }])).toEqual([]); // empty payloads are skipped
    expect(queue.listQueue("s1")).toHaveLength(3);
  });
  ...
  it("treats an immediate-send race after direct acceptance as idempotent success", async () => {
    const host = recordingHost();
    const queue = new SessionMessageQueue({ dispatch: host.dispatch });
    const direct = queue.enqueue("s1", { text: "already accepted" });
    expect(direct.outcome).toBe("dispatched");
    host.pending[0].resolve(undefined);
    await queue.waitForDispatch("s1");
    expect(queue.listQueue("s1")).toEqual([]);

    const accepted = await queue.cutInMessage("s1", direct.message.id);
    await queue.waitForMessageDelivery("s1", direct.message.id);

    expect(accepted).toMatchObject({ id: direct.message.id, text: "already accepted", state: "sending" });
    expect(host.calls.map((call) => call.payload.text)).toEqual(["already accepted"]);
  });
  ...
  it("acknowledges immediate-send only after delivery and retains a failed item for retry", async () => {
    const host = recordingHost();
    const queue = new SessionMessageQueue({ dispatch: host.dispatch });
    queue.markBusy("s1");
    const chosen = queue.enqueue("s1", { text: "chosen" }).message;
    await queue.cutInMessage("s1", chosen.id);
    const accepted = queue.waitForMessageDelivery("s1", chosen.id);

    const idle = queue.notifyIdle("s1");
    expect(host.calls.map((call) => call.payload.text)).toEqual(["chosen"]);
    host.pending[0].reject(new Error("Boss rejected prompt"));
    await idle;

    await expect(accepted).rejects.toThrow(/immediate send failed: Boss rejected prompt/);
    expect(queue.listQueue("s1")).toEqual([
      expect.objectContaining({ id: chosen.id, text: "chosen", state: "failed", error: "Boss rejected prompt" }),
    ]);
    expect(queue.retryMessage("s1", chosen.id)).toMatchObject({ id: chosen.id, state: "queued", error: undefined });
  });
  ...
  it("manual-stop suppression keeps restored items undelivered until the host lifts it", async () => {
    // Mirrors the backend contract: a user Stop persists suppression;
    // restoreQueue may resurrect items as queued but can never re-arm the
    // idle/watchdog/compaction drain past this gate. Only an explicit user
    // send (host clears the flag) unlocks delivery again.
    let suppressed = true;
    const host = recordingHost();
    const queue = new SessionMessageQueue({
      dispatch: host.dispatch,
      isDrainSuppressed: () => suppressed,
    });
    // A persisted `sending` item restored conservatively to `queued`.
    queue.restoreQueue("s1", [
      { id: "q1", sessionId: "s1", text: "stopped mid-send", attachments: [], createdAt: 1, state: "sending" },
    ]);

    await queue.notifyIdle("s1");
    expect(queue.listQueue("s1").map((item) => ({ text: item.text, state: item.state })))
      .toEqual([{ text: "stopped mid-send", state: "queued" }]);
    expect(host.calls).toHaveLength(0);

    // Watchdog orphan-busy recovery also funnels notifyIdle → drain.
    await queue.notifyIdle("s1", 0);
    expect(host.calls).toHaveLength(0);

    // Explicit user send lifts suppression; next idle delivers FIFO head once.
    suppressed = false;
    const idle = queue.notifyIdle("s1");
    expect(host.calls.map((call) => call.payload.text)).toEqual(["stopped mid-send"]);
    host.pending[0].resolve(undefined);
    await idle;
  });
});
