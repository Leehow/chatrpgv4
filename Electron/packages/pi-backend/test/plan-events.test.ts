...
describe("plan events → Plan panel", () => {
  ...
  it("advertises the plan capability only when the plan runtime is really installed", async () => {
    ...
    // A package declaring the plan tool is what makes the surface real: whichever
    // extension owns `plan_publish` is the one whose events the panel receives.
    // Installed the only way a package is installed: its directory placed in an
    // extensions home the loader scans — here the App profile of the next host.
    const planPackage = join(root, "agent-mounted", "extensions", "plan-extension");
    const planDir = join(planPackage, "agent");
    await mkdir(planDir, { recursive: true });
    await writeFile(join(planDir, "index.ts"), "export default () => undefined\n", "utf8");
    await writeFile(join(planPackage, "pipiui-extension.json"), JSON.stringify({
      id: "plan-extension",
      name: "PipiUI Plan",
      version: "1.0.0",
      capabilities: [],
      agent: { extension: "agent/index.ts", tools: ["plan_publish"] },
    }), "utf8");
  ...
  it("reports no plan capability when no installed package declares the plan tool", async () => {
    root = await mkdtemp(join(tmpdir(), "pipi-plan-off-"));
    const runtimeRoot = join(root, "runtime");
    const otherDir = join(runtimeRoot, "extensions", "other-extension", "agent");
    await mkdir(otherDir, { recursive: true });
    await writeFile(join(otherDir, "index.ts"), "export default () => undefined\n", "utf8");
    await writeFile(join(runtimeRoot, "extensions", "other-extension", "pipiui-extension.json"), JSON.stringify({
      id: "other-extension",
      name: "Other",
      version: "1.0.0",
      capabilities: [],
      agent: { extension: "agent/index.ts", tools: ["other_tool"] },
    }), "utf8");
