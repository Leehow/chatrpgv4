...
type Live = {
  ...
  /** When the host last abandoned a call on this turn (contract §NN). */
  hostAbortedTurnAt?: number;
  /** When the current turn's most recent assistant message began (contract §NN). */
...
/**
 * Contract §NN: does the host's abandonment of this turn still cover the silence being measured?
 *
...
export class PiHostBackend implements HostBackend {
  ...
  private async abortSessionTurn(
    ...
    if (live) {
      ...
      // §NN: which call was abandoned, not merely that one was. The watchdog compares this against the
  ...
  private rpcEvent(live: Live, e: Rpc) {
    ...
    // §NN: the only event that says a provider call has gone in flight. A continuation emits no
  ...
  private async checkTurnWatchdogs(): Promise<void> {
    ...
    for (const live of this.live.values()) {
      ...
      if (tailState !== "terminal") {
        // §NN: `abandonmentStillCoversTheSilence`, not `hostAbortedTurn`. The flag alone latched the
