...
export const MAX_ANYDOC_BYTES = 256 * 1024 * 1024;
...
/**
 * Where the anydoc engine lives for this host.
 *
 * The engine belongs to the `document-workbench` package, and the base ships no
 * packages, so there is no shipped fallback to guess at: the caller passes the
 * installed package's `anydoc` directory (or `PIPIUI_ANYDOC_ROOT` names it).
 * Neither present means no engine, and conversion reports that instead of
 * probing a path that was never installed.
 */
export function anydocNodeModules(anydocRoot?: string): string | undefined {
  const root = anydocRoot?.trim() || process.env.PIPIUI_ANYDOC_ROOT?.trim();
  return root ? join(root, "node_modules") : undefined;
}
...
export async function convertDocumentFileToMarkdown(
  ...
  try {
    ...
    const nodeModules = anydocNodeModules(options.anydocRoot);
    if (!nodeModules) return undefined;
    const bytes = await readFile(target);
    const markdown = (await convertBytes(bytes, target, nodeModules)).trim();
