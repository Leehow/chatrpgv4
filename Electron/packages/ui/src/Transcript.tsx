...
type PresentationActionProps = {onChoose?: (entry:NonNullable<ChatMessage['presentation']>,option:string)=>Promise<unknown>; onDraftOverride?: (entry:NonNullable<ChatMessage['presentation']>,request:Record<string,unknown>)=>Promise<Record<string,any>>; onDraftConfirm?: DraftAction; onDraftSpread?: DraftAction; onDraftReroll?: DraftAction; onDraftCatalog?: DraftAction}
...
export function Transcript({ stateKey, messages: rawMessages, transcriptRef, waiting, active = true, onLoadOlder, documentBasePath, onOpenDocument, onOpenSubagents, onChoose, onDraftOverride, onDraftConfirm, onDraftSpread, onDraftReroll, onDraftCatalog, onCopy, onResend, resendDisabled, copiedId, onBranch, branchMessageIds, branchDisabled, onIllustrate, illustrateDisabled, illustrations, actionWords, navigation }: {
...
export const MessageView = memo(function MessageView({ message, showFooter, draftSuperseded, documentBasePath, onOpenDocument, onOpenSubagents, onChoose, onDraftOverride, onDraftConfirm, onDraftSpread, onDraftReroll, onDraftCatalog, onCopy, onResend, resendDisabled, copied, onBranch, branchMessageIds, branchDisabled, onIllustrate, illustrateDisabled, illustration, actionWords, canJump = false, onJump }: { message: ChatMessage; showFooter?: boolean; draftSuperseded?: boolean; copied?: boolean; canJump?: boolean; onJump?: () => void; illustration?: IllustrationState } & DocumentOpenProps & SubagentOpenProps & PresentationActionProps & Omit<MessageActionHandlers, 'copiedId' | 'illustrations'>) {
  ...
  if (message.presentation) return <><PresentationEntry message={message} illustration={illustrationNode} draftSuperseded={draftSuperseded} onChoose={onChoose} onDraftOverride={onDraftOverride} onDraftConfirm={onDraftConfirm} onDraftSpread={onDraftSpread} onDraftReroll={onDraftReroll} onDraftCatalog={onDraftCatalog} />{marked && <div className="message presentation-footer">{footer}</div>}</>
...
/**
 * A word for a superseded draft row, from the projection the row already carries.
 *
 * The card's own words arrive on `details.presentation.texts`; a row whose projection never
 * landed, or whose glossary has no entry for this caption, shows the English literal, which is a
 * gap a player can report. Nothing here is a table, and nothing is written per language.
 */
function draftWord(details: unknown, key: string): string {
  const texts=(details as {presentation?:{texts?:Record<string,string>}})?.presentation?.texts
  const word=texts?.[key]
  return typeof word==='string'&&word?word:key
}

function PresentationEntry({message,illustration,draftSuperseded,onChoose,onDraftOverride,onDraftConfirm,onDraftSpread,onDraftReroll,onDraftCatalog}:{message:ChatMessage;illustration?:ReactNode;draftSuperseded?:boolean}&PresentationActionProps) {
  useToolRenderers()
