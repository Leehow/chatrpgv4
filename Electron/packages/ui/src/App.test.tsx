...
const virtuosoHarness = {
  atBottom: undefined as undefined | ((value: boolean) => void),
  startReached: undefined as undefined | ((index: number) => void),
  scrollToIndex: vi.fn(),
}
...
beforeEach(() => {
  ...
  virtuosoHarness.startReached = undefined
})
afterEach(() => {
  cleanup()
  vi.clearAllTimers()
...
vi.mock('react-virtuoso', async () => {
  ...
  return { Virtuoso: React.forwardRef(({ data, itemContent, atBottomStateChange, startReached }: { data: unknown[]; itemContent: (index: number, item: never) => JSX.Element; atBottomStateChange?: (value: boolean) => void; startReached?: (index: number) => void }, ref) => { virtuosoHarness.atBottom = atBottomStateChange; virtuosoHarness.startReached = startReached; React.useImperativeHandle(ref, () => ({ scrollToIndex: virtuosoHarness.scrollToIndex })); return <div>{data.map((item, index) => <React.Fragment key={index}>{itemContent(index, item as never)}</React.Fragment>)}</div> }) }
})
...
describe('PipiUI main layout', () => {
  ...
  it('loads older history pages without gaps and preserves the newest successful page when an older read fails', async () => {
    ...
    act(() => virtuosoHarness.startReached?.(0))
  ...
  it('does not let a late history snapshot swallow a live user follow-up or subagent confirmation', async () => {
    const base = createMockHost()
    let listener: ((event: StreamEvent) => void) | undefined
    let welcomeReads = 0
    const durable: HistoryEntry[] = [
      { id: 'u1', role: 'user', content: '先派发子任务', timestamp: 1 },
    ...
    render(<App host={host} />)
    expect(await screen.findByText('先派发子任务')).toBeTruthy()
    await waitFor(() => expect(listener).toBeDefined())
    const readsAfterLoad = welcomeReads

    act(() => {
      listener?.({ type: 'user_message', sessionId: 'welcome', id: 'done-1', content: '[subagent-done] agentId=a1 name=explore ok=true\nTitle: 修合流' })
      listener?.({ type: 'user_message', sessionId: 'welcome', id: 'steer-1', content: '先停一下，改验证标准' })
    })
    expect(await screen.findByText('先停一下，改验证标准')).toBeTruthy()
    expect(screen.getByTestId('subagent-signal-card').textContent).toContain('修合流')

    act(() => listener?.({ type: 'status', sessionId: 'welcome', status: 'settled' }))
    await waitFor(() => expect(welcomeReads).toBeGreaterThan(readsAfterLoad))
    expect(screen.getByText('先停一下，改验证标准')).toBeTruthy()
    expect(screen.getByTestId('subagent-signal-card').textContent).toContain('修合流')
  })
  ...
  it('ignores a background ghost started after a finished assistant', async () => {
    ...
    const { container } = render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    // The sidebar renders inside the active form, so wait for the row itself.
    ...
    await act(async () => { background?.({ type: 'status', sessionId: 'tool-burst', status: 'started' }) })
  ...
  it('keeps a sent session marked running in the sidebar after switching away', async () => {
    ...
    await waitFor(() => expect(container.querySelector('[data-session-id="site"]')).toBeTruthy())
  ...
  it('fits a remembered browser pane so the chat column keeps 360px', () => {
    expect(fitBrowserToolsWidth(1280, 258, 586)).toBe(586)
    expect(fitBrowserToolsWidth(1024, 258, 520)).toBe(394)
    expect(fitBrowserToolsWidth(1600, 258, 920)).toBe(920)
    expect(fitBrowserToolsWidth(800, 0, 920)).toBe(428)
    expect(fitBrowserToolsWidth(2000, 258, 400)).toBe(520)
  })
  ...
  it('opens Browser as a wide adjustable IDE split while keeping chat visible', async () => {
    ...
    const sidebarWidth = Number.parseInt(shell.style.getPropertyValue('--sidebar-w'), 10)
    expect(browserWidth).toBeGreaterThanOrEqual(320)
    expect(sidebarWidth + PANE_HANDLE_TRACKS + browserWidth + CHAT_MIN_WIDTH).toBeLessThanOrEqual(window.innerWidth)
    expect(JSON.parse(localStorage.getItem('pipiui:eui-pane-widths')!)).toMatchObject({ browserTools: expect.any(Number) })
  })
  ...
  it('renders the floating quick rail over the chat column and toggles/closes tools', async () => {
    ...
    // The Plan rail button appears once the session's async getPlans resolves —
    // the unscoped rail ships only 4 buttons until then.
    await waitFor(() => expect(rail.querySelectorAll('button')).toHaveLength(5))
  ...
  it('defines narrow breakpoints that collapse panes, overlay expanded panes, and cap overflow', () => {
    ...
    expect(css).toMatch(/@media \(max-width:899px\)\{[^\n]*\.pipiui-shell\.browser-workspace\{[^}]*--tools-col:var\(--tools-w/)
    // 720px breakpoint: auto-collapse hides panes, re-expanded panes become overlays.
    // The five pane tracks must stay in place — a 3-column template would put the
    // left handle in the 1fr track and crush chat whenever both overlays are open.
    const narrow = css.match(/@media \(max-width:720px\)\{[^\n]*\}/)?.[0] ?? ''
    expect(narrow).toMatch(/grid-template-columns:0 0 minmax\(0,1fr\) 0 0/)
    expect(narrow).not.toMatch(/grid-template-columns:0 minmax\(0,1fr\) 0[;}]/)
    ...
    // Mobile drawer scroll contract: the drawer overlays from the left edge at
    // its capped width (no page widening) and the document itself never scrolls.
    expect(narrow).toMatch(/\.pipiui-shell \.sb-root\{[^}]*left:0/)
    expect(css).toMatch(/html,body,#root\{width:100%;height:100%;min-width:0;min-height:0;overflow:hidden\}/)
  ...
  it('replaces the desktop vertical float rail with a header-docked horizontal phone bar', () => {
    const css = readFileSync(join(import.meta.dirname, 'app.css'), 'utf8')
    // Phone-only block (≤720px): the rail becomes a horizontal bar under the
    // chat header instead of the tall desktop column, safe-area aware.
    const phone = css.match(/@media \(max-width:720px\)\{\.tool-quick-rail-float[^\n]*\}/)?.[0] ?? ''
    expect(phone).toContain('flex-direction:row')
    expect(phone).toMatch(/top:calc\(60px \+ env\(safe-area-inset-top/)
  ...
  it('keeps the compact phone rail usable over the chat while panes open as overlays', async () => {
    ...
    const shell = container.querySelector('.pipiui-shell')!
    expect(shell.className).toContain('tools-collapsed')
    // Collapsed tools ship the float rail inside the chat viewport — not a
    // fixed side pane — and the composer stays mounted and usable.
    const floatRail = container.querySelector('.chat-viewport .tool-quick-rail')!
    expect(floatRail.className).toContain('tool-quick-rail-float')
    expect(screen.getByLabelText('消息输入框')).toBeTruthy()
    await waitFor(() => expect((screen.getByRole('button', { name: 'Browser' }) as HTMLButtonElement).disabled).toBe(false))
    // Tapping a tool opens the pane as a full overlay with its own header
    // tabs; the float launcher unmounts so nothing obscures the overlay.
    fireEvent.click(screen.getByRole('button', { name: 'Browser' }))
    expect(await screen.findByTestId('browser-panel')).toBeTruthy()
    expect(shell.className).not.toContain('tools-collapsed')
    expect(container.querySelector('.chat-viewport .tool-quick-rail')).toBeNull()
    expect(container.querySelector('.tool-panel-header')!.contains(container.querySelector('.tool-quick-rail'))).toBe(true)
    // Re-tapping the active tool closes the overlay and restores the bar; the
    // document never gains scroll: the shell lock stays in place throughout.
    fireEvent.click(screen.getByRole('button', { name: 'Browser' }))
    expect(shell.className).toContain('tools-collapsed')
    expect(container.querySelector('.chat-viewport .tool-quick-rail')).toBeTruthy()
    expect(screen.getByLabelText('消息输入框')).toBeTruthy()
    vi.unstubAllGlobals()
  })

  it('collapses the phone tool rail to the active tool and remembers the choice after reopening a pane', async () => {
    const media = { matches: true, media: '(max-width: 720px)', onchange: null, addEventListener: vi.fn(), removeEventListener: vi.fn(), addListener: vi.fn(), removeListener: vi.fn(), dispatchEvent: vi.fn() } as unknown as MediaQueryList
    vi.stubGlobal('matchMedia', vi.fn(() => media))
    localStorage.removeItem('pipiui:eui:tool-quick-rail-collapsed:v1')
    const { container } = render(<App host={createMockHost()} />)
    await screen.findAllByText('Electron 三栏界面')

    const floatRail = () => container.querySelector('.chat-viewport .tool-quick-rail')
    expect(screen.getByRole('button', { name: '收起工具快捷栏' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Browser' })).toBeTruthy()
    fireEvent.click(screen.getByRole('button', { name: '收起工具快捷栏' }))

    expect(floatRail()?.className).toContain('is-collapsed')
    expect(screen.getByRole('button', { name: '展开工具快捷栏' })).toBeTruthy()
    expect(screen.getByRole('button', { name: 'Subagents' })).toBeTruthy()
    expect(screen.queryByRole('button', { name: 'Browser' })).toBeNull()
    expect(localStorage.getItem('pipiui:eui:tool-quick-rail-collapsed:v1')).toBe('true')

    fireEvent.click(screen.getByRole('button', { name: 'Subagents' }))
    expect(floatRail()).toBeNull()
    fireEvent.click(screen.getByRole('button', { name: 'Subagents' }))
    expect(floatRail()?.className).toContain('is-collapsed')

    fireEvent.click(screen.getByRole('button', { name: '展开工具快捷栏' }))
    expect(screen.getByRole('button', { name: 'Browser' })).toBeTruthy()
    expect(localStorage.getItem('pipiui:eui:tool-quick-rail-collapsed:v1')).toBe('false')
    vi.unstubAllGlobals()
  })

  it('phone overlays are mutually exclusive so drawer footer buttons are never covered', async () => {
    const media = { matches: true, media: '(max-width: 720px)', onchange: null, addEventListener: vi.fn(), removeEventListener: vi.fn(), addListener: vi.fn(), removeListener: vi.fn(), dispatchEvent: vi.fn() } as unknown as MediaQueryList
    vi.stubGlobal('matchMedia', vi.fn(() => media))
    const { container } = render(<App host={createMockHost()} />)
    await screen.findAllByText('Electron 三栏界面')
    const shell = container.querySelector('.pipiui-shell')!
    // Regression at 390x844: tools (Subagents) already expanded, then the left
    // drawer opened — both fixed panes stacked and .tool-panel swallowed taps
    // aimed at the drawer footer (theme button reported target_obscured).
    fireEvent.click(screen.getByLabelText('展开右栏'))
    expect(shell.className).not.toContain('tools-collapsed')
    const storedBefore = localStorage.getItem('pipiui:eui-pane-widths')
    fireEvent.click(screen.getByLabelText('展开左栏'))
    // Opening the drawer must close the tool panel: .sb-root becomes the only
    // visible overlay, so nothing later in DOM can cover its footer controls.
    expect(shell.className).not.toContain('sidebar-collapsed')
    expect(shell.className).toContain('tools-collapsed')
    const themeButton = screen.getByTestId('sidebar-theme-toggle') as HTMLButtonElement
    ...
    expect(shell.className).toContain('sidebar-collapsed')
    expect(shell.className).not.toContain('tools-collapsed')
    // Closing stays straightforward: one tap collapses the active overlay.
    fireEvent.click(screen.getByTestId('toggle-tools'))
    expect(shell.className).toContain('tools-collapsed')
    expect(shell.className).toContain('sidebar-collapsed')
    expect(localStorage.getItem('pipiui:eui-pane-widths')).toBe(storedBefore)
    vi.unstubAllGlobals()
  })
  ...
  it('keeps collapsed drag handles in the grid flow instead of removing them', () => {
    ...
    // Extra siblings (narrow backdrop, modals) must not steal auto-placement tracks.
    expect(desktop).toMatch(/\.pipiui-shell>\.sb-root\{grid-column:1;grid-row:1\}/)
    expect(desktop).toMatch(/\.pipiui-shell>\.resize-handle-left\{grid-column:2;grid-row:1\}/)
    expect(desktop).toMatch(/\.pipiui-shell>\.chat-column\{grid-column:3;grid-row:1\}/)
    expect(desktop).toMatch(/\.pipiui-shell>\.resize-handle-right\{grid-column:4;grid-row:1\}/)
    expect(desktop).toMatch(/\.pipiui-shell>\.tool-panel\{grid-column:5;grid-row:1\}/)
    expect(css).toMatch(/\.pane-overlay-backdrop\{[^}]*grid-column:1\/-1/)
  })
  ...
  it('uses the pane headers as macOS hiddenInset chrome in Electron only', async () => {
    ...
    // Real Electron UAs always carry the framework token even after the app
    // rename ("…PipiUI/32.3.3 … Electron/32.3.3"); isElectronChrome() keys on it.
    Object.defineProperty(navigator, 'userAgent', { value: 'Mozilla/5.0 PipiUI/32.3.3 Electron/32.3.3', configurable: true })
    try {
      const { container } = render(<App host={createMockHost()} />)
      await screen.findAllByText('Electron 三栏界面')
  ...
  it.each([
    ...
    host.capabilities = async () => mockCapabilities({ revealInFinder: true, terminal: true, documents: true, browser: true, git: true, plan: false, retainedWorktreeDisposition: false })
  ...
  it('reveals the exact Terminal only for the currently selected chat session', async () => {
    ...
    host.capabilities = async () => mockCapabilities({ revealInFinder: true, terminal: true, browser: true, git: true, plan: false, retainedWorktreeDisposition: false })
    render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    await waitFor(() => expect((screen.getByRole('button', { name: 'Terminal' }) as HTMLButtonElement).disabled).toBe(false))
  ...
  it('keeps Subagent selection/disclosure/scroll and the terminal instance across rail switches', async () => {
    ...
    const host: PipiHostAPI = { ...base, terminal: terminalHost, capabilities: async () => mockCapabilities({ revealInFinder: true, terminal: true, documents: true, browser: true, git: true, plan: false, retainedWorktreeDisposition: false }) }
    ...
    // Subagent transcript step groups default collapsed; the boss expands them manually.
    const stepsDetail = await within(subagentTranscript).findByRole('button', { name: /个步骤/ })
    expect(stepsDetail.getAttribute('aria-expanded')).toBe('false')
    fireEvent.click(stepsDetail)
    expect(stepsDetail.getAttribute('aria-expanded')).toBe('true')
    // Nested ordinary tools stay folded after the outer card opens.
  ...
  it('does not attach a panel-opened document to the next prompt', async () => {
    const host = createMockHost()
    const notifyDocumentsDropped = vi.fn(async () => undefined)
    ...
    fireEvent.change(composer, { target: { value: '把这个 bug 改一下' } })
    await waitFor(() => expect((screen.getByLabelText('发送消息') as HTMLButtonElement).disabled).toBe(false))
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '把这个 bug 改一下', undefined, rendererTelemetryMatcher()))
    expect(notifyComposerDocumentsDropped).not.toHaveBeenCalled()
  ...
  it('attaches a document drop on the composer as a removable chip without opening the panel', async () => {
    ...
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '请分析这些文件的内容', undefined, rendererTelemetryMatcher()))
    expect(notifyDocumentsDropped).not.toHaveBeenCalled()
  ...
  it('disables Browser for a remote host without browser capability', async () => {
    const base = createMockHost()
    const host: PipiHostAPI = { ...base, capabilities: async () => mockCapabilities({ revealInFinder: true, terminal: true, browser: false, plan: false, retainedWorktreeDisposition: false }) }
    render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    const browser = screen.getByRole('button', { name: 'Browser' }) as HTMLButtonElement
  ...
  it('disables server-only browser, terminal, and unavailable Finder controls', async () => {
    const base = createMockHost()
    const host: PipiHostAPI = { ...base, capabilities: async () => mockCapabilities({ revealInFinder: false, terminal: false, browser: false, plan: false, retainedWorktreeDisposition: false }) }
    render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    await waitFor(() => {
  ...
  it('renders a header action contribution inside the chat header', async () => {
    // The real git branch pill is contributed by git-capability's runtime app half
    // (packs/git-capability/app/branch-menu.tsx), which is loaded
    // through the controlled-component pipeline and covered there
    // (controlled-component-loader.test.ts: "loads the real git-capability manifest header
    // action ..."). The mock host used here does not drive that pipeline, so this test
    // exercises the generic mechanism the shell offers to any such contribution — an
    // extension registers a header action and App renders it inside `.chat-header` — with an
    // inline fake contribution, instead of special-casing git in the shell.
    // 'demo-workbench' is the form the mock host ships enabled (a fixture: the base
    // itself ships no form), so the shell activates it (and its contributions) from
    // listExtensions like any other enabled extension — no need to opt into the fuller
    // productPacks mock just to exercise the render path.
    const dispose = registerHeaderAction('demo-workbench', {
      id: 'test.header-action',
      render: () => <button data-testid="test-header-action">测试头部按钮</button>,
    })
    try {
      render(<App host={createMockHost()} />)
      const action = await screen.findByTestId('test-header-action')
      expect(action.closest('.chat-header')).not.toBeNull()
    } finally {
      dispose()
    }
  })
  ...
  it('shows a read-only lease and allows explicit takeover', async () => {
    ...
    render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    // The lease read is async: wait for read-only to land before asserting it.
    await waitFor(() => expect((screen.getByLabelText('消息输入框') as HTMLTextAreaElement).disabled).toBe(true))
    await waitFor(() => expect(screen.getByText(/由 pipiui-swift 运行中/)).toBeTruthy())
  ...
  it('keeps the fixed Composer and explicit takeover available for a real canWrite/ownerLabel read-only lease', async () => {
    ...
    // The lease read (getSessionLease) is async: wait for the read-only state to
    // land before asserting the composer it gates.
    await waitFor(() => expect(textarea.disabled).toBe(true))
    await waitFor(() => expect(screen.getByText(/由 pipiui-electron 运行中/)).toBeTruthy())
  ...
  it('keeps the context ring/used-window and adds the Swift-style quota pill, both from the selected session', async () => {
    ...
    const getQuotaSnapshot = vi.fn(async (sessionId?: string) => sessionId === 'layout'
      ? { provider: 'openai', accountLabel: 'Codex 账号额度', windows: [{ id: 'primary', usedPercent: 63, label: '5h', title: '5小时额度' }, { id: 'secondary', usedPercent: 12, label: '周', title: '周额度' }] }
      : { provider: 'openai', accountLabel: 'Codex 账号额度', windows: [{ id: 'primary', usedPercent: 4, label: '5h', title: '5小时额度' }, { id: 'secondary', usedPercent: 12, label: '周', title: '周额度' }] })
    const host: PipiHostAPI = {
      ...base,
      getSessionStats,
      getQuotaSnapshot,
  ...
  it('starts from the system-resolved dark theme when no manual preference is stored', async () => {
  ...
  it('switches theme from the sidebar footer toggle, persists it, and keeps it over later system flips', async () => {
    let listener: ((event: MediaQueryListEvent) => void) | undefined
    const media = { matches: false, media: '(prefers-color-scheme: dark)', onchange: null, addEventListener: (_: string, callback: (event: MediaQueryListEvent) => void) => { listener = callback }, removeEventListener: () => undefined, addListener: () => undefined, removeListener: () => undefined, dispatchEvent: () => true } as MediaQueryList
    vi.stubGlobal('matchMedia', vi.fn(() => media))
    window.localStorage.removeItem('pipiui.theme')
  ...
  it('auto-expands a collapsed project when selecting a non-working session', async () => {
    ...
    const { container } = render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    // Default expansion lands one effect after the project list, so wait for the collapse
    // control rather than racing it: clicking before it exists fails on a busy machine.
    fireEvent.click(await screen.findByRole('button', { name: '收起项目 Website' }))
    await waitFor(() => expect(screen.getByRole('button', { name: '展开项目 Website' })).toBeTruthy())
  ...
  it('does not auto-expand a collapsed project when selecting a pinned session', async () => {
    const host = createMockHost()
    const workspace = await host.listProjects()
    localStorage.setItem(sidebarPreferencesKey(workspace), JSON.stringify({
      expandedIds: [],
      pinnedSessionIds: ['layout'],
      visibleLimit: 10,
    }))
    const { container } = render(<App host={host} />)
    const pinned = await waitFor(() => {
      const row = container.querySelector('.sb-pinned-section [data-session-id="layout"]')
      expect(row).toBeTruthy()
      return row!
    })
    await waitFor(() => expect(screen.getByRole('button', { name: '展开项目 PipiUI' })).toBeTruthy())
    fireEvent.click(pinned)
    await waitFor(() => expect(pinned.getAttribute('aria-current')).toBe('true'))
    expect(screen.getByRole('button', { name: '展开项目 PipiUI' })).toBeTruthy()
    await waitFor(() => expect(JSON.parse(localStorage.getItem(sidebarPreferencesKey(workspace))!).expandedIds).not.toContain('pipiui'))
  })
  ...
  it('keeps sibling episodes of one agentId separate so an old run terminal never stops the current run', () => {
    const run1: AgentSummary = { agentId: 'bg', runId: 'r1', name: 'explore', task: '', state: 'running', sessionId: 'layout', createdAt: 10, updatedAt: 10 }
    const run2: AgentSummary = { ...run1, runId: 'r2', createdAt: 50, updatedAt: 50 }
    // A re-dispatch of the same agentId adds a sibling episode, not an overwrite.
    let merged = mergeAgentSummary([run1], run2)
    expect(merged).toHaveLength(2)
    // A LATE terminal for the retired run repaints only its own row.
    merged = mergeAgentSummary(merged, { ...run1, state: 'interrupted', updatedAt: 500 })
    expect(merged.find(agent => agent.runId === 'r2')?.state).toBe('running')
    expect(merged.find(agent => agent.runId === 'r1')?.state).toBe('interrupted')
    // The last running episode terminating clears every running row.
    merged = mergeAgentSummary(merged, { ...run2, state: 'ok', updatedAt: 600 })
    expect(merged.filter(agent => agent.state === 'running')).toHaveLength(0)
    // Within one run, a late running event must not reopen a terminal verdict.
    expect(mergeAgentSummary(merged, { ...run2, state: 'running', updatedAt: 700 }).find(agent => agent.runId === 'r2')?.state).toBe('ok')
  })

  it('reconciles agent snapshots per episode: fresh terminal wins, stale snapshot cannot cross-run overwrite', () => {
    const running: AgentSummary = { agentId: 'bg', runId: 'r2', name: 'explore', task: '', state: 'running', sessionId: 'layout', createdAt: 50 }
    // End event missed by the subscription: a fresh snapshot terminal for the
    // SAME run must clear the stale running row (last worker ends → cleared).
    expect(mergeAgentSnapshot([running], [{ ...running, state: 'ok', updatedAt: 100 }]).find(agent => agent.runId === 'r2')?.state).toBe('ok')
    // A stale snapshot row captured before the verdict must not downgrade it.
    expect(mergeAgentSnapshot([{ ...running, state: 'ok', updatedAt: 100 }], [running]).find(agent => agent.runId === 'r2')?.state).toBe('ok')
    // The reported bug: an old-run row in a folded snapshot must survive as a
    // sibling WITHOUT repainting the current running run.
    const retired: AgentSummary = { ...running, runId: 'r1', state: 'interrupted', updatedAt: 500 }
    const reconciled = mergeAgentSnapshot([retired], [running])
    expect(reconciled.find(agent => agent.runId === 'r2')?.state).toBe('running')
    expect(reconciled.find(agent => agent.runId === 'r1')?.state).toBe('interrupted')
    expect(reconciled).toHaveLength(2)
  })

  it('shows background subagents running in the sidebar after the Boss turn settled, until the last run ends', () => {
    const agents: AgentSummary[] = [
      { agentId: 'bg', runId: 'r1', name: 'explore', task: '', state: 'running', sessionId: 'subtask' }
    ]
    // Boss turn terminal (observed mapped to completed/interrupted): the row
    // must still advertise the live background worker.
    expect(sidebarStatusForSession('subtask', 'selected', false, 'completed', agents)).toEqual({ status: 'subagents-running', subagentCount: 1 })
    expect(sidebarStatusForSession('subtask', 'selected', false, 'interrupted', agents)).toEqual({ status: 'subagents-running', subagentCount: 1 })
    // Once the last run terminates, the subagents-running projection is gone.
    const ended: AgentSummary[] = [{ ...agents[0], state: 'interrupted' }]
    expect(sidebarStatusForSession('subtask', 'selected', false, 'completed', ended).status).not.toBe('subagents-running')
  })

  it('keeps the subagents-running badge across a late old-run terminal and clears it when the last run ends', async () => {
    const base = createMockHost()
    const agentListeners = new Set<(event: AgentEvent) => void>()
    const host: PipiHostAPI = {
      ...base,
    ...
    const { container } = render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    const emit = (agent: AgentSummary) => act(() => {
      const event: AgentEvent = { type: 'agent', agent }
      for (const listener of agentListeners) listener(event)
    })
    const status = () => container.querySelector('[data-session-id="layout"]')?.getAttribute('data-status')
    emit({ agentId: 'bg', runId: 'r1', sessionId: 'layout', name: 'explore', task: '', state: 'running', updatedAt: 10 })
    await waitFor(() => expect(status()).toBe('subagents-running'))
    // Same agentId re-dispatched: a new episode starts beside the old one.
    emit({ agentId: 'bg', runId: 'r2', sessionId: 'layout', name: 'explore', task: '', state: 'running', updatedAt: 20 })
    expect(status()).toBe('subagents-running')
    // A LATE terminal for the retired run (e.g. a terminal_replay broadcast)
    // must not stop the badge while the successor run is still live.
    emit({ agentId: 'bg', runId: 'r1', sessionId: 'layout', name: 'explore', task: '', state: 'interrupted', updatedAt: 500 })
    expect(status()).toBe('subagents-running')
    // The last running episode terminating clears the running projection promptly.
    emit({ agentId: 'bg', runId: 'r2', sessionId: 'layout', name: 'explore', task: '', state: 'interrupted', updatedAt: 600 })
    await waitFor(() => expect(status()).not.toBe('subagents-running'))
  })

  it('re-syncs background agent rows from the host index when the window regains focus', async () => {
    const base = createMockHost()
    const agentListeners = new Set<(event: AgentEvent) => void>()
    let snapshot: AgentSummary[] = []
    const host: PipiHostAPI = {
      ...base,
      listAgents: async () => snapshot,
      subscribeAgents: listener => {
        agentListeners.add(listener)
        return () => { agentListeners.delete(listener) }
      }
    ...
    await screen.findAllByText('Electron 三栏界面')
    act(() => {
      const event: AgentEvent = { type: 'agent', agent: { agentId: 'bg', runId: 'r1', sessionId: 'layout', name: 'explore', task: '', state: 'running' } }
      for (const listener of agentListeners) listener(event)
    })
    const status = () => container.querySelector('[data-session-id="layout"]')?.getAttribute('data-status')
    await waitFor(() => expect(status()).toBe('subagents-running'))
    // The terminal event was missed (hidden window / silent subscription loss).
    // The durable host index is authoritative: regaining focus must clear it.
    snapshot = [{ agentId: 'bg', runId: 'r1', sessionId: 'layout', name: 'explore', task: '', state: 'ok', updatedAt: 100 }]
  ...
  it('keeps idle submits direct and queues busy composer submits without cutting in', async () => {
    const base = createMockHost()
    ...
    const cutInQueuedMessage = vi.fn(async (sessionId: string, messageId: string) => queuedMessage(messageId, sessionId, 'busy insert', 'sending'))
    const steerQueuedMessage = vi.fn(async (sessionId: string, messageId: string) => queuedMessage(messageId, sessionId, 'busy insert', 'sending'))
    const host: PipiHostAPI = { ...base, sendPrompt, enqueueMessage, cutInQueuedMessage, steerQueuedMessage, listQueue: async () => [] }
    render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    ...
    await waitFor(() => expect((screen.getByLabelText('发送消息') as HTMLButtonElement).disabled).toBe(false))
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', 'idle direct', undefined, rendererTelemetryMatcher()))
    expect(enqueueMessage).not.toHaveBeenCalled()

    fireEvent.change(screen.getByLabelText('消息输入框'), { target: { value: 'busy insert' } })
    await waitFor(() => expect(screen.getByLabelText('发送消息')).toBeTruthy())
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(enqueueMessage).toHaveBeenCalledWith('welcome', 'busy insert', undefined, rendererTelemetryMatcher()))
    expect(cutInQueuedMessage).not.toHaveBeenCalled()
    expect(steerQueuedMessage).not.toHaveBeenCalled()
    expect(sendPrompt).toHaveBeenCalledTimes(1)
  })

  it('keeps a busy queued message out of the transcript until the queue drains it', async () => {
    const base = createMockHost()
    let listener: ((event: StreamEvent) => void) | undefined
    const sendPrompt = vi.fn(async () => undefined)
    const enqueueMessage = vi.fn(async (sessionId: string, text: string) => ({ outcome: 'queued' as const, message: queuedMessage('cut-in-item', sessionId, text) }))
    const cutInQueuedMessage = vi.fn(async (sessionId: string, messageId: string) => queuedMessage(messageId, sessionId, 'cut-in pending', 'sending'))
    ...
    render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    fireEvent.change(screen.getByLabelText('消息输入框'), { target: { value: 'idle hold' } })
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', 'idle hold', undefined, rendererTelemetryMatcher()))

    fireEvent.change(screen.getByLabelText('消息输入框'), { target: { value: 'cut-in pending' } })
    await waitFor(() => expect(screen.getByLabelText('发送消息')).toBeTruthy())
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(enqueueMessage).toHaveBeenCalledWith('welcome', 'cut-in pending', undefined, rendererTelemetryMatcher()))
    expect(cutInQueuedMessage).not.toHaveBeenCalled()
    expect(steerQueuedMessage).not.toHaveBeenCalled()
    // The queued message stays in the queue panel only — no transcript bubble yet.
    await act(async () => { await new Promise(resolve => setTimeout(resolve, 50)) })
    expect([...document.querySelectorAll('[data-user-prompt]')].every(node => !node.textContent?.includes('cut-in pending'))).toBe(true)

    await act(async () => {
      listener?.({ type: 'user_message', sessionId: 'welcome', id: 'echo-cut-in', content: 'cut-in pending' })
    })
    const confirmed = screen.getAllByText('cut-in pending').filter(node => node.closest('[data-user-prompt]'))
    expect(confirmed).toHaveLength(1)
    expect(confirmed[0].closest('[data-user-prompt]')?.getAttribute('data-queued')).toBeNull()
  })

  it('busy queued send stays out of the transcript across stopped reconciliation', async () => {
    const base = createMockHost()
    let listener: ((event: StreamEvent) => void) | undefined
    const sendPrompt = vi.fn(async () => undefined)
    const enqueueMessage = vi.fn(async (sessionId: string, text: string) => ({ outcome: 'queued' as const, message: queuedMessage('cut-in-stop', sessionId, text) }))
    const cutInQueuedMessage = vi.fn(async (sessionId: string, messageId: string) => queuedMessage(messageId, sessionId, 'cut-in after stop', 'sending'))
    const steerQueuedMessage = vi.fn()
    const historyOnlyMarker = 'HISTORY_ONLY_STOPPED_RECONCILE_MARKER'
    const durableHistory: HistoryEntry[] = [
      { id: 'u1', role: 'user', content: '请实现 Electron 三栏主界面。', timestamp: 1 },
      { id: 'a1', role: 'assistant', content: '我会先检查现有结构，然后完成 UI。', timestamp: 2 },
    ]
    const abortedHistory: HistoryEntry[] = [
      ...durableHistory,
      { id: 'u-idle', role: 'user', content: 'idle hold', timestamp: 3 },
      { id: 'a-aborted', role: 'assistant', content: 'partial before cut-in', timestamp: 4 },
      { id: 'u-history-only', role: 'user', content: historyOnlyMarker, timestamp: 5 },
    ]
    let serveAbortedHistory = false
    const getSessionHistory = vi.fn(async () => serveAbortedHistory ? abortedHistory : durableHistory)
    const host: PipiHostAPI = {
      ...base,
      sendPrompt,
      enqueueMessage,
    ...
    await act(async () => {
      listener?.({ type: 'status', sessionId: 'welcome', status: 'started' })
      listener?.({ type: 'text', sessionId: 'welcome', contentIndex: 0, delta: 'partial before cut-in' })
    })

    fireEvent.change(screen.getByLabelText('消息输入框'), { target: { value: 'cut-in after stop' } })
    await waitFor(() => expect(screen.getByLabelText('发送消息')).toBeTruthy())
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(enqueueMessage).toHaveBeenCalledWith('welcome', 'cut-in after stop', undefined, rendererTelemetryMatcher()))
    expect(cutInQueuedMessage).not.toHaveBeenCalled()
    expect(steerQueuedMessage).not.toHaveBeenCalled()
    expect([...document.querySelectorAll('[data-user-prompt]')].every(node => !node.textContent?.includes('cut-in after stop'))).toBe(true)

    serveAbortedHistory = true
    await act(async () => {
      listener?.({ type: 'status', sessionId: 'welcome', status: 'stopped' })
    })
    expect(await screen.findByText(historyOnlyMarker)).toBeTruthy()
    // The post-stop reconciliation must not resurrect the queued item either.
    expect([...document.querySelectorAll('[data-user-prompt]')].every(node => !node.textContent?.includes('cut-in after stop'))).toBe(true)

    await act(async () => {
      listener?.({ type: 'user_message', sessionId: 'welcome', id: 'echo-after-stop', content: 'cut-in after stop' })
    })
    const confirmedAfterStop = screen.getAllByText('cut-in after stop').filter(node => node.closest('[data-user-prompt]'))
    expect(confirmedAfterStop).toHaveLength(1)
    expect(confirmedAfterStop[0].closest('[data-user-prompt]')?.getAttribute('data-queued')).toBeNull()
  })

  it('busy enqueue failure keeps the transcript clean and restores the draft', async () => {
    const base = createMockHost()
    const sendPrompt = vi.fn(async () => undefined)
    const enqueueMessage = vi.fn(async () => { throw new Error('enqueue rejected') })
    const cutInQueuedMessage = vi.fn()
    const removeQueuedMessage = vi.fn(async (sessionId: string, messageId: string) => queuedMessage(messageId, sessionId, 'cut-in fail'))
    const steerQueuedMessage = vi.fn()
    const host: PipiHostAPI = {
      ...base,
      sendPrompt,
    ...
    fireEvent.change(screen.getByLabelText('消息输入框'), { target: { value: 'cut-in fail' } })
    await waitFor(() => expect(screen.getByLabelText('发送消息')).toBeTruthy())
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(enqueueMessage).toHaveBeenCalledWith('welcome', 'cut-in fail', undefined, rendererTelemetryMatcher()))
    expect(cutInQueuedMessage).not.toHaveBeenCalled()
    expect(removeQueuedMessage).not.toHaveBeenCalled()
    expect(steerQueuedMessage).not.toHaveBeenCalled()
    expect([...document.querySelectorAll('[data-user-prompt]')].every(node => !node.textContent?.includes('cut-in fail'))).toBe(true)
    expect((screen.getByLabelText('消息输入框') as HTMLTextAreaElement).value).toBe('cut-in fail')
  })
  ...
  it('wires edit/remove/retry and immediate dispatch to their supported queue host APIs', async () => {
    ...
    await waitFor(() => expect(updateQueuedMessage).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        turnId: expect.stringMatching(/^turn-[a-f0-9]{24}$/),
...
function rendererTelemetryMatcher() {
  return expect.objectContaining({
    turnId: expect.stringMatching(/^turn-[a-f0-9]{24}$/),
    submittedAt: expect.any(Number),
    preflightStartedAt: expect.any(Number),
    preflightEndedAt: expect.any(Number),
    promptBytes: expect.any(Number),
...
describe('session switch transcript cache', () => {
  it('keeps only the selected transcript mounted while revalidating cached content on return', async () => {
    const base = createMockHost()
    let welcomeLoads = 0
    const host: PipiHostAPI = {
      ...base,
    ...
    fireEvent.click(container.querySelector('[data-session-id="layout"]')!)
    await screen.findByText('左栏宽度要能持久化。')
    expect(screen.queryByText('请实现 Electron 三栏主界面。')).toBeNull()
    ...
    expect(container.querySelectorAll('[data-session-transcript]')).toHaveLength(1)
    ...
    expect(screen.queryByText('左栏宽度要能持久化。')).toBeNull()
    expect(container.querySelectorAll('[data-session-transcript]')).toHaveLength(1)
    expect(welcomeLoads).toBe(2)
  })

  it('loads only the newest history page until Virtuoso reaches the top', async () => {
    const base = createMockHost()
    const latest = Array.from({ length: 500 }, (_, index) => ({
      id: `latest-${index}`,
      role: 'user' as const,
      content: `latest page ${index}`,
      timestamp: 1_000 + index,
    }))
    let releaseOlder: ((entries: HistoryEntry[]) => void) | undefined
    const getSessionHistory = vi.fn(async (_sessionId: string, before?: number | string) => {
      if (before !== undefined) return new Promise<HistoryEntry[]>(resolve => { releaseOlder = resolve })
      return latest
    })
    render(<App host={{ ...base, getSessionHistory }} />)
    await screen.findByText('latest page 499')
    await act(async () => { await Promise.resolve() })
    expect(getSessionHistory).toHaveBeenCalledTimes(1)

    act(() => virtuosoHarness.startReached?.(0))
    await waitFor(() => expect(getSessionHistory).toHaveBeenCalledTimes(2))
    await act(async () => { releaseOlder?.([{ id: 'older-1', role: 'user', content: 'older page', timestamp: 1 }]) })
  ...
  it('stops retrying an empty history page after one follow-up instead of looping', async () => {
    const base = createMockHost()
    const getSessionHistory = vi.fn(async () => [])
    render(<App host={{ ...base, getSessionHistory }} />)
    await waitFor(() => expect(getSessionHistory).toHaveBeenCalled())
    await waitFor(() => expect(getSessionHistory.mock.calls.length).toBeGreaterThanOrEqual(2), { timeout: 1_000 })
    const afterRetry = getSessionHistory.mock.calls.length
    await new Promise(resolve => setTimeout(resolve, 800))
    expect(getSessionHistory.mock.calls.length).toBe(afterRetry)
  })
...
describe('composer slash commands and model management', () => {
  ...
  it('sends a transformed formal-plan prompt for /plan with args', async () => {
    ...
    fireEvent.keyDown(composer, { key: 'Enter' })
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '请为以下目标制定正式计划并发布，等我批准后再执行：实现登录', undefined, rendererTelemetryMatcher()))
  ...
  it('sends the bare formal-plan prompt for /plan with no args', async () => {
    ...
    fireEvent.keyDown(composer, { key: 'Enter' })
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '请把当前目标整理成正式计划并发布，等我批准后再执行。', undefined, rendererTelemetryMatcher()))
    expect(composer.value).toBe('')
  })
  ...
  it('sends /goal status as the raw prompt without transforming it', async () => {
    ...
    fireEvent.keyDown(composer, { key: 'Enter' })
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '/goal status', undefined, rendererTelemetryMatcher()))
    expect(composer.value).toBe('')
  })
...
describe('demo mock host model persistence (browser reload)', () => {
  ...
  it('shows the target session model immediately while its authoritative refresh is still pending', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId('model-chip').textContent).toContain('Claude Sonnet 4'))
    // The sidebar belongs to the active form, which lands with the extension
    // list: wait for the row itself rather than for any one thing that precedes it.
    const layoutRow = await waitFor(() => {
...
describe('composer image attachments', () => {
  ...
  it('adds dropped image files as attachments and sends them with the prompt', async () => {
    const host = createMockHost()
    const sendPrompt = vi.spyOn(host, 'sendPrompt')
    const composer = await renderChat(host)
    const footer = document.querySelector('footer.composer') as HTMLElement
    const dataTransfer = fileDataTransfer(makeImageFile('dropped.png'))
    fireEvent.dragOver(footer, { dataTransfer })
    fireEvent.drop(footer, { dataTransfer })
    const thumb = await screen.findByTestId('composer-thumb-0')
    expect(thumb.querySelector('img')?.getAttribute('alt')).toBe('dropped.png')
    expect(screen.queryByTestId('composer-doc-chips')).toBeNull()
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '', [
      expect.objectContaining({ mimeType: 'image/png', name: 'dropped.png', dataBase64: expect.any(String) })
    ], rendererTelemetryMatcher()))
    expect(composer.value).toBe('')
  })

  it('routes a dropped non-image document through the document path, not attachments', async () => {
    window.pipiPathForFile = (file: File) => `/Users/haoli/Downloads/${file.name}`
    render(<App host={createMockHost()} />)
    await screen.findAllByText('Electron 三栏界面')
    const footer = document.querySelector('footer.composer') as HTMLElement
    const file = new File(['survey'], 'form.doc', { type: 'application/msword' })
  ...
  it('allows sending images without text', async () => {
    ...
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '', [expect.objectContaining({ name: 'only.png' })], rendererTelemetryMatcher()))
  })
  ...
  it('keeps pasted image attachments across session switches and restores them', async () => {
    ...
    fireEvent.click(screen.getByLabelText('发送消息'))
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '', [expect.objectContaining({ name: 'keep.png' })], rendererTelemetryMatcher()))
  })
...
describe('extensions settings tab', () => {
  it('opens the MCP / 扩展 tab and copies an add prompt for the main session', async () => {
    ...
    await openModelModal(createMockHost())
    expect(screen.queryByTestId('model-tab-profiles')).toBeNull()
    ...
    // Pack sub-nav indents into the settings sidebar under the 扩展 tab.
    const subnavSlot = screen.getByTestId('extensions-subnav-slot')
    expect(subnavSlot.closest('.model-modal-sidebar')).toBeTruthy()
    expect(subnavSlot.contains(screen.getByTestId('extensions-pack-dropdown'))).toBe(true)
    expect(screen.getByTestId('extensions-packages').contains(screen.getByTestId('extensions-pack-dropdown'))).toBe(false)
    fireEvent.click(screen.getByTestId('extensions-nav-mcp'))
  ...
  it('does not add a top-level Acme Build settings tab when the extension is enabled', async () => {
    const host = createMockHost()
    ...
    expect(screen.queryByTestId('model-tab-acme-build-oauth')).toBeNull()
    expect(screen.queryByRole('tab', { name: 'Acme Build' })).toBeNull()
    fireEvent.click(screen.getByTestId('model-tab-extensions'))
    expect(await screen.findByTestId('extensions-pkg-acme-build-oauth')).toBeTruthy()
  })

  it('does not register a Acme Build right-sidebar panel or rail icon', async () => {
    const host = createMockHost()
    // The rail's own panels belong to enabled extensions too, so keep the mock's
    // list and add Acme on top of it.
    const bundled = await host.listExtensions!()
    host.listExtensions = vi.fn(async () => [...bundled, {
      id: 'acme-build-oauth',
      name: 'Acme Build',
      version: '0.2.0',
      state: 'enabled' as const,
    ...
    } as Awaited<ReturnType<NonNullable<PipiHostAPI['listExtensions']>>>[number]])
    const { container } = render(<App host={host} />)
    await screen.findAllByText('Electron 三栏界面')
    await waitFor(() => expect(host.listExtensions).toHaveBeenCalled())
    const rail = container.querySelector('.tool-quick-rail')!
    // Plan joins the rail after the async plans fetch settles.
    await waitFor(() => expect(rail.querySelectorAll('button')).toHaveLength(5))
    expect(screen.queryByRole('button', { name: 'Acme Build' })).toBeNull()
    expect(screen.queryByRole('button', { name: 'acme-build-status' })).toBeNull()
    expect(screen.queryByRole('button', { name: /acme-build/i })).toBeNull()
    expect(screen.queryByText('扩展面板「Acme Build」加载失败')).toBeNull()
  })
})
...
describe('model catalog fallback', () => {
  it('falls back when the selected model disappears and does not abort an in-flight response', async () => {
    ...
    host.sendPrompt = vi.fn(async sessionId => {
      for (const listener of streamListeners) listener({ type: 'status', sessionId, status: 'started' })
    })

    await renderChat(host)
    fireEvent.click(await screen.findByTestId('model-chip'))
    fireEvent.click(await screen.findByTestId(`quick-row-${grok.provider}-${grok.id}`))
    await waitFor(() => expect(screen.getByTestId('model-chip').textContent).toContain(grok.name))

    fireEvent.change(screen.getByLabelText('消息输入框'), { target: { value: '继续生成' } })
    fireEvent.click(screen.getByLabelText('发送消息'))
...
describe('update center settings flow', () => {
  it('closes settings and sends exactly one update request through the selected main session', async () => {
    ...
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledTimes(1))
    expect(sendPrompt).toHaveBeenCalledWith('welcome', expect.stringMatching(/^\[\[PIPIUI_UPDATE_EVALUATION_INTENT\]\]\{"version":1,"id":"pi","name":"Pi","packageName":"@earendil-works\/pi-coding-agent","currentVersion":"0\.84\.0","latestVersion":"0\.84\.2"\}$/), undefined, rendererTelemetryMatcher())
...
describe('composer textarea sizing and hints', () => {
  ...
  it('keeps Enter-to-send and Shift+Enter newline behavior', async () => {
    ...
    fireEvent.keyDown(composer, { key: 'Enter' })
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', 'hello', undefined, rendererTelemetryMatcher()))
    await waitFor(() => expect(composer.value).toBe(''))
...
describe('composer thinking selector', () => {
  ...
  it('uses a hydrated GPT-5.6 Sol catalog map over provisional xhigh and falls back safely', async () => {
    const base = createMockHost()
    const restoredSession: Session = {
      id: 'sol-session',
      projectId: 'pipiui',
      name: 'GPT Sol thinking',
      updatedAt: Date.now(),
    ...
    const setThinkingLevel = vi.fn(async (_sessionId: string, level: ModelState['thinkingLevel']): Promise<ModelState> => {
      if (level === 'xhigh') throw new Error('thinking level xhigh is unavailable')
      return { model: catalogModel, thinkingLevel: level, availableThinkingLevels: ['off', 'minimal', 'max'] as ModelState['availableThinkingLevels'] }
    })
    let resolveCatalog!: (models: Model[]) => void
    const host: PipiHostAPI = {
      ...base,
    ...
    render(<App host={host} />)
    expect(await screen.findByLabelText('思考级别（当前：xhigh）')).toBeTruthy()

    await act(async () => { resolveCatalog([catalogModel]) })

    const chip = await screen.findByLabelText('思考级别（当前：off）') as HTMLButtonElement
    expect(chip.disabled).toBe(false)
    fireEvent.click(chip)
    expect(screen.getByTestId('thinking-row-off')).toBeTruthy()
    expect(screen.getByTestId('thinking-row-minimal')).toBeTruthy()
    expect(screen.getByTestId('thinking-row-max')).toBeTruthy()
    expect(screen.queryByTestId('thinking-row-xhigh')).toBeNull()
    expect(screen.queryByTestId('thinking-row-high')).toBeNull()
    fireEvent.click(screen.getByTestId('thinking-row-max'))
    await waitFor(() => expect(setThinkingLevel).toHaveBeenCalledWith('sol-session', 'max'))
    expect(screen.queryByText(/thinking level xhigh is unavailable/)).toBeNull()
  })
