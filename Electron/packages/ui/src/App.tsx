...
/**
 * The host's placeholder for a session it has not read yet (§85).
 *
 * `{provider: "unknown", id: "unknown", name: "Unknown"}` is the backend saying it does not know,
 * not a model anybody chose. Rendered as a name it becomes ✦Unknown in the composer — measured at
 * 12.5 seconds on screen per session in the remote browser, beside a thinking chip that said 「…」
 * for the very same read.
 */
export function isUnreadModel(model: Model | undefined | null): boolean {
...
function AppContent({ host: injectedHost }: { host?: PipiHostAPI }) {
  ...
  /** The session whose thinking level the host has actually reported. The
   *  provisional snapshot built from `listSessions` metadata carries no level,
   *  so until this matches, the chip says so instead of guessing (§63). */
  const [thinkingKnownFor, setThinkingKnownFor] = useState('')
  ...
  const selectedConversationPackId = sessions.find(item => item.id === selectedSession)?.productProfile?.id
  const conversationFormKnown = Boolean(selectedConversationPackId)
  ...
  const packSnapshotMismatch = projectFormKnown && conversationFormKnown && Boolean(selectedSession) && selectedConversationPackId !== activeProductPackId
  ...
  // A product pack's composer says what to type in its own words, by the session's phase (the setup
  // guide's answer or the Keeper's table); the shell's generic line stays for everything else.
  const composerWords = timeline?.ui?.words?.composer as Record<string, unknown> | undefined
  const composerKey = timeline?.phase === 'setup' ? 'placeholder_setup' : timeline?.phase === 'play' ? 'placeholder_play' : undefined
  const composerPlaceholder = composerKey && typeof composerWords?.[composerKey] === 'string' ? composerWords[composerKey] as string : undefined
  ...
  const applySelectedModelState = (state: ModelState) => {
    ...
    setThinkingKnownFor(selectedSession)
