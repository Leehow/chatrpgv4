...
describe('App message actions', () => {
  it('shows a full date and time before left-aligned assistant message actions', () => {
  ...
  it('copies and resends the user prose without the attachment footnote', async () => {
    ...
    fireEvent.click(within(user).getByRole('button', { name: '重发消息' }))
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '看图', undefined, expect.anything()))
  })
})
