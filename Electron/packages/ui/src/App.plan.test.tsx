...
import { App } from './App'
import { createMockHost, mockCapabilities } from './mock-host'

const CAPABILITIES = mockCapabilities({ revealInFinder: true, terminal: false, documents: true, browser: true, git: true, plan: true, retainedWorktreeDisposition: false })
...
describe('Plan approval bar in main chat', () => {
  ...
  it('sends 批准该计划 when 批准 is clicked', async () => {
    ...
    await waitFor(() => expect(sendPrompt).toHaveBeenCalledWith('welcome', '批准该计划', undefined, expect.anything()))
    expect(screen.queryByTestId('plan-approval-bar')).toBeNull()
  })

  it('keeps 批准 hidden after click even if the host object is replaced while the plan stays draft', async () => {
    const sendPrompt = vi.fn(async () => undefined)
    const host: PipiHostAPI = {
      ...createMockHost(),
      capabilities: async () => CAPABILITIES,
    ...
    const nextHost: PipiHostAPI = { ...host, getPlans: async () => [draftPlan()] }
    view.rerender(<App host={nextHost} />)
    await waitFor(() => expect(screen.queryByTestId('plan-approval-bar')).toBeNull())
    expect(screen.getByText('批准该计划')).toBeTruthy()
  })
