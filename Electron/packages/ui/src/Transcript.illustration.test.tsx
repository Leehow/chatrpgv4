...
it('hides the illustrate button on streaming, presentation, and user rows', () => {
  const onIllustrate = vi.fn()
  renderRow({ onIllustrate }, { ...narration, streaming: true })
  expect(screen.queryByRole('button', { name: '生成插画' })).toBeNull()
  cleanup()
  renderRow({ onIllustrate }, { id: 'card', role: 'assistant', content: '', presentation: { renderer: 'coc-mechanics', details: { marked_text: ' prose ' } } } as unknown as ChatMessage)
  expect(screen.queryByRole('button', { name: '生成插画' })).toBeNull()
  cleanup()
