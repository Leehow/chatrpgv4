...
function scrapeFields(name: string, text: string): string | undefined {
  ...
  switch (name) {
    ...
    case 'code_interpreter': {
      const code = scrapeJSONString('code', text)
      return code == null ? undefined : truncate(code.split('\n')[0] ?? code)
    }
    case 'fetch_content':
    case 'browser_fetch':
...
const skillNameSummary = (args: Record<string, unknown>) => stringField(args, 'name') ?? '…'
...
for (const contribution of [
  ...
  { toolName: 'x_search', summarizeArgs: querySummary, scrapeSummary: (text: string) => scrapeJSONString('query', text) },
  {
    toolName: 'code_interpreter',
    summarizeArgs: (args: Record<string, unknown>) => {
      const code = stringField(args, 'code')
      if (!code) return '…'
      const line = code.trim().split('\n').find(item => item.trim()) ?? code.trim()
      return truncate(line)
    },
    scrapeSummary: (text: string) => {
      const code = scrapeJSONString('code', text)
      return code == null ? undefined : truncate(code.split('\n')[0] ?? code)
    },
  },
  ...
  { toolName: 'skill_load', summarizeArgs: skillNameSummary, scrapeSummary: (text: string) => scrapeJSONString('name', text) },
  { toolName: 'skill_search', summarizeArgs: querySummary, scrapeSummary: (text: string) => scrapeJSONString('query', text) },
]) {
