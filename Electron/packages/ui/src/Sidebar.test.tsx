...
describe('Sidebar', () => {
  it('renders its pane toggle above search, pinned section, project rows and session rows', () => {
    ...
    expect(brand.textContent).toBe('PipiUI')
  ...
  it('moves a session onto another session with placement awareness', () => {
    ...
    expect(row('s2').getAttribute('data-drop-placement')).toBe('after')
    fireEvent.drop(row('s2'), { dataTransfer: transfer, clientY: 10 })
    expect(onMoveSession).toHaveBeenCalledTimes(1)
    expect(onMoveSession).toHaveBeenCalledWith('s1', 'p1', 's2', 'after')

    fireEvent.dragStart(row('s1'), { dataTransfer: transfer })
    fireEvent.dragOver(row('s3'), { dataTransfer: transfer, clientY: 10 })
    expect(row('s3').getAttribute('data-drop-placement')).toBe('after')
    fireEvent.drop(row('s3'), { dataTransfer: transfer, clientY: 10 })
    expect(onMoveSession).toHaveBeenCalledTimes(2)
    expect(onMoveSession).toHaveBeenLastCalledWith('s1', 'p2', 's3', 'after')
  })
  ...
  it('moves a session onto another project row and highlights it', () => {
    ...
    expect(p1.getAttribute('data-drop-placement')).toBe('after')
    fireEvent.dragOver(p2, { dataTransfer: transfer })
    expect(p2.getAttribute('data-drop-placement')).toBe('after')
    fireEvent.drop(p2, { dataTransfer: transfer })
    expect(onMoveSession).toHaveBeenCalledTimes(1)
    expect(onMoveSession).toHaveBeenCalledWith('s1', 'p2')
  })

  it('pins by dropping into the pinned section and moves a pinned session back onto a project row', () => {
    const onMoveSession = vi.fn()
    ...
    fireEvent.dragStart(row, { dataTransfer: transfer })
    expect(pinned.getAttribute('data-drop-active')).toBe('true')
    ...
    expect(onMoveSession).toHaveBeenCalledWith('pin1', 'p1')
  })

  it('reorders pinned sessions with drop placement', () => {
    const onMoveSessionToPinned = vi.fn()
    ...
    expect(pin2.getAttribute('data-drop-placement')).toBe('after')
  ...
  it('selects a pinned session without asking the parent to expand its collapsed project', () => {
    const props = defaultProps({
      expandedIds: [],
      pinnedSessions: [session({ id: 'pin1', projectId: 'p1', status: 'completed' })],
    })
    render(<Sidebar {...props} />)
    expect(screen.getAllByTestId('project-row')[0].getAttribute('aria-expanded')).toBe('false')
    fireEvent.click(screen.getByText('pin1').closest('.sb-session')!)
    expect(props.onSelectSession).toHaveBeenCalledTimes(1)
    expect(props.onSelectSession).toHaveBeenCalledWith('pin1', { expandProject: false })
    expect(props.onToggleProject).not.toHaveBeenCalled()
    expect(screen.getAllByTestId('project-row')[0].getAttribute('aria-expanded')).toBe('false')
  })
  ...
  it('renders the Swift-parity footer actions in order and reports every click', () => {
    ...
    render(<Sidebar {...defaultProps({ onOpenSettings, onOpenRemote, onOpenSubagentModels })} />)
    const buttons = within(screen.getByTestId('sidebar-footer')).getAllByRole('button')
    expect(buttons.map(button => button.getAttribute('aria-label'))).toEqual(['设置', '远程控制', 'Subagent 模型'])
  ...
  it('shows a manual theme toggle with mode-accurate labels in both states', () => {
    const onToggleTheme = vi.fn()
    const props = { ...defaultProps(), theme: 'light' as const, onToggleTheme }
    const view = render(<Sidebar {...props} />)
    let toggle = screen.getByTestId('sidebar-theme-toggle')
    expect(toggle.getAttribute('aria-label')).toBe('切换到深色模式')
    expect(toggle.getAttribute('title')).toBe('切换到深色模式')
    expect(toggle.querySelector('svg')).not.toBeNull()
    fireEvent.click(toggle)
    expect(onToggleTheme).toHaveBeenCalledTimes(1)
    // Dark mode flips the accessible name/title to the reverse action.
    view.rerender(<Sidebar {...props} theme="dark" />)
  ...
  it('hides the theme toggle when no handler is wired', () => {
  ...
  it('keeps .sb-scroll as the sole flexible vertical touch-scroll region of the drawer', () => {
    const css = readFileSync(join(import.meta.dirname, 'sidebar.css'), 'utf8')
    const scroll = css.match(/\.sb-scroll\{[^}]*\}/)?.[0] ?? ''
    // The list owns the leftover flex space and is the only region that scrolls.
    expect(scroll).toMatch(/flex:1 1 auto/)
    expect(scroll).toMatch(/min-height:0/)
    expect(scroll).toMatch(/overflow-x:hidden/)
    expect(scroll).toMatch(/overflow-y:auto/)
    // Mobile touch contract: vertical pan with momentum, contained so the
    // gesture never chains into the document behind the fixed drawer.
    expect(scroll).toMatch(/touch-action:pan-y/)
    expect(scroll).toMatch(/-webkit-overflow-scrolling:touch/)
    expect(scroll).toMatch(/overscroll-behavior:contain/)
    // Topbar and footer stay pinned around it (flex row/column items, no scroll).
    expect(css).toMatch(/\.sb-topbar\{[^}]*flex:0 0 50px/)
    expect(css).toMatch(/\.sb-footer\{[^}]*flex:0 0 auto/)
    // The drawer itself is a plain column — it must not scroll its own content.
    expect(css).toMatch(/\.sb-root\{[^}]*display:flex[^}]*flex-direction:column[^}]*}/s)
  })

  it('orders the drawer chrome so topbar/search sit above and footer below the scroll list', () => {
    const props = defaultProps({ onOpenSettings: vi.fn(), onOpenRemote: vi.fn() })
    render(<Sidebar {...props} />)
    const nav = screen.getByTestId('sidebar')
    const order = [...nav.children].map(el => el.className.split(' ')[0])
    expect(order.indexOf('sb-topbar')).toBe(0)
    expect(order).toContain('sb-search')
    const scrollIdx = order.indexOf('sb-scroll')
    expect(scrollIdx).toBeGreaterThan(order.indexOf('sb-search'))
    expect(order[order.length - 1]).toBe('sb-footer')
  })

  it('defers touch selection until pointer-up and absorbs the trailing click', () => {
    const props = defaultProps()
    render(<Sidebar {...props} />)
    const row = screen.getByText('s1').closest('.sb-session')!
    const preventDefault = vi.fn()
    const down = createEvent.pointerDown(row)
    ...
    const up = createEvent.pointerUp(row)
  ...
  it('keeps a vertical touch drag in the sidebar from selecting a session', () => {
    const props = defaultProps()
    render(<Sidebar {...props} />)
    const row = screen.getByText('s1').closest('.sb-session')!
    ...
    const move = createEvent.pointerMove(row)
...
describe('Sidebar custom logo', () => {
  it('renders img.sb-brand-logo from the stored localStorage logo instead of the wordmark', () => {
    localStorage.setItem('pipiui.logo', 'data:image/png;base64,abc123')
    render(<Sidebar {...defaultProps()} />)
    const logo = document.querySelector('img.sb-brand-logo') as HTMLImageElement | null
    expect(logo).toBeTruthy()
    expect(logo!.getAttribute('src')).toBe('data:image/png;base64,abc123')
    expect(logo!.getAttribute('alt')).toBe('自定义 Logo')
    expect(screen.queryByLabelText('PipiUI')).toBeNull()
  })
})
