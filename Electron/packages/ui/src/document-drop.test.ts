...
describe('document drop helpers', () => {
  ...
  it('includes readable text or a local parse hint in the opened-document injection', () => {
    // No document-parsing extension (pipiui_firecrawl_pdf, office) ships in this kernel-only
    // fork, so the hint is generic: which extension (if any) is enabled is project-specific.
    const genericHint = '这是二进制文档，无法直接用 read 读取；能否解析取决于当前项目启用的文档扩展，请优先使用其提供的解析工具，没有时如实说明。'
    expect(documentsOpenedInjection([
      ...
      `[文档面板] 用户打开了文档：/abs/form.doc。文件已在右侧面板打开（word，约 4KB）；${genericHint}打开预览本身不会解析。`,
      `[文档面板] 用户打开了文档：/abs/scan.pdf。文件已在右侧面板打开（pdf，约 2KB）；${genericHint}打开预览本身不会解析。`,
    ].join('\n\n'))
  })
