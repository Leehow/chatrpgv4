...
describe('PlanPanel', () => {
  ...
  it('labels an ordinary non-gated plan as directly executing instead of awaiting confirmation', async () => {
    const { host } = hostWith([plan({ lifecycle: 'active' })])
    render(<PlanPanel host={host} sessionId="s1" />)

    expect(await screen.findByText('直接执行')).toBeTruthy()
  ...
  it('renders blocking edges as ordinal references to the tasks they name', async () => {
    const { host } = hostWith([plan({ tasks: [
      { id: 'expand', title: '扩展', state: 'completed' },
      { id: 'migrate', title: '迁移', state: 'pending', blockedBy: ['expand'] },
      { id: 'contract', title: '收缩', state: 'pending', blockedBy: ['migrate', 'ghost'] },
    ] })])
    render(<PlanPanel host={host} sessionId="s1" />)
    await screen.findByTestId('plan-card')
    const deps = screen.getAllByTestId('plan-task-deps')
    expect(deps).toHaveLength(2)
    expect(deps[0].textContent).toBe('依赖 #1')
    expect(deps[1].textContent).toBe('依赖 #2、ghost')
  })
  ...
  it('lets a settled plan expand on click instead of snapping back closed', async () => {
    const settled = plan({
      id: 'plan-done',
      title: '已完成的计划',
      tasks: [{ id: 'a', title: '收尾', state: 'completed' }],
    })
    const { host } = hostWith([settled])
    render(<PlanPanel host={host} sessionId="s1" />)
    ...
    expect(card.textContent).toContain('收尾')
  })

  it('expands a settled plan from a touch pointerdown', async () => {
    const settled = plan({
      id: 'plan-done',
      title: '已完成的计划',
      tasks: [{ id: 'a', title: '收尾', state: 'completed' }],
    ...
    render(<PlanPanel host={host} sessionId="s1" />)
    const card = await screen.findByTestId('plan-card')
    fireEvent.pointerDown(card.querySelector('summary')!, { pointerType: 'touch' })
    expect(card.hasAttribute('open')).toBe(true)
    fireEvent.click(card.querySelector('summary')!)
    expect(card.hasAttribute('open')).toBe(true)
    expect(screen.getByTestId('plan-task').textContent).toContain('收尾')
  })
})
