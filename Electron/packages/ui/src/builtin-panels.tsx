/**
 * The shell's own tool panels.
 *
 * They are compiled into the renderer, so they register as `pipiui` — the shell — rather
 * than borrowing the id of the extension whose capability they front. Each names that
 * extension in `requires`, which is what makes it appear and disappear with it. Registering
 * under the extension's id achieved the same visibility by pretending to *be* that
 * extension, so a panel outlived nothing and belonged to nobody.
 */
...
import { DOCUMENT_PANEL_TAB, registerPanel } from './ui-registries'
...
registerPanel(BUILTIN_EXTENSION_ID, {
  requires: 'webview-browser-extension',
  id: 'Browser',
...
registerPanel(BUILTIN_EXTENSION_ID, {
  requires: 'document-workbench',
  id: DOCUMENT_PANEL_TAB,
  icon: { src: docTextIcon, ratio: 44 / 49 },
  lazy: true,
  render: ctx => (
    <div className="tool-page document-content" hidden={!ctx.active}>
      <DocumentTabs host={ctx.host} activePath={ctx.openedDocumentPath} paths={ctx.openedDocumentPaths ?? (ctx.openedDocumentPath ? [ctx.openedDocumentPath] : [])} onActivate={ctx.onActivateDocument} onClose={ctx.onCloseDocument} />
    </div>
  ),
})

registerPanel(BUILTIN_EXTENSION_ID, {
  requires: 'terminal-extension',
  id: 'Terminal',
