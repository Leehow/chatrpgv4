...
describe('update evaluation intent envelope', () => {
  it('encodes only the versioned minimal renderer-to-Pi fields', () => {
    ...
    const owned = encodePipiuiUpdateEvaluationIntent({
      id: 'doc-tools:doc-parser',
      name: 'Doc parser',
      packageName: '@example/doc-parser',
      ownerExtensionId: 'doc-tools',
      ownerExtensionName: 'Doc Tools',
      currentVersion: '1.15.0',
      latestVersion: '1.17.0',
    })
    expect(JSON.parse(owned.slice(PIPIUI_UPDATE_EVALUATION_INTENT_PREFIX.length))).toEqual({
      version: PIPIUI_UPDATE_EVALUATION_INTENT_VERSION,
      id: 'doc-tools:doc-parser',
      name: 'Doc parser',
      packageName: '@example/doc-parser',
...
describe('thinking capability tri-state helper', () => {
  ...
  it('lets a hydrated catalog map override a stale provisional list', () => {
    const sol = {
      ...unknown,
      provider: 'openai-codex',
      id: 'gpt-5.6-sol',
      name: 'GPT-5.6 Sol',
      reasoning: true,
      thinkingLevelMap: { minimal: 'low', max: 'max' } as const,
    }
    const stale = ['off', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max'] as const
    expect(thinkingLevelsForHydratedModel(sol, stale)).toEqual(['off', 'minimal', 'max'])
    expect(thinkingLevelsForHydratedModel(sol, stale)).not.toContain('xhigh')
    expect(resolveThinkingLevel('xhigh', thinkingLevelsForHydratedModel(sol, stale))).toBe('off')
  })

  it('keeps reported levels when the hydrated catalog is silent', () => {
    const grok = { ...unknown, provider: 'xai', id: 'grok-4.6', name: 'Grok 4.6', reasoning: true, thinkingConfigurable: true }
    expect(thinkingLevelsForHydratedModel(grok, ['minimal', 'low', 'medium', 'high', 'xhigh'])).toEqual([
...
function createContractMockBackend(): HostBackend {
  ...
  const agent: any = { agentId: 'agent-1', runId: 'run-1', sessionId: 'session-1', name: 'builder', role: 'general-purpose', title: 'Mock build', task: 'Mock task', state: 'running', depth: 1, createdAt: 1, cost: 0, turns: 1, outputCount: 0 }
  const agentLogs: any[] = []
  let worktree: any = { agentId: agent.agentId, sessionId: agent.sessionId, runId: agent.runId, branch: 'pipiui/mock', path: '/tmp/pipiui-mock', lifecycle: 'pendingReview', merge: 'ready', discard: 'ready' }
  ...
  // Exact control contract: every agent/worktree control call must match the
  // exact {sessionId, agentId, runId} or the host rejects it. There is no
  // agentId-only resolution — that was the cross-session misrouting this
  // contract removed.
  const resolveEpisode = (params: readonly unknown[]): void => {
    const [sessionId, agentId, runId] = params as [string, string, string]
    if (sessionId !== agent.sessionId || agentId !== agent.agentId || runId !== agent.runId) throw new Error(`unknown episode ${sessionId}/${agentId}/${runId}`)
  }
      ...
      switch (method) {
        ...
        case 'getQuotaSnapshot': return state.model.provider === 'mock'
          ...
          : state.model.provider === 'deepseek'
            ...
            : null
        case 'listAgentDefinitions': return [
        ...
        case 'abortAgent': resolveEpisode(params); agent.state = 'aborted'; emit('agents', { type: 'agent', agent: { ...agent } }); return
        case 'resolveAgent': resolveEpisode(params); agent.state = 'ok'; emit('agents', { type: 'agent', agent: { ...agent } }); return
        case 'checkAgent': resolveEpisode(params); return agent
        case 'getWorktreeStatus': resolveEpisode(params); return worktree
        case 'mergeWorktree': resolveEpisode(params); worktree = { ...worktree, lifecycle: 'merged', merge: 'merged', discard: 'unavailable' }; emit('agents', { type: 'worktree', status: worktree }); return worktree
        case 'discardWorktree': resolveEpisode(params); worktree = { ...worktree, lifecycle: 'discarded', merge: 'unavailable', discard: 'discarded' }; emit('agents', { type: 'worktree', status: worktree }); return worktree
        case 'terminalOpen': {
        ...
        case 'capabilities': return { revealInFinder: true, terminal: true }
...
function contract(name: string, factory: Factory, expectedCapabilities: Record<string, boolean> = { revealInFinder: true, terminal: true }): void {
  describe(name, () => {
    let close = async () => {}
    ...
    it('exposes equivalent projects, sessions, history, models, capabilities and control plane', async () => {
      ...
      expect((await host.checkAgent('session-1', 'agent-1', 'run-1')).state).toBe('running')
      expect(await host.getWorktreeStatus('session-1', 'agent-1', 'run-1')).toMatchObject({ lifecycle: 'pendingReview', merge: 'ready', discard: 'ready' })
      expect(await host.mergeWorktree('session-1', 'agent-1', 'run-1')).toMatchObject({ lifecycle: 'merged', merge: 'merged' })
      await host.abortAgent('session-1', 'agent-1', 'run-1')
      expect((await host.checkAgent('session-1', 'agent-1', 'run-1')).state).toBe('aborted')
      await host.resolveAgent('session-1', 'agent-1', 'run-1')
      expect((await host.checkAgent('session-1', 'agent-1', 'run-1')).state).toBe('ok')
      expect(await host.discardWorktree('session-1', 'agent-1', 'run-1')).toMatchObject({ lifecycle: 'discarded', discard: 'discarded' })
      // Exact episode identity: another session's same-slug worker and a stale run
      // are both rejected instead of routing to the globally newest row.
      await expect(host.checkAgent('session-2', 'agent-1', 'run-1')).rejects.toThrow(/unknown episode session-2\/agent-1\/run-1/)
      await expect(host.abortAgent('session-1', 'agent-1', 'run-stale')).rejects.toThrow(/unknown episode session-1\/agent-1\/run-stale/)
      await expect(host.resolveAgent('session-1', 'agent-1', 'run-stale')).rejects.toThrow(/unknown episode/)
      await expect(host.mergeWorktree('session-2', 'agent-1', 'run-1')).rejects.toThrow(/unknown episode/)
...
contract('WebSocket transport', wsFactory, { revealInFinder: false, terminal: true, browser: false })

describe('agent control call-shape', () => {
  it('always sends the exact composite wire form {sessionId, agentId, runId}', async () => {
    const requests: { method: string; params: unknown[] }[] = []
    const ipc: IpcRendererLike = {
      invoke: async (_channel, request: any) => {
        requests.push({ method: request.method, params: request.params })
        return { protocolVersion: request.protocolVersion, id: request.id, type: 'response', ok: true, result: null }
      },
      on: () => {},
      removeListener: () => {}
    }
    const host = createIpcHost(ipc)
    // Composite shape (exact episode identity): {sessionId, agentId, runId}. There
    // is deliberately no agentId-only wire form: a shorter call would resolve to an
    // undefined episode at runtime — the misbinding this contract removes.
    await host.abortAgent('session-1', 'agent-1', 'run-1')
    await host.resolveAgent('session-1', 'agent-1', 'run-1')
    await host.checkAgent('session-1', 'agent-1', 'run-1')
    await host.getWorktreeStatus('session-1', 'agent-1', 'run-1')
    await host.mergeWorktree('session-1', 'agent-1', 'run-1')
    await host.discardWorktree('session-1', 'agent-1', 'run-1')
    expect(requests.map(({ method, params }) => [method, params])).toEqual([
      ['abortAgent', ['session-1', 'agent-1', 'run-1']],
      ['resolveAgent', ['session-1', 'agent-1', 'run-1']],
      ['checkAgent', ['session-1', 'agent-1', 'run-1']],
      ['getWorktreeStatus', ['session-1', 'agent-1', 'run-1']],
      ['mergeWorktree', ['session-1', 'agent-1', 'run-1']],
      ['discardWorktree', ['session-1', 'agent-1', 'run-1']]
    ])
  })
})
...
describe('Host API v2 transport contract', () => {
  ...
  it('projects WebSocket event payloads to the selected session before serialization', async () => {
    const [client, server] = pairedSockets()
    const [otherClient, otherServer] = pairedSockets()
    const backend = transportBackend()
    bindHostBackend(backend, server)
    bindHostBackend(backend, otherServer)
    const host = createWsHost(client)
    const otherHost = createWsHost(otherClient)

    expect(host.setEventProjectionSession).toBeTypeOf('function')
    await host.setEventProjectionSession?.('selected')
    await otherHost.setEventProjectionSession?.('other')
    const sentBeforeEvents = server.sent.length
    const otherSentBeforeEvents = otherServer.sent.length

    backend.emit({ protocolVersion: 2, channel: 'stream', event: { type: 'text', sessionId: 'selected', contentIndex: 0, delta: 'selected text' } })
    backend.emit({ protocolVersion: 2, channel: 'stream', event: { type: 'text', sessionId: 'other', contentIndex: 0, delta: 'other text' } })
    backend.emit({ protocolVersion: 2, channel: 'stream', event: { type: 'text', sessionId: 'background', contentIndex: 0, delta: 'background text' } })
    backend.emit({ protocolVersion: 2, channel: 'stream', event: { type: 'status', sessionId: 'background', status: 'started' } })
    backend.emit({ protocolVersion: 2, channel: 'agents', event: { type: 'agent_log', sessionId: 'background', agentId: 'background-worker', runId: 'run', itemType: 'text', text: 'background log' } })
    backend.emit({ protocolVersion: 2, channel: 'agents', event: { type: 'agent_log', sessionId: 'selected', agentId: 'selected-worker', runId: 'run', itemType: 'text', text: 'selected log' } })
    backend.emit({ protocolVersion: 2, channel: 'agents', event: { type: 'agent', agent: { agentId: 'background-worker', runId: 'run', sessionId: 'background', name: 'worker', task: '', state: 'running' } } })

    const projected = server.sent.slice(sentBeforeEvents).map(frame => JSON.parse(frame) as HostWireFrame)
    expect(projected).toEqual([
      { protocolVersion: 2, type: 'event', channel: 'stream', event: { type: 'text', sessionId: 'selected', contentIndex: 0, delta: 'selected text' } },
      { protocolVersion: 2, type: 'event', channel: 'stream', event: { type: 'status', sessionId: 'background', status: 'started' } },
      { protocolVersion: 2, type: 'event', channel: 'agents', event: { type: 'agent_log', sessionId: 'selected', agentId: 'selected-worker', runId: 'run', itemType: 'text', text: 'selected log' } },
      { protocolVersion: 2, type: 'event', channel: 'agents', event: { type: 'agent', agent: { agentId: 'background-worker', runId: 'run', sessionId: 'background', name: 'worker', task: '', state: 'running' } } },
    ])
    expect(otherServer.sent.slice(otherSentBeforeEvents).map(frame => JSON.parse(frame) as HostWireFrame)).toEqual([
      { protocolVersion: 2, type: 'event', channel: 'stream', event: { type: 'text', sessionId: 'other', contentIndex: 0, delta: 'other text' } },
      { protocolVersion: 2, type: 'event', channel: 'stream', event: { type: 'status', sessionId: 'background', status: 'started' } },
      { protocolVersion: 2, type: 'event', channel: 'agents', event: { type: 'agent', agent: { agentId: 'background-worker', runId: 'run', sessionId: 'background', name: 'worker', task: '', state: 'running' } } },
    ])

    await host.setEventProjectionSession?.(undefined)
    const sentBeforeReset = server.sent.length
    backend.emit({ protocolVersion: 2, channel: 'stream', event: { type: 'text', sessionId: 'background', contentIndex: 0, delta: 'restored text' } })
    expect(server.sent.slice(sentBeforeReset).map(frame => JSON.parse(frame))).toEqual([
      { protocolVersion: 2, type: 'event', channel: 'stream', event: { type: 'text', sessionId: 'background', contentIndex: 0, delta: 'restored text' } },
    ])
  })
  ...
  it('rejects a hanging WebSocket RPC after requestTimeoutMs and forgets the pending id', async () => {
    const [client, server] = pairedSockets()
    const host = createWsHost(client, { requestTimeoutMs: 40 })
    const hung = host.listProjects()
    await expect(hung).rejects.toMatchObject({ message: 'transport request timed out', code: TRANSPORT_TIMEOUT })
    expect(client.sent).toHaveLength(1)
    const stale = JSON.parse(client.sent[0]) as { id: string }
    server.send(JSON.stringify({ protocolVersion: 2, id: stale.id, type: 'response', ok: true, result: [{ id: 'stale' }] }))
    bindHostBackend(transportBackend(), server)
    await expect(host.listProjects()).resolves.toEqual([{ id: 'p1', name: 'P', path: '/tmp/p' }])
  })

  it('does not time out a WebSocket RPC that responds in time', async () => {
    const [client, server] = pairedSockets()
    bindHostBackend(transportBackend(async () => {
      await new Promise(resolve => setTimeout(resolve, 15))
      return [{ id: 'p1', name: 'P', path: '/tmp/p' }]
    }), server)
    const host = createWsHost(client, { requestTimeoutMs: 80 })
    await expect(host.listProjects()).resolves.toEqual([{ id: 'p1', name: 'P', path: '/tmp/p' }])
  })

  it('close still wins over a pending timeout and does not double-reject', async () => {
    const [client, server] = pairedSockets()
    bindHostBackend(transportBackend(async () => new Promise(() => {})), server)
    const host = createWsHost(client, { requestTimeoutMs: 50 })
    const pending = host.listProjects()
    client.close()
    await expect(pending).rejects.toMatchObject({ code: TRANSPORT_DISCONNECTED })
...
describe('Electron-native Product Pack ZIP extension', () => {
  it('keeps local archive paths off remote/default hosts and routes opted-in IPC calls', async () => {
    const calls: Array<{ method: string; params: unknown[] }> = []
    const ipc: IpcRendererLike = {
      invoke: async (_channel, request) => {
        calls.push({ method: request.method, params: request.params })
        return { protocolVersion: 2, id: request.id, type: 'response', ok: true, result: '/Users/demo/coding.pipiui-pack.zip' }
      },
      on: () => undefined,
      removeListener: () => undefined
    }
    ...
    const defaultHost = createIpcHost(ipc)
    expect(defaultHost.pickProductPackArchive).toBeUndefined()
    expect(defaultHost.saveProductPackArchive).toBeUndefined()
    expect(defaultHost.exportProductPackArchive).toBeUndefined()
    expect(defaultHost.installProductPackArchive).toBeUndefined()
    expect(defaultHost.installExtensionZip).toBeUndefined()

    const host = createIpcHost(ipc, undefined, { productPackFileDialogs: true })
    await expect(host.saveProductPackArchive?.('coding.pipiui-pack.zip')).resolves.toBe('/Users/demo/coding.pipiui-pack.zip')
    await expect(host.pickProductPackArchive?.()).resolves.toBe('/Users/demo/coding.pipiui-pack.zip')
    await expect(host.exportProductPackArchive?.('pipiui', 'coding', '/Users/demo/coding.pipiui-pack.zip')).resolves.toBe('/Users/demo/coding.pipiui-pack.zip')
    await expect(host.installExtensionZip?.('pipiui', '/Users/demo/tool.zip')).resolves.toBe('/Users/demo/coding.pipiui-pack.zip')
    expect(calls).toEqual([
      { method: 'saveProductPackArchive', params: ['coding.pipiui-pack.zip'] },
      { method: 'pickProductPackArchive', params: [] },
      { method: 'exportProductPackArchive', params: ['pipiui', 'coding', '/Users/demo/coding.pipiui-pack.zip'] },
      { method: 'installExtensionZip', params: ['pipiui', '/Users/demo/tool.zip'] },
    ])
  })
})
  ...
  it('passthrough-invokes optional extension methods over pipi-host:v1', async () => {
    ...
    const ipc: IpcRendererLike = {
      invoke: async (_channel, request) => {
        ...
        if (request.method === 'getExtensionUiEntrySource') {
          return { protocolVersion: 2, id: request.id, type: 'response', ok: true, result: 'export const HeaderAction = () => null' }
        }
        if (request.method === 'getExtensionAuthStatus') {
          return { protocolVersion: 2, id: request.id, type: 'response', ok: true, result: { extensionId: 'quota', providerId: 'acme-chat', loggedIn: false, usable: false } }
        }
        if (request.method === 'beginExtensionLogin') {
          return { protocolVersion: 2, id: request.id, type: 'response', ok: true, result: { loginId: 'login-1' } }
        }
        if (request.method === 'logoutExtension') {
          return { protocolVersion: 2, id: request.id, type: 'response', ok: true, result: { model: { provider: 'unknown', id: 'unknown', name: '无可用模型' }, thinkingLevel: 'off', availableThinkingLevels: [] } }
        }
        return { protocolVersion: 2, id: request.id, type: 'response', ok: true, result: undefined }
      },
      on: () => undefined,
    ...
    await expect(host.listExtensions?.('project-1')).resolves.toEqual([descriptor])
    await expect(host.setExtensionEnabled?.('quota', true, 'project')).resolves.toEqual({ ...descriptor, state: 'enabled' })
    await expect(host.setExtensionEnabled?.('quota', true, 'project', 'project-1')).resolves.toEqual({ ...descriptor, state: 'enabled' })
    ...
    await expect(host.getExtensionUiEntrySource?.('quota', 'app/dist/header.js')).resolves.toContain('HeaderAction')
    await expect(host.getExtensionUiEntrySource?.('quota', 'app/dist/header.js', 'project-1')).resolves.toContain('HeaderAction')
    await expect(host.getExtensionAuthStatus?.('quota')).resolves.toEqual({ extensionId: 'quota', providerId: 'acme-chat', loggedIn: false, usable: false })
    await expect(host.beginExtensionLogin?.('quota', 'oauth')).resolves.toEqual({ loginId: 'login-1' })
    await expect(host.logoutExtension?.('quota')).resolves.toMatchObject({ model: { provider: 'unknown' } })
    ...
    expect(calls).toEqual([
      ...
      { method: 'listExtensions', params: ['project-1'] },
      { method: 'setExtensionEnabled', params: ['quota', true, 'project'] },
      { method: 'setExtensionEnabled', params: ['quota', true, 'project', 'project-1'] },
      ...
      { method: 'getExtensionUiEntrySource', params: ['quota', 'app/dist/header.js'] },
      { method: 'getExtensionUiEntrySource', params: ['quota', 'app/dist/header.js', 'project-1'] },
      { method: 'getExtensionAuthStatus', params: ['quota'] },
      { method: 'beginExtensionLogin', params: ['quota', 'oauth'] },
      { method: 'logoutExtension', params: ['quota'] },
    ])
  })
})
