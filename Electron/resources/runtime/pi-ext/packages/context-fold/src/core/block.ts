...
export interface DigestBlock {
	...
	kind: BlockKind;
	text: string;
	/** Exact payload persisted for recall when it differs from the display/index text. */
	spoolText?: string;
	tokens: number;
	toolName?: string;
	isError?: boolean;
...
export function linearize(messages: AgentMessage[]): WireBlock[] {
	...
	const push = (
		...
		kind: WireBlock["kind"],
		text: string,
		extra: Partial<Pick<WireBlock, "spoolText" | "toolName" | "toolArgs" | "callId" | "model" | "isError" | "opaque">> = {},
	) => {
				...
				parts.forEach((b, j) => {
					if (b?.type === "thinking") push(blockId(m, i, j), "thinking", (b as ThinkingPart).thinking || "", { model: m.model });
						...
						push(blockId(m, i, j), "tool_call", `${c.name} ${JSON.stringify(c.arguments ?? {})}`, {
							spoolText: JSON.stringify(c.arguments ?? {}),
							toolName: c.name,
