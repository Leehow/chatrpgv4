...
export function exposePipiHost(contextBridge: ContextBridgeLike, ipc: IpcRendererLike): PipiHostAPI {
  const host = createIpcHost(ipc, PIPI_HOST_IPC_CHANNEL, { openExternal: true, openDocumentExternally: true, projectDirectoryPicker: true, productPackFileDialogs: true, updateCenter: true })
...
export function exposePipiRemoteControl(
  ...
  const remote: PipiRemoteControlAPI = {
    ...
    checkLink: () => ipc.invoke(PIPI_REMOTE_CONTROL_IPC_CHANNEL, { type: 'checkLink' }),
    subscribe(listener) {
