...
/** Documented idle_ms default (browser help: "default 400ms"). */
export const BROWSER_WAIT_DEFAULT_IDLE_MS = 400
/**
 * Headroom subtracted from the wait budget when capping the idle quiet window: the
 * poll loop needs at least one check after the (capped) quiet window elapses and
 * before the timeout deadline, otherwise a short wait still cannot report idle.
 */
export const BROWSER_WAIT_IDLE_BUDGET_SLACK_MS = 60
...
export const PAGE_SAFE_SERIALIZE_SOURCE = `function __pipiSafeSerialize(value, budget) {
  ...
  try {
    ...
    void max;
    return { ok: true, json: text, truncated: false };
  } catch (error) {
...
export function buildScriptWrapper(js: string, requestId: string, deadlineMs = BROWSER_SCRIPT_DEADLINE_MS): { source: string; sourceURL: string; headerLines: number } {
  ...
  function step(label) {
    ...
    __steps.push({ label: String(label == null ? "" : label), t: Date.now() });
  }
  function el(token) {
