...
function ensurePiBackendRuntimeAgentsBundle(): void {
  const script = resolve(workspaceRoot, 'packages/pi-backend/scripts/bundle-runtime-agents.mjs')
  const bundle = resolve(workspaceRoot, 'packages/pi-backend/src/runtime-agents.bundle.mjs')
  const source = resolve(workspaceRoot, 'packs/agent-orchestration/subagent/agents.ts')
  const stale =
    !existsSync(bundle) ||
    (existsSync(source) && existsSync(bundle) && statSync(source).mtimeMs > statSync(bundle).mtimeMs)
  if (!stale) return
  const result = spawnSync(process.execPath, [script], { stdio: 'inherit' })
  if (result.status !== 0) {
    throw new Error('Failed to bundle pi-backend runtime agents core')
  }
}
ensurePiBackendRuntimeAgentsBundle()
...
const hostApiBrowserSourceEntry = resolve(workspaceRoot, 'packages/host-api/src/browser.ts')
...
// repeatedly restarts Electron, interrupting live sessions.
export const generatedWatchExcludes = [
  '**/dist/**',
  '**/out/**',
  '**/build/**',
  '**/*.tsbuildinfo',
  '**/runtime-agents.bundle.mjs'
]
...
const hostSourceAliases = [
  // Subpath before the bare package: alias matching is prefix-based.
  { find: '@pipi/host-api/browser', replacement: hostApiBrowserSourceEntry },
