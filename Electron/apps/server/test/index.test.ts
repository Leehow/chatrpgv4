...
function quietBackend(): HostBackend {
  return {
    async handle(method) {
      if (method === "capabilities") return { revealInFinder: true, terminal: true, browser: true };
...
`);
  ...
  it("runs the real Pi backend over the Host API WebSocket and scopes events to its connection", async () => {
    ...
    expect(await host.capabilities()).toMatchObject({ revealInFinder: false, terminal: false, browser: false });
