...
// Gated method sets come from the host-api contract itself so a new terminal or
// browser method can never slip past the server's capability check.
const TERMINAL_METHODS = new Set<HostMethod>(TERMINAL_HOST_METHODS);
const BROWSER_METHODS = new Set<HostMethod>(BROWSER_HOST_METHODS);
...
export function createWsHostServer(
  ...
  app.get("/pair/:pairID", (req, res) => {
    ...
    const granted = pairing.authorize(req.headers.cookie);
    if (granted && granted.pairID === pairID) {
    ...
    res.type("html").send(pairPage(pairID, nonce));
  });
...
export function serverOptionsFromEnvironment(env = process.env): WsHostServerOptions {
  ...
  return {
    ...
    backendOptions: {
      ...
      sessionsRoot: env.PI_COC_UI_ROOT ? resolve(env.PI_COC_UI_ROOT, 'sessions') : resolve(dirname(fileURLToPath(import.meta.url)), '../../../../.pi/coc-agent/ui-sessions', env.PI_COC_MODE === 'setup' ? 'setup' : 'play'),
      managedNodeModulesRoot: resolve(dirname(fileURLToPath(import.meta.url)), '../../../../node_modules'),
      runtimeRoot: env.PI_COC_UI_ROOT ? resolve(env.PI_COC_UI_ROOT, 'runtime') : resolve(dirname(fileURLToPath(import.meta.url)), '../../../../.pi/coc-agent/ui-runtime'),
