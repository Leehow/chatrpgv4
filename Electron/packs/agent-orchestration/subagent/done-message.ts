...
/** Files listed on the Scope-drift done line before the tail collapses to a +N count. */
export const SCOPE_DRIFT_LIST_CAP = 20;
...
export interface DoneMessageResult {
	...
	/** Attested completion measurement: worker-branch files outside the declared scope (empty = none). Absent when no scope was declared or the branch diff was not measurable. */
	scopeDrift?: string[];
...
export function formatSubagentDoneMessage(
	...
	// Attested scope drift, same nature as the verify exit code: machine-measured, not worker-claimed.
	// No line at all when the dispatch declared no scope or the branch diff was not measurable —
	// an unmeasured run must never read as "none".
	if (result.scopeDrift) {
		const total = result.scopeDrift.length;
		if (total === 0) {
			lines.push("Scope-drift: none");
		} else {
			const listed = result.scopeDrift.slice(0, SCOPE_DRIFT_LIST_CAP);
			const more = total > listed.length ? `, …+${total - listed.length} more` : "";
			lines.push(`Scope-drift: ${total} file(s) outside declared scope: ${listed.join(", ")}${more}`);
		}
	}
	lines.push(
		"Result:",
	...
	lines.push(
		"Handling: The Wave line above is the runtime snapshot of still-running workers taken at this completion; use it to decide whether the whole related goal is terminal yet. this is a worker event, not a new user request. One unfiltered subagent_status() without agentId lists running jobs plus the most recent ended jobs; it does not dump the full archive. If this turn already has that snapshot, reuse it and do not call again; if this turn has no unfiltered subagent_status() yet, call it once without agentId and inspect every worker relevant to this user's goal, including this one. If any related worker is running (including stalled) or expected related work is still unfinished, do not give a final closeout or present the goal as complete: continue orchestration/internal ledger work or dispatch follow-up work, then wait for the next event. Total silence is not required while you wait — when the user directly asks about progress or what is going on, when an important diagnostic or route decision must be shared, or when you need to explain what you are waiting for, give a brief factual update grounded in the status snapshot; never fabricate results, never emit one update per done event, and never send an empty assistant message to comply with this wait. ONLY after status confirms every related worker is terminal may you give the user exactly one complete final closeout in their language — verdict, key evidence, and what changed. Do not end silently once that final-closeout condition is met. Never reply \"already completed\" without a status snapshot this turn; if a related worker is still running but its work is done, close it with subagent_abort({agentId, runId}) (or /subagent_abort <agentId> <runId>) so its messages stop, or resolve terminal failed/aborted/interrupted episodes with subagent_resolve({agentId, runId}) (or /subagent_resolve <agentId> <runId>).",
	);
	return lines.join("\n");
}
