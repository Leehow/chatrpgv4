...
async function runLeftoverWorktreeSweep(store: WorktreeRecoveryStoreV1): Promise<void> {
	...
	try {
		...
		if (!provider) throw new Error("git-capability worktree provider is unavailable; leftover sweep skipped");
		await provider.createFinalizationService({
			logger: {
				info(event, fields) {
...
/** A dirty main changes the verify-failure guidance, so probe it via the Git provider. */
export function probeMainDirty(mainCwd: string): boolean {
	try {
		return requestGitWorktreeServiceProviderV1()?.probeMainDirty(mainCwd) ?? true;
	} catch {
		return true;
	}
}
