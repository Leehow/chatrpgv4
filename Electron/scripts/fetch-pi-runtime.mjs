...
const brokerPackage = JSON.parse(await readFile(join(electronRoot, 'packs', 'memory-extension', 'memory-broker', 'package.json'), 'utf8'))
const brokerLock = JSON.parse(await readFile(join(electronRoot, 'packs', 'memory-extension', 'memory-broker', 'package-lock.json'), 'utf8'))
...
// host: the already-running Electron binary. Do not exec Helper.app — each
// Helper launch is a new sealed-code execution and Gatekeeper re-verifies the
// whole bundle. Fallbacks only cover invocations the app did not launch.
const NODE_SHIM_POSIX = `#!/bin/sh
# Generated by scripts/fetch-pi-runtime.mjs — PipiUI ships no standalone Node.
# Never exec * Helper.app here: syspolicyd re-scans the sealed bundle each time.
if [ -n "\${PIPIUI_ELECTRON_BINARY:-}" ] && [ -x "\${PIPIUI_ELECTRON_BINARY}" ]; then
  case "\${PIPIUI_ELECTRON_BINARY}" in
...
async function inspectRuntime(root, expected) {
  ...
  try {
    const head = (await readFile(node)).subarray(0, 16)
    const text = head.toString('utf8')
    const isShim = text.startsWith('#!') || text.startsWith('@echo')
    if (expected.platform === 'darwin' && isShim) {
      return { ok: false, reason: 'embedded Node is the Electron shim; darwin needs a standalone Node so Gatekeeper does not re-scan PipiUI.app' }
    }
    if (expected.platform !== 'darwin' && !isShim) {
...
async function buildRuntime({ asset, destination, key, platform, arch, runtimesRoot }) {
  ...
  try {
    ...
    if (platform === 'darwin') {
      // Official Node is a self-contained Mach-O. Execing Electron/.app binaries
      // (main or Helper) makes syspolicyd re-verify the rejected PipiUI Dev bundle.
      await copyFile(extractedNode, stagedNode)
      await chmod(stagedNode, 0o755)
      const saved = await stripEmbeddedNode(stagedNode, platform, arch)
      console.log(`Embedded standalone Node ${metadata.version}${saved ? ` (stripped ${saved} bytes)` : ''}`)
    } else {
...
async function main() {
  ...
  // Same target as extraResources `${env.PIPIUI_EMBEDDED_RUNTIME_TARGET}`.
  for (const [name, version] of Object.entries(requiredPackages)) {
