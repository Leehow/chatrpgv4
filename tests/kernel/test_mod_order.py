...
def test_later_named_rule_changes_execution_and_earlier_policy_is_suppressed(kernel,tmp_path):
    ...
    later=["enhanced-items","natural-npc","npc-overhaul"]
    ...
    earlier=["enhanced-items","npc-overhaul","natural-npc"]
...
def test_materializer_and_editor_follow_order_and_stale_job_is_rejected(kernel,tmp_path):
    ...
    kernel.ok("mods.order",{"campaign":CAMPAIGN,"order":["enhanced-items","natural-npc","item-overhaul"]})
    context=kernel.ok("mods.context",{"campaign":CAMPAIGN})
    ...
    kernel.ok("mods.order",{"campaign":CAMPAIGN,"order":["item-overhaul","natural-npc","enhanced-items"]})
...
def test_dependencies_and_default_order_are_preserved(kernel,tmp_path):
    ...
    assert kernel.err("mods.order",{"campaign":CAMPAIGN,"order":["npc-overhaul","natural-npc","enhanced-items"]})["code"]=="invalid_params"
    assert read_json(campaign_dir(kernel.workspace)/"world.json")==prior
    order=["natural-npc","enhanced-items","npc-overhaul"]
...
def test_an_observer_can_explicitly_replace_an_existing_audit_slot(kernel,tmp_path):
    ...
    kernel.ok("mods.order",{"campaign":CAMPAIGN,"order":["enhanced-items","natural-npc","audit-overhaul"]})
