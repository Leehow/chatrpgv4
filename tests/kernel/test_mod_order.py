...
def test_later_named_rule_changes_execution_and_earlier_policy_is_suppressed(kernel,tmp_path):
    ...
    later=["guided-creation","enhanced-items","natural-npc","npc-overhaul"]
    ...
    earlier=["guided-creation","enhanced-items","npc-overhaul","natural-npc"]
...
def test_materializer_and_editor_follow_order_and_stale_job_is_rejected(kernel,tmp_path):
    ...
    kernel.ok("mods.order",{"campaign":CAMPAIGN,"order":["guided-creation","enhanced-items","natural-npc","item-overhaul"]})
    context=kernel.ok("mods.context",{"campaign":CAMPAIGN})
    ...
    assert [r["id"] for r in providers]==["item-overhaul"]
    kernel.ok("mods.order",{"campaign":CAMPAIGN,"order":["guided-creation","item-overhaul","natural-npc","enhanced-items"]})
...
def test_dependencies_and_default_order_are_preserved(kernel,tmp_path):
    alternate(kernel,tmp_path,"natural-npc","npc-overhaul",dependencies={"natural-npc":"1.1.0"})
    create_campaign(kernel)
    kernel.ok("mods.configure",{"campaign":CAMPAIGN,"id":"npc-overhaul","enabled":True})
    prior=read_json(campaign_dir(kernel.workspace)/"world.json")
    assert kernel.err("mods.order",{"campaign":CAMPAIGN,"order":["guided-creation","npc-overhaul","natural-npc","enhanced-items"]})["code"]=="invalid_params"
    assert read_json(campaign_dir(kernel.workspace)/"world.json")==prior
    order=["guided-creation","natural-npc","enhanced-items","npc-overhaul"]
...
def test_an_observer_can_explicitly_replace_an_existing_audit_slot(kernel,tmp_path):
    ...
    kernel.ok("mods.order",{"campaign":CAMPAIGN,"order":["guided-creation","enhanced-items","natural-npc","audit-overhaul"]})
    ...
    assert [r["id"] for r in identity["packages"]]==["audit-overhaul"]
