...
def test_steward_agents_are_distributable_project_definitions():
    ...
    for filename, name in expected.items():
        ...
        assert frontmatter["tools"] == "read, grep, find, bash, subagent, await_subagent"
...
def test_pi_coc_launcher_keeps_declarative_packages():
    """Only filesystem-path packages are the launcher's to rewrite.

    The pinning rewrote the whole `packages` array, which deleted declarative
    entries on every launch -- silently uninstalling
    `npm:@arhen/pi-core-subagent`, the extension that provides the `subagent`
    tool, one boot after it was installed.
    """
    launcher = LAUNCHER.read_text(encoding="utf-8")
    assert '/^(npm|git|https|ssh):/.test(entry)' in launcher
    assert "const next = [repoRoot, ...declarative];" in launcher


def test_pi_coc_launcher_mirrors_coordinators_beside_the_stewards():
    """`@arhen/pi-core-subagent` discovers agents from `.pi/agents`.
    ...
    while the launcher mirrored only the stewards. Dispatching it failed with
    `Unknown agent: "coc-opening-source-coordinator"` and the campaign could
    not leave that gate. Seen live on 2026-09-02 in campaign too-many-1920.
    """
    launcher = LAUNCHER.read_text(encoding="utf-8")
    assert (
        "COORDINATOR_AGENT_SOURCE=$REPO_ROOT/plugins/coc-keeper/agents" in launcher
    )
    assert "COORDINATOR_AGENT_RUNTIME=$STEWARD_AGENT_RUNTIME" in launcher
