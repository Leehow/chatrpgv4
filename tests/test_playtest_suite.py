...
def test_semantic_eval_request_exports_llm_judge_contract(tmp_path):
    ...
    assert "pushed-roll risk ownership" in quality_questions["rulebook_procedure"]
    assert "Keeper frames and foreshadows the failure consequence" in quality_questions["rulebook_procedure"]
    assert "player confirms the risk without authoring the consequence" in quality_questions["rulebook_procedure"]
    ...
    assert "single-player current scope" in quality_questions["virtual_player_pressure"]
    assert "multiple player profiles" not in quality_questions["virtual_player_pressure"]
    assert "group-table" not in quality_questions["virtual_player_pressure"]
