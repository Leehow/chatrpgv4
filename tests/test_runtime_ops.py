...
def _answer(value, *, pdf_index: int = 1) -> dict:
    return {
        "status": "source",
        "value": value,
        "source_refs": [{"source_id": "pdf:raw-source", "pdf_index": pdf_index}],
    }
...
def _unresolved(*, pdf_index: int = 1) -> dict:
    return {
        ...
        "inspected_source_refs": [
            {"source_id": "pdf:raw-source", "pdf_index": pdf_index}
        ],
    }
...
def _bind_fast_facts_source(
    ...
    campaign_id: str,
    *,
    # Not `pdf:raw`: b6c82558 refuses a source_id the Keeper cannot read, and
    # `pdf:` needs at least four characters after it. This id rides into
    # model-facing context through source_refs. The fixture predated that rule
    # and had been failing closed at bind time ever since, which made three
    # tests about gating facts fail for a reason that had nothing to do with
    # gating -- they never reached the logic they exist to check.
    source_id: str = "pdf:raw-source",
...
def test_fast_facts_unblock_character_creation_and_reach_the_briefing(tmp_path):
    ...
    assert canonical_ref["source_id"] == "pdf:raw-source"
...
@pytest.mark.parametrize("facts,match", [
    (_fast_facts(era={"status": "source", "value": "  ",
                      "source_refs": [{"source_id": "pdf:raw-source", "pdf_index": 0}]}),
    ...
    (_fast_facts(era={"status": "source", "value": "1920s",
                      "source_refs": [{"source_id": "pdf:raw-source", "pdf_index": -1}]}),
...
def test_adopt_source_facts_projects_source_content_out_of_campaign_and_result(
    ...
    for facts in (returned_facts, stored_facts):
        ...
        assert ref["source_id"] == "pdf:raw-source"
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"source_id": "pdf:raw-source", "pdf_index": 9}, "uncached pdf_index 9"),
    ],
)
