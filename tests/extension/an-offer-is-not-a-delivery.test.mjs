...
test('§NN: a handover that cites a roll which has not settled is refused, so the write cannot precede its own decision', async t => {
...
test('§NN: a handover that cites a failed roll is refused, and the fix names the call that records the truth', async t => {
	const game = await room(t);
...
test('§NN: a handover that cites a roll which passed carries the move, and the receipt says which roll carried it', async t => {
	const game = await room(t);
...
test('§NN: a person-to-person move states its ground, and a move with no second person in it states none', async t => {
	const game = await room(t);
...
test('§NN: an offer moves nothing, stands in the capsule, and a plain move cannot walk past it', async t => {
	const game = await room(t);
	const call_id = game.next();
	await game.call('table.apply', {call_id, effects: [{kind: 'object', name: 'Study chair',
...
test('§NN: a declined offer leaves the paper where the prose said it was, and the object can then move on', async t => {
	const game = await room(t);
...
test('§NN: an accepted offer needs its ground, and closes with the move', async t => {
	const game = await room(t);
	await game.apply([{kind: 'object', name: 'Study chair', from: game.sheet.name, to: 'Steven Knott', offer: 'made'}]);
...
test('§NN: an offer that was never made cannot be closed', async t => {
	const game = await room(t);
