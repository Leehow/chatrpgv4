...
def test_completion_audit_fails_when_source_transcript_outcome_note_ignores_selected_language(tmp_path):
    runs = [
        {"run_id": "v2-haunting-module", "audit_profile": "haunting_module", "audit_result": "PASS", "coverage_evaluator": "codex-llm-semantic-v1"},
        {"run_id": "v3-chase-drill", "audit_profile": "chase_drill", "audit_result": "PASS", "coverage_evaluator": "codex-llm-semantic-v1"},
        {"run_id": "v4-multi-profile-pressure", "audit_profile": "multi_profile_pressure", "audit_result": "PASS", "coverage_evaluator": "codex-llm-semantic-v1"},
    ...
    write_json(metadata_path, metadata)
    transcript = read_jsonl(run_dir / "transcript.jsonl")
    expected_note = f"{metadata['play_language']} localized roll note"
    transcript[5]["outcome_note"] = "canonical fixture roll note"
    transcript[5]["localized_text"] = {
        metadata["play_language"]: {"outcome_note": expected_note},
    }
    write_jsonl(run_dir / "transcript.jsonl", transcript)
    write_index(tmp_path, runs)
    automation_path = tmp_path / "automation.toml"
    ...
    finding = next(finding for finding in audit["findings"] if finding["code"] == "transcript_source_text_not_localized")
    assert finding["run_id"] == "v2-haunting-module"
    assert "turn 6 outcome_note" in finding["source_transcript_text_mismatches"]
...
def test_completion_audit_fails_when_battle_report_omits_handout_content(tmp_path):
    runs = [
        {"run_id": "v2-haunting-module", "audit_profile": "haunting_module", "audit_result": "PASS", "coverage_evaluator": "codex-llm-semantic-v1"},
        {"run_id": "v3-chase-drill", "audit_profile": "chase_drill", "audit_result": "PASS", "coverage_evaluator": "codex-llm-semantic-v1"},
        {"run_id": "v4-multi-profile-pressure", "audit_profile": "multi_profile_pressure", "audit_result": "PASS", "coverage_evaluator": "codex-llm-semantic-v1"},
    ...
    write_json(campaign_dir / "scenario" / "handouts.json", [
        {
            "id": "fixture-handout",
            "title": "Fixture handout title",
            "summary": "Fixture handout summary",
            "content": "Fixture handout full player-facing content.",
        },
    ])
    write_index(tmp_path, runs)
    automation_path = tmp_path / "automation.toml"
    ...
    finding = next(finding for finding in audit["findings"] if finding["code"] == "battle_report_handouts_missing")
    assert finding["run_id"] == "v2-haunting-module"
    assert "Fixture handout full player-facing content." in finding["missing_handout_samples"]
