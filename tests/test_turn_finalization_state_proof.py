...
def test_failed_and_unknown_ops_are_rejected() -> None:
    ...
    failed = _reasons(
        [_call("state.item_grant", "grant-keys", ok=False)],
        [effect],
    )
    assert failed == ["failed"]
    unknown = _reasons(
        [_call("state.not_a_registered_op", "grant-keys")],
        [effect],
    )
    assert unknown == ["unknown"]
    advisory = _reasons(
        [_call("state.inventory_list", "grant-keys")],
        [effect],
    )
...
def test_successful_registered_state_op_is_accepted() -> None:
    ...
    rows = coc_turn_finalization._state_delta_proof_violations(
        [_call("state.item_grant", "grant-keys")],
        [effect],
    )
    assert rows == []
...
def test_combat_receipt_proves_loaded_ammunition_not_item() -> None:
    ...
    window = [_call("combat.resolve", "combat-shot")]
...
def test_decision_effect_and_subject_mismatch_are_rejected() -> None:
    ...
    wrong_decision = _reasons(
        [_call("state.item_grant", "other-id")],
        [item],
    )
    assert wrong_decision == ["missing"]
    wrong_effect = _reasons(
        [_call("state.cash_grant", "shared")],
        [item],
    )
    assert wrong_effect == ["mismatch"]
    wrong_subject = _reasons(
        [_call("state.item_grant", "shared", investigator="other")],
        [item],
    )
...
def test_idempotent_replay_links_to_original_success() -> None:
    effect = _item_effect(decision_id="grant-keys")
    original = _call("state.item_grant", "grant-keys")
    replay = _call("state.item_grant", "grant-keys", replay=True)
    ...
    failed_then_replay = coc_turn_finalization._state_delta_proof_violations(
        [_call("state.item_grant", "grant-keys", ok=False), replay],
        [effect],
    )
    assert failed_then_replay == []


def test_multi_effect_window_proves_each_effect_once() -> None:
    window = [
        _call("state.item_grant", "grant-keys"),
        _call("state.cash_grant", "grant-cash"),
        _call("state.item_grant", "grant-keys", replay=True),
    ]
