...
> Task1 `5421b9a`（roll contract 归一）· Task2 `932dfd1`（通用失败路由，另 `60e64f0` 遗留告警）· Task3 `33878a5`（关键线索可达审计）· Task4 `0eb6eff`（Idea Roll v2 / 场景出口压力 / roll density）· Task5 `e007c32`（proposal transform）· Task6 `c9b1280`（NPC agency 分类法）· Task7 `4931e09`（sync 收尾；zcode 轨道其后已由 `e314156` 移除，single-track 化）。
...
**Tech Stack:** Python standard library, existing plugin scripts, pytest, JSON files, `scripts/sync_coc_plugin_copy.py` for Codex synchronization.

## Global Constraints

- Canonical edits start in `plugins/coc-keeper/`; keep changes only in `plugins/coc-keeper/`.
...
- Before finishing plugin work, run:
  ...
  and `# single-track: no sync script required`.
...
**Files:**
- Modify via script: `plugins/coc-keeper/**`
- Test: focused test files and required sync tests

**Interfaces:**
- Consumes: completed Codex-track changes.
- Produces: Codex single-track plugin and passing verification evidence.
...
- [ ] **Step 2: Confirm Codex single-track plugin**
...
Expected: sync writes expected Codex files, then `--check` reports clean.

- [ ] **Step 3: Run required single-track tests**
...
Expected: only intended plugin, tests, skill docs, and synchronized Codex files changed, plus any pre-existing unrelated `AGENTS.md` remains unstaged and untouched.
...
- Spec coverage: roll contracts, fail-forward, clue reachability, Idea Roll, scene exit pressure, proposal transform, Psychology policy, roll density, NPC agency, storylet trigger preservation, and single-track sync are each mapped to a task.
