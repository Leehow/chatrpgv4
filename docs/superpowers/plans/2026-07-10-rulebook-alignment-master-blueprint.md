...
- Narration envelope 增强（narrator 首跑暴露）：`approved_reveals` 附 `player_safe_summary` 正文（现只有 clue_ids）；envelope 加已结算 `rule_results`（成败/代价，narrator 现只见 requests）；场景感官锚点/地点显示名；`npc_moves` 消毒后保留对白种子与显示名
