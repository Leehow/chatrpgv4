...
**Canonical track:** `plugins/coc-keeper/`（Codex single-track）。历史双轨 `plugins/coc-keeper-zcode/` 已移除；本文所有 `file:line` 指向 `plugins/coc-keeper/`。
...
- 运行时只维护 `plugins/coc-keeper/`；不要再创建 `coc-keeper-zcode` 副本。
