...
1. **COC Plugin Single-Track Law** — 运行时只维护 `plugins/coc-keeper/`。立绘生成等宿主能力用 `CODEX_ONLY_IMAGEGEN` 等显式标记门控，不要再复制第二套插件树。
...
> 全局前提（写进每条方案的隐含约束）：
> - **Single-Track**：运行时只改 `plugins/coc-keeper/`。立绘等宿主能力用 `CODEX_ONLY_IMAGEGEN` 显式门控。
> - **Semantic Matcher Constitution**：不扫自由文本。P0-4 允许新增语义步骤，输出必须是结构化字段+recorded reason。
> - **测试**：每条加针对性单测；改插件行为至少跑 `test_plugin_metadata.py`。
...
- 根因：注册表只写不读 + 数据无字段。
- 方向：scenario-import 编译时把 handout asset 挂到 clue/scene 的结构化字段（如 `clue.handout_asset_id`）；clue_reveal 事件带上 asset ref；narration contract 在 `player_visible` 时渲染绝对路径 Markdown 图片（Codex）或标题+摘要（其他宿主按平台能力降级）。
...
### 8.1 Single-Track 与测试门禁

- 运行时只维护 `plugins/coc-keeper/`；不要再创建 `coc-keeper` 副本。
- 立绘生成等宿主能力保留在 `CODEX_ONLY_IMAGEGEN` 标记块内；非 Codex 宿主跳过。
- 改插件行为后至少跑：
  ```
...
- 2026-07-10 实现对账：波次 0-5 全部落地（P0-1..4、P1-1..8、P2-1/2/2-3'/4'/5/6/7），commit 对账清单见各波次 plan 文件头部的"实现进度对账"块。scenario 数据激活由 `2026-07-09-coc-scenario-data-wiring.md` 收口，其遗留的 arrival/first_contact 场景型 storylet 边界亦已补齐。
- 2026-07-10 增补：Idea Roll 恢复阀 + 线索 signpost 阶梯落地（`ff6ce36`，规则书 ~p.199 对齐，报告见 `.superpowers/sdd/idea-roll-signpost-report.md`）。本蓝图使命完成；下一阶段优化空间另立 `2026-07-10-next-phase-optimization-audit.md`（N1-N8），不再追加到本文。
