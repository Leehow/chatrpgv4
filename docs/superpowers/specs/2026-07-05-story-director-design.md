...
- **PDF→JSON 硬解析代码**——模组剧情图由 skill 驱动 LLM 用 zcode 原生检索（grep/read）生成，脚本只做结构校验。
...
```
Layer 1 — LLM 解析（skill 驱动，利用 zcode 原生检索能力）
  coc-scenario-compile skill 读模组 → 产出剧情图 JSON
  工具：grep/PDF read/webfetch（zcode 原生，不写硬解析代码）
...
理由：codex/zcode 有成熟的 grep/read，LLM 自己能从 PDF 提取结构化数据。我们不写解析代码（不同模组结构差异大，LLM 能自适应）。但剧情图质量直接影响 director 的"灵魂"（缺 fallback route 的 clue graph 会让玩家卡死），所以 Layer 2 用脚本做硬性结构校验。
