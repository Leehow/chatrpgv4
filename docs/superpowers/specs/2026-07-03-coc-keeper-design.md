...
The localization layer applies only to player-visible narration, NPC speech, player prompts, player-view transcript text, recaps, and reports. `player-view.jsonl` must keep the same safe event structure as `transcript.jsonl`, but its visible `text` fields should be rendered for `play_language`, including system roll summaries such as success levels and skill display names. Keep machine-facing markers, JSON keys, filenames, canonical skill keys, rule enum values, and Mechanical Log roll text stable. Stored transcript payload fields also stay canonical. Player-visible skill display names render through `localized_terms[play_language]` while stored JSON and Mechanical Log entries preserve canonical skill keys.
