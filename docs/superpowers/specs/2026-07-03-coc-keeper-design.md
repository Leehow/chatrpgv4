...
Player-readable Scene-by-Scene Replay must also render `resource_change` events when a rulebook-required resource changes. The Haunting specifically requires Corbitt Magic points to be tracked for The Floating Knife and Corbitt's body movement; otherwise emit `haunting_corbitt_magic_points_missing`.
...
- Corbitt Magic points `resource_change` events for The Floating Knife's 1 Magic point combat-round cost and Corbitt's 2 Magic points body-movement cost; otherwise emit `haunting_corbitt_magic_points_missing`
