...
- half/fifth values for characteristics and skills
...
- SAN rolls
- sanity loss
- SAN before/delta/after tracking
...
`index.json`, `loop-decision.json`, and `suite-report.md` are regenerated by `coc_playtest_suite.py`. They summarize all indexed playtest runs and provide a `Core Coverage Matrix` with `character_dossier`, `kp_player_transcript`, `mechanical_rolls`, `combat`, `chase`, `sanity`, `meta_game`, and `player_feedback` for the current `evaluated_runs` set. Each run stores `coverage_evaluator` and `coverage_reasons` so coverage decisions are auditable; historical baseline runs remain visible as `ignored_historical_runs` but must not satisfy current coverage or quality gates. Non-default duplicate language runs can remain visible as `optional_evidence_runs` and in `language_coverage`, but they must not satisfy coverage/quality gates or drive repair blockers when a default `zh-Hans` completion-required run set already exists. Empty `Remaining Gaps` and `Remaining Quality Gaps` report conclusions apply to evaluated playtest runs; empty `Remaining Language Gaps` applies to the current language coverage scope. Completion audit emits `suite_matrix_references_non_evaluated_run` if any coverage or quality matrix entry lists a run id outside `loop_decision.evaluated_runs`.
...
`coc_playtest_suite.py --write-semantic-requests --root <repo-root>` writes `artifacts/semantic-eval-request.json` for each run. The harness must not fabricate `semantic-eval-result.json`; Codex or another LLM semantic evaluator reads that exact request and writes `artifacts/semantic-eval-result.json` with `schema_version`, `run_id`, `evaluator_id`, `evaluation_provenance`, `coverage`, `quality`, `root_cause_classification`, and `next_loop_fix_target`. `evaluation_provenance.kind` must be `llm`, and `evaluation_provenance.request_sha256` must match the canonical JSON hash of the reviewed request. Then `coc_playtest_suite.py --root <repo-root>` consumes those results through the CLI default evaluator `semantic-artifact` and records the LLM evaluator id and reasons in the suite index. Missing result files must be treated as missing semantic evidence, not as permission to fall back to a natural-language matcher. `--evaluator structured-source` is only an explicit offline fixture or mechanical source smoke mode, not for completion-oriented quality gates.

The semantic request includes `coverage_keys` and `quality_dimensions`; completion audit must reject semantic results whose reviewed request omits the required coverage, quality, or expected-output contract even when the provenance hash matches. The result `coverage` object uses the same keys as the suite matrix, and the result `quality` object scores `module_fidelity`, `rulebook_procedure`, `immersion_and_pacing`, `localized_visible_dialogue`, `actual_play_replay`, `state_continuity`, `spoiler_safety`, `player_agency`, `virtual_player_pressure`, and `report_completeness`; each dimension includes `score`, `passed`, and a non-empty `reason`. `localized_visible_dialogue` judges whether visible Keeper/player text follows the selected `play_language` while machine-readable keys, ids, markers, and enums stay English/ASCII-stable. The suite index and `## Run Index` must expose each run's `play_language` and language profile so localized quality gates are auditable from the report. When the completion-required active profiles (`haunting_module`, `chase_drill`, and `multi_profile_pressure`) are all present, the suite must also emit `language_coverage` and block on `language_gaps` unless at least one active run proves the default `zh-Hans` path. Non-default selected languages are optional evidence, not a completion blocker; when a default `zh-Hans` completion set exists, duplicate non-default runs belong in `optional_evidence_runs` instead of `evaluated_runs`. The suite report writes evaluated-run `## Quality Matrix`, `## Quality Evidence`, and `quality_gaps` so the next loop can tell whether the blocker is test coverage, system behavior, report output, or design.

`loop-decision.json` is the machine-readable next-action gate. It contains `evaluated_runs`, `optional_evidence_runs`, `ignored_historical_runs`, `blockers`, `next_action`, `thread_goal_status`, `thread_goal_next_action`, and a status of either `needs_repair` or `ready_for_completion_audit`. Historical baseline runs remain visible only as `ignored_historical_runs`; non-default duplicate language runs can remain visible only as `optional_evidence_runs` when the default `zh-Hans` completion set is already available. `Non-Passing Evaluated Runs` lists failures from the currently evaluated run set, so old smoke artifacts and optional language artifacts do not read like current repair blockers when the evaluated runs already cover the suite. When the status is `ready_for_completion_audit`, it means the current artifact set has no suite-level coverage or quality gaps and `coc_completion_audit.py` can run; it is not a Codex thread-goal completion signal. `thread_goal_status` must stay `active_not_complete` until the external goal has separate thread-level proof. Campaign save lifecycle values must not use `complete` for a concluded scenario, because that reads like a goal-completion signal in semantic requests and indexes; use `concluded` for finished campaign saves while reserving goal completion for `goal_completion_gate` and the external Codex goal state. `coc_completion_audit.py` generates `.coc/playtests/completion-audit.json` and `.coc/playtests/completion-audit.md` from the suite index, semantic artifacts, active run source files, active run artifacts, optional evidence run ids, and watchdog automation state; completion audit JSON and Markdown must preserve `optional_evidence_runs` as visible context without treating those runs as active repair scope. The completion audit must reject active runs that no longer have non-empty, parseable JSON/JSONL transcript, player view, Keeper view, feedback, campaign files, roll/event logs, memory summaries, and reusable investigator source records, so stale reports or semantic results cannot pass after the underlying actual-play evidence disappears, is blanked, or is no longer structured. The completion audit must also reject source shells that parse but do not contain minimum machine-readable actual-play evidence, starting with `transcript.jsonl` rows for both `keeper_under_test` and `player_simulator` with visible text; `player-view.jsonl` rows for public character state plus visible transcript turns; `keeper-view.jsonl` rows for Keeper context, `keeper_secret_ids`, and visible transcript turns; `player-feedback.jsonl` rows with a numeric `score` and visible feedback `text`; campaign `scenario/handouts.json` rows with player-visible label/title and required player-visible summary; campaign `logs/rolls.jsonl` rows with type, payload, dice roll, target, and outcome; campaign `logs/events.jsonl` rows with type and payload plus required event type enums for the active `audit_profile`; campaign memory rows with visible `summary`; reusable investigator files with skill allocation, character skills, history summary, development record, and inventory summary evidence; required pushed-roll source evidence with Keeper-owned failure consequences, player risk confirmation, stable `roll_id`, and ordered transcript stages; single-player style-pressure source evidence with each required `player_profile` enum represented in transcript and feedback rows plus at least one transcript row per required profile carrying structured `intent` and localized `intent_display`; and meta-game source evidence with separated `mode=meta` player questions and Keeper answers in `transcript.jsonl`. The completion audit must reject workspace investigator indexes that mix campaign state such as current HP, current SAN, active scene, conditions, or skill checks into reusable investigator entries; those values belong under campaign `save/` and investigator history/development records. It must also reject `player-view.jsonl` public character state whose `current_state` is missing or stale compared with `save/investigator-state/<investigator-id>.json`, because the player-safe view must show the current campaign overlay without mutating the reusable character card. For localized runs, the completion audit must also reject `player-view.jsonl` public character state that leaks canonical player-visible terms from `localized_terms[play_language]` in scenario display fields, investigator display fields, player-visible skill display keys, derived display values, current-state display values, or backstory values, reject source `transcript.jsonl` KP/player top-level `text` that does not match `localized_text[play_language].text` for non-default play languages, reject source `transcript.jsonl` player-visible adjunct prose fields such as `outcome_note` that do not match `localized_text[play_language]`, reject player-view transcript `speaker` display values that leak canonical names, profile ids, or unlocalized English speaker tokens, reject player-view/player-feedback rows whose `player_profile_display` is missing or still equals the canonical `player_profile` enum when `player_profile_labels[play_language]` supplies a localized label, reject player-view `localized_text[play_language]` values that leak canonical player-visible terms, and reject player-view transcript `intent`/`ruling` rows whose `intent_display`/`ruling_display` is missing or still equals the canonical enum when `localized_text[play_language]` supplies a localized detail. The completion audit must reject battle-report shells that contain the required anchors but do not render the visible non-system source dialogue from `transcript.jsonl` in `Actual Play Replay` or `Session Transcript`, or that render those source dialogue turns out of transcript order, because a真人跑团实录 must show what KP and players actually said in table order rather than only title placeholders. It must also reject handout sources without player-visible summaries and battle reports whose `Handouts` section omits player-visible labels, titles, summaries, or routes from `scenario/handouts.json`, so the report cannot pass while hiding the table handout register. It must also reject battle reports whose `Mechanical Log`, after stripping HTML comments, omits visible source roll evidence for any roll from `logs/rolls.jsonl`; visible evidence may be either the canonical source roll line assembled from machine fields (`skill`, `actor`, `roll`, `target`/`effective_target`, and `outcome`) or the selected `play_language` roll sentence rendered from `language_profile`, `localized_terms`, and source character names. Hidden comments such as `roll-source` are audit anchors only and must not prove player-visible actual-play output. It must reject battle reports whose Scene-by-Scene Replay or State Changes omit structured `logs/events.jsonl` `payload.summary` values, so the actual-play report cannot pass while hiding the durable scene, clue, state, combat, chase, sanity, status, or ending events. It must reject battle reports whose Player Feedback On KP section omits structured `player-feedback.jsonl` `text` values, so the report cannot pass while hiding virtual-player evaluation of the KP experience. It must reject battle reports whose Story Recap omits structured `memory/session-summaries.jsonl` `summary` values, so saved campaign memory remains visible in the actual-play record. It must reject battle reports whose Investigator Chronicle omits structured reusable-investigator `history.jsonl`, `development.jsonl`, or `inventory-history.jsonl` player-readable summary/carryover values, allowing machine glossary localization for player-visible names and scenario titles. The completion audit must cross-check the suite index against current active `semantic-eval-result.json` artifacts so stale `covered` or `passed` matrix values cannot survive after the semantic artifacts change; the supporting artifact must be one of the active run ids listed on that exact matrix entry, not merely any active run.

Clarification for `player-view.jsonl`: `public_character_state` must never localize JSON object keys. `skills` and `derived` keep canonical/ASCII keys; localized player labels belong in `skill_display[]` and `derived_display[]` values. Completion audit should emit `player_view_public_state_unstable_keys` when public character state uses localized/non-ASCII JSON keys.

The completion audit must also reject battle reports whose Player Feedback On KP lines render source feedback comments and scores without binding the same source row's category, score, profile voice, and comment in one visible feedback line; otherwise emit `battle_report_feedback_binding_missing`.

The completion audit must also reject battle reports whose Investigator Creation omits structured reusable-investigator `creation.json` values for generated characteristics, characteristic half/fifth values, age choice, age modifier bracket, EDU improvement checks, characteristic reductions, APP reduction, MOV penalty, occupation skill points, personal-interest skill points, credit rating, living standard, cash, assets, spending level, skill allocation, skill half/fifth values, and equipment, allowing machine glossary localization for player-visible names, occupations, skills, gear, and finance display values. It must reject `creation.json` records that lack rulebook Table II finance fields derived from Credit Rating and period, even if credit rating itself is present. It must also reject character source records whose `derived.MOV` or `creation.derived.MOV.value` does not match the rulebook Movement Rate table using STR, DEX, SIZ, and the age MOV penalty; otherwise emit `derived_movement_rate_mismatch`.

The completion audit must also reject battle reports whose Character Dossier omits structured reusable-investigator `character.json` values for name/id, occupation, era, characteristics, characteristic half/fifth values, derived values, skills, skill half/fifth values, and backstory, allowing language-profile labels and glossary localization for player-visible names, occupations, skills, and character-sheet terms.

The completion audit must also reject battle reports whose Chase Tracker omits structured `save/chase.json` values for participants, DEX order, location chain, round summaries, and outcome, allowing language-profile labels and glossary localization for player-visible chase terms. Chase drills must also persist `rounds[].turns[].actor_id` entries ordered as a DEX-order subsequence, so `chase_dex_order_not_proven` can catch runs whose saved chase state cannot prove who acted first in each chase round. Any `rounds[].turns[]` entry that crosses a `location_chain[]` entry with `label: hazard` must record `hazard_id` and `hazard_roll_id`, and `logs/rolls.jsonl` must contain a matching payload with `roll_id` and `chase_hazard_id`; otherwise emit `chase_hazard_resolution_missing`, so every participant who crosses a hazard is proven to have resolved it. Any `rounds[].turns[]` entry that crosses a `location_chain[]` entry with `label: barrier` must record `barrier_id` and `barrier_roll_id`, and hide-to-escape turns must record `hide_attempt_id`, `hide_roll_id`, `hide_search_actor_id`, and `hide_search_roll_id` with matching `logs/rolls.jsonl` payload links; otherwise emit `chase_barrier_hide_resolution_missing`, so escape by hiding is proven from structured state rather than narration alone. The completion audit must also reject final chase narration, transcript, or report text that places a participant at a different location than `save/chase.json.participants[].position`, using machine location ids plus `localized_terms[play_language]` aliases; emit `chase_transcript_position_conflict` so state continuity errors are not hidden by a correctly rendered Chase Tracker.
...
Current completion-oriented playtests must use exactly one active investigator in `party.json`; if the merged `active_investigator_ids` / `investigator_ids` set contains more than one id, `coc_completion_audit.py` emits `active_run_party_not_single_player`. The current completion scope is single-player only; extra-player requirements are not current blockers.
...
- derive HP, MP, SAN, Luck, DB, Build, and MOV from the rulebook Movement Rate table
...
-> compute loss
-> record san_before, san_delta, and san_after
-> on failure, record involuntary_action (rulebook p.166): kind in jump_in_fright / cry_out / involuntary_movement / involuntary_combat_action / freeze, plus summary
...
Per Keeper Rulebook p.166, failing a SAN roll always causes the investigator to lose self-control for a moment; the Keeper chooses one of five involuntary actions. This is distinct from a bout of madness: a failed SAN roll that does not reach the 5+ SAN-loss temporary-insanity threshold still records an `involuntary_action`, and a failed SAN roll that does trigger temporary insanity records both the momentary `involuntary_action` and the subsequent bout. The structured SAN roll payload (and the matching `type: sanity` event) carries an `involuntary_action` block with `kind`, `summary`, and `rule_ref: core.sanity.failure_involuntary_action`; successful SAN rolls do not carry one.
...
2. `player_simulator`
   - Simulates one human player; the current completion scope is single-player only.
...
`suite-report.md` is the cross-run table of contents and coverage proof. It should include `## Run Index`, `## Non-Passing Evaluated Runs`, `## Evaluator Note Blockers`, `## Loop Decision`, `## Core Coverage Matrix`, `## Coverage Evidence`, `## Quality Matrix`, `## Quality Evidence`, and `## Remaining Gaps`. Run Index must preserve canonical `campaign_title`, `scenario`, `audit_profile`, `player_profile`, and `party_size` in `index.json` while rendering matching display fields in visible Markdown, so localized active runs can keep stable source values without exposing confusing labels such as `multi_profile_matrix`, untranslated module titles, or hidden party-size assumptions to the evaluator. The coverage matrix must report `character_dossier`, `kp_player_transcript`, `mechanical_rolls`, `combat`, `chase`, `sanity`, and `player_feedback` using only the current `evaluated_runs`, so the evaluator can see whether the active playtest set covers the requested Keeper workflows without hiding failed or missing audits. The quality matrix follows the same evaluated-run boundary, and no-gap prose for coverage/quality must say evaluated playtest runs rather than indexed runs. Language no-gap prose must refer to the current language coverage scope, because optional non-default evidence can remain indexed without joining the repair scope. Completion audit must emit `suite_matrix_references_non_evaluated_run` if either matrix lists a run outside `loop_decision.evaluated_runs`. The suite loop must turn active medium-or-higher or failing-severity evaluator notes into `evaluator_note_blocker` entries even when semantic coverage and quality matrices pass, so a human/evaluator warning such as scripted actual-play compression or a state error cannot be hidden behind a green matrix. Completion audit must also reread active `evaluator-notes.jsonl` files directly and emit `active_evaluator_note_blocker` if a medium-or-higher or failing-severity note appears after a stale suite report was generated. Completion audit must treat watchdog monitor liveness as structured automation status, not monitor prompt text. An automation file with `status = "ACTIVE"` is active even if its prompt is reworded or localized; do not exact-match a natural-language prompt phrase as a completion gate.

The `semantic-artifact` evaluator is the default CLI path when Codex is available as evaluator. It should read `semantic-eval-result.json`, preserve `evaluation_provenance`, `root_cause_classification`, and `next_loop_fix_target` for the next repair loop. `coc_completion_audit.py` then performs the artifact-level completion audit once `loop-decision.json` says `ready_for_completion_audit`, including the `request_sha256` provenance check.
...
`coc_playtest_harness.py` provides reproducible baselines for this loop. Its `--play-language <language>` option chooses the visible play language for persisted run/campaign metadata and generated reports, defaulting to `zh-Hans`; non-default language runs must store `language_profile` plus `localized_terms[play_language]` and must not silently reuse the `zh-Hans` glossary under another language key. The `rulebook-smoke` profile should generate a small The Haunting-derived run with a real opening hook, player intent, Keeper rulings, an investigation roll, clue flow, a sanity prompt, memory, feedback, and then run the report and audit generators. The `haunting-module` profile should generate a module-level The Haunting transcript with Mr. Knott, Arty Wilmot, Chapel clues, The Old Corbitt Place, Bed Attack, basement hazards, structured HP `damage` rolls for Bed Attack and pushed basement-search failure, The Floating Knife, Corbitt's Hiding Place, Corbitt combat, Corbitt Magic points `resource_change` events for `flesh_ward`, `floating_knife_attack`, and `animate_body`, a structured `own_dagger_ignores_spells` combat exception showing why Flesh Ward no longer protects Corbitt when his own dagger hits, a structured `reward` roll proving the conclusion `1D6` SAN reward and final SAN continuity, final HP/SAN, rewards, explicit Chase Summary non-applicability, sandbox investigator `creation.json`, `history.jsonl`, `development.jsonl`, and player feedback. The `chase-drill` profile should generate a rulebook chase drill that writes `save/chase.json`, writes sandbox investigator `creation.json`, `history.jsonl`, and `development.jsonl`, shows speed roll, MOV, movement actions, location chain, DEX order, hazard resolution for every participant who crosses the hazard, barrier and hide/search roll links, conflict, and why the quarry escapes, and renders `save/chase.json` as `## Chase Tracker`.
...
- structured Keeper-controlled NPC roleplay turns for core social/investigation scenes, including Mr. Knott, Arty Wilmot, Gabriela Macario, and 马卡里奥一家, rendered with localized `KP[NPC]` labels in Actual Play Replay and Session Transcript; otherwise emit `haunting_npc_dialogue_missing`
- sandbox investigator creation records with rulebook Chapter 3 steps, generated characteristics, skill-point formulas, `skill_allocation`, credit rating, rulebook Table II finances, backstory, and starting equipment; otherwise emit `investigator_creation_missing` or `investigator_skill_allocation_missing`
- derived MOV values that do not match STR/DEX/SIZ plus the age MOV penalty; otherwise emit `derived_movement_rate_mismatch`
...
- structured HP damage rolls for Bed Attack and pushed basement-search failure, with `damage_kind: hit_points`, `source`, `die`, `die_rolls`, `flat_modifier`, `roll`, `hp_before`, `hp_delta`, `hp_after`, and matching `damage_roll_id` links from `events.jsonl`; otherwise emit `haunting_damage_roll_missing`
- damage and reward dice must render as `die = total` with die-face/modifier breakdown rather than percentile-style `roll / target`; otherwise emit `non_percentile_roll_rendering_invalid`
- SAN loss rolls must record and render `san_before`, `san_delta`, and `san_after`; otherwise emit `sanity_resource_delta_missing`
- every failed SAN roll must carry an `involuntary_action` block (`kind` in `jump_in_fright` / `cry_out` / `involuntary_movement` / `involuntary_combat_action` / `freeze`, plus `summary` and `rule_ref: core.sanity.failure_involuntary_action`); the matching `type: sanity` event carries the same block. Per Keeper Rulebook p.166 a failed SAN roll always causes momentary loss of self-control, distinct from any later bout of madness; otherwise emit `sanity_failure_involuntary_action_missing`
...
- a structured `reward` roll proving The Haunting conclusion `1D6` SAN reward with `reward_kind: sanity`, `source: conclusion_rewards`, `die: 1D6`, `roll_id`, `san_before`, `san_delta`, and `san_after` matching final SAN; otherwise emit `haunting_conclusion_reward_roll_missing`
- if a structured roll payload sets `temporary_insanity_triggered: true`, a `bout_of_madness` event and battle-report `疯狂发作` entry showing `mode`, Keeper control boundary, `control_returned: true`, and recovery note. `mode: real_time` requires the 1D10-round loss-of-control episode, actual `duration_roll`/`duration_rounds`, and one keeper-controlled `rounds[]` entry for each bout round. `mode: summary` requires `summary_table: table_viii_summary`, `summary_roll`, `summary_result`, `duration_die: 1D10`, `duration_roll`, and `duration_hours` instead of `duration_rounds`/`rounds`; The Haunting's Corbitt temporary-insanity scene must use summary mode when the investigator is alone.
- if temporary insanity was triggered, the final status event and battle-report final-state summary must either set `temporary_insanity_resolved: true` with recovery evidence or carry an `unresolved_conditions[]` entry with `condition: temporary_insanity_underlying`, `duration_hours`, `remaining_hours`, `summary`, and `player_visible_summary`; the `player_visible_summary` must also appear in the player-visible KP transcript before session end, otherwise emit `temporary_insanity_final_state_missing` or `temporary_insanity_final_state_not_player_visible`
...
- single-player-first runtime; current completion scope is single-player only
