...
`suite-report.md` is the cross-run table of contents and coverage proof. It should include `## Run Index`, `## Non-Passing Runs`, `## Evaluator Note Blockers`, `## Loop Decision`, `## Core Coverage Matrix`, `## Coverage Evidence`, `## Quality Matrix`, `## Quality Evidence`, and `## Remaining Gaps`. The coverage matrix must report `character_dossier`, `kp_player_transcript`, `mechanical_rolls`, `combat`, `chase`, `sanity`, and `player_feedback` so the evaluator can see whether the current playtest set covers the requested Keeper workflows without hiding failed or missing audits. The suite loop must turn active medium-or-higher evaluator notes into `evaluator_note_blocker` entries even when semantic coverage and quality matrices pass, so a human/evaluator warning such as scripted actual-play compression cannot be hidden behind a green matrix. Completion audit must also reread active `evaluator-notes.jsonl` files directly and emit `active_evaluator_note_blocker` if a medium-or-higher note appears after a stale suite report was generated.
