...
**starter（起步模组）** — 已经是 v3 图形式、随仓库分发的模组：`the-haunting`、`mystery-house`、`the-haunting-rulebook`，`content/starters/<id>/module-graph.json`（`the-white-war` 于 2026-09-08 连同内容一并删除）。产生：`scripts/starter_graph.py`（把旧七文件 IR 投影成 v3 图，契约 §14.9）；消费：`campaign.create`、`ModuleStore.register_starter`。契约 §14.1、§14.9。代码：`kernel/coc/modules/store.py` 的 `register_starter`（约 186 行）。
