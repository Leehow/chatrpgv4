...
| coc-tiered-background-orchestration | [Tiered background orchestration and first-contact readiness](coc-tiered-background-orchestration.md) | `In Progress` | 2026-07-21 | Cursor coordinator is fail-closed after three nested-MCP capability mismatches; retain the unintegrated thin contract and run the window-equivalent opening/first-contact path through supported direct-leaf hosts next. |
