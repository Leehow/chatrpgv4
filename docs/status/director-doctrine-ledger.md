...
| `inert-in-matrix` | 36 | exercised by this checkpoint, but perturbing it changed no decision |
| `not-exercised` | 60 | belongs to a structure type this checkpoint is not; the sweep says nothing about it |
...
**113 of 117 doctrine nodes (97%) cannot name their origin.** Each carries a `falsifiable_by` describing the DebugExperiment that could settle it. Retiring them one recorded experiment at a time is slice D5.
...
| --- | --- | --- | --- |
| `scoring-rule:character:agenda-npc-in-scene` | `0.7` | inert-in-matrix (0) | Lanes on a checkpoint with both an available clue and an agenda NPC; vary this value and compare REVEAL-vs-CHARACTER. |
...
| `scoring-rule:cut:exit-condition-met` | `0.8` | inert-in-matrix (0) | Lanes from a checkpoint whose exit condition is met while a threat clock is near full; compare CUT-vs-PRESSURE. |
| `scoring-rule:cut:explicit-move-intent` | `1.0` | inert-in-matrix (0) | Lanes on a move-intent checkpoint under hub_sandbox (CUT weight 0.7); confirm whether movement still wins after weighting. |
| `scoring-rule:cut:main-line-complete` | `0.7` | inert-in-matrix (0) | Lanes from a main-line-complete checkpoint in a non-final scene; vary this value and compare whether the Director pushes toward the ending. |
| `scoring-rule:cut:stalled-transition-pressure` | `[0.45, 0.15, 0.85]` | inert-in-matrix (0) | Lanes replaying a checkpoint at two, three and four stalled turns; compare the turn at which CUT overtakes RECOVER. |
...
| `scoring-rule:payoff:structured-entity-overlap` | `[0.15, 0.12, 0.85]` | inert-in-matrix (0) | Lanes on a checkpoint with one-entity and three-entity temporal overlap; compare when PAYOFF first wins. |
...
| `scoring-rule:pressure:yielded-scene` | `0.85` | inert-in-matrix (0) | Lanes from a checkpoint with two recorded low-agency continues; vary this value and compare whether the Director escalates or keeps revealing. |
...
| `scoring-rule:reveal:social-intent` | `0.75` | inert-in-matrix (0) | Two lanes on a social checkpoint with an agenda NPC present; vary the social REVEAL base and compare REVEAL-vs-CHARACTER selection. |
...
| `threshold:clue-route-default-priority` | `eq 0.5` | inert-in-matrix (0) | Lanes on a scene mixing authored and unauthored route priorities; compare route ordering. |
| `threshold:compression-max-beats-ceiling` | `lte 8` | inert-in-matrix (0) | Lanes with an authored max_beats of sixteen; confirm the clamp and compare pacing. |
| `threshold:compression-max-beats-default` | `eq 4` | inert-in-matrix (0) | Lanes on a low-agency stretch with the default and a doubled cap; compare beats spent before the Director forces progress. |
| `threshold:compression-max-beats-floor` | `gte 2` | inert-in-matrix (0) | Lanes with an authored max_beats of one; confirm the clamp and compare pacing. |
| `threshold:compression-max-minutes-ceiling` | `lte 30` | inert-in-matrix (0) | Lanes with an authored max_minutes of ninety; confirm the clamp. |
| `threshold:compression-max-minutes-default` | `eq 10` | inert-in-matrix (0) | Lanes with ten and thirty minute caps; compare in-fiction clock drift over a low-agency stretch. |
| `threshold:compression-min-beats-default` | `eq 2` | inert-in-matrix (0) | Lanes with min_beats one and two; compare the shortest compressed stretch. |
| `threshold:cut-stalled-transition-turns` | `gte 2` | inert-in-matrix (0) | Lanes at one, two and three stalled turns; compare when transition pressure appears. |
...
| `threshold:low-agency-max-beats-fallback` | `eq 4` | inert-in-matrix (0) | Lanes on an unbudgeted bridge scene; vary the fallback and compare when the bridge exhausts. |
...
| `threshold:pressure-move-low-agency-count` | `gte 2` | inert-in-matrix (0) | Lanes at one and two continues with zero stalled turns; compare pressure-move presence. |
| `threshold:pressure-move-stalled-gate` | `gte 1` | inert-in-matrix (0) | Lanes at zero and one stalled turn under a REVEAL action; compare whether a pressure move accompanies the reveal. |
...
| `threshold:scene-exit-pressure-continue-count` | `gte 2` | inert-in-matrix (0) | Lanes at one and two continues; compare whether the exit directive reaches the Keeper. |
