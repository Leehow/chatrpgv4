...
**已建立会话中的一个普通回合是 36–83 秒（六个样本），对 180 秒预算余量很大。**
...
**混淆已经单独测掉了。** r51 的第二条输入（「扶着墙站稳，继续往楼梯口挪」）确实
比第一条轻，所以 r52 换了一条**同等重量**的第二输入——主动攻击：「我猛地转身扑向
那具尸体，用手电砸它的头，同时喊出他的名字。」

| lane | 第一回合 | 第二回合（重） |
| --- | --- | --- |
| heavy-1 | 178s / 22 调用 / 2 结算 | **38s** / 7 / 1 |
| heavy-2 | 147s / 24 / 4 结算 | **55s** / 8 / 1 |
| heavy-3 | 194s / 26 / 6 结算 | **83s** / 10 / 2 |

**六个「已建立会话中的回合」（r51 三轻 + r52 三重）跨度 36–83 秒，全部远低于
180 秒预算。** 最重的一个 83 秒，不到预算一半。

仍未取样到的是**最重的那类回合**：r52 的第二回合结算 1–2 次，而首回合（带预置的
Corbitt 对峙）结算 2–6 次。所以结论是「普通回合有很大余量」，不是「任何回合都不会
超」。
