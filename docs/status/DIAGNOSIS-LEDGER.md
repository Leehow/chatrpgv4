...
| Player, Narrator and Pi spawned a process/session per turn | REAL_DEFECT → RESOLVED | one-shot adapters were the initial compatibility implementation | scoped JSONL worker pool, session reuse, isolation, timeout/crash/protocol retirement and close | A31 |
