...
| `--live` plus an arbitrary fake runner could manufacture gameplay evidence | TEST_ENSHRINES_DEFECT → RESOLVED | Caller-controlled metadata was treated as provenance; old tests accepted it | report class is now decided only by a recomputed eligible receipt; missing/invalid/tampered receipts force a NON-GAMEPLAY sample; pinned runner and spoof tests | A09, A10 |
