...
下一次出现慢回合，用新增的 request/response 时间、HTTP 状态/request-id 和原有消息、工具、重试时间线区分阶段。若旧记录没有响应头信息，就明确证据缺口；不要给瞬态供应商问题写一个猜测性的生产补丁。相关约定在 [pi-host-contract.md](pi-host-contract.md) 最后一个“Provider latency evidence”小节。
