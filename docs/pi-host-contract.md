...
- A run started by `pi.sendMessage(..., {triggerTurn: true})` reaches its first provider request without Pi re-diffing the transcript's system sections (only `prompt()` and the next-turn refresh do). Setup and play share one session file, so the opening's first request carried `prompts/setup.md`. The kernel extension's `context_with_system` handler replaces a stale replayed prompt with `ctx.getSystemPrompt()` for that request, in Pi's own forced-prompt shape; nothing is persisted (contract §128.1). Re-check on upgrade: if Pi prepares the loadout for triggered custom messages, the handler becomes a no-op and its `lane: "prompt"` rows stop.
...
## 2026-09-14: 思考强度档位由能力快照管；修 pipicoc 先找对 pi home

「某个模型在 App 里没有某个思考强度档」有五层来源，排查顺序从显示端往回走，不要在任何一层猜：

1. **UI 的档位 = `model.thinkingLevelMap` 里值为字符串的键**（`Electron/packages/host-api` 的 `thinkingLevelsForModel`，full-map 方言：键在且值是字符串才显示，null 隐藏，缺键只有 off 兜底）。UI 不看 models.json、不看 store，只看后端 hydration 后的模型表。
2. **hydration 的顶上是 models.json 的 `modelOverrides`**（pi `provider-composer.js`：composed 模型表最后一步按 id merge override，内置目录、网络刷新目录都压得住）。某个模型死活没某档，正确修复层就是这一条。
3. **但 pipicoc App 每次启动会用打包的能力快照重写 models.json 的 override**（`Electron/apps/electron/src/main/pi-profile.ts` 的 `installBundledModelCapabilityOverrides`，来源 `Electron/resources/runtime/model-capabilities/models-dev-reasoning-options.json`，provenance 在 pi home 的 `.pipiui-model-capability-overrides-v1.json`；merge 保留用户改过的字段）。所以**系统路径是改快照，不是手改 models.json**：快照进库、随包走、换机器生效；手改 models.json 只是当场验证手段。
