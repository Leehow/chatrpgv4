...
- **The intro card** (`Electron/packages/ui/src/CocGameIntro.tsx`, `content/ui/*/intro.json`) is gone. Its one useful paragraph became `setup_opening_lead` on the extension surface (`content/ui/en/extension.json`, projected per play language, the zh-Hans seed shipped), sent as the first paragraph of the `coc-setup-opening` message, above the prologue: what this is, that what you type is what your investigator does, and that a name and a trade is enough to start.
