...
## 6.5 读者的三样工具与整张图的标准（2026-09-04 落地）

这份规格写在 agent 化之前，下面三样是实施时补的，都不在原文里。

### 整张图的验收标准

`references/module-graph-template-v1.json` + `coc_module_graph_template.py`。
三道抽取闸只判**单个分片**：契约、来源溯源、跨度覆盖。三闸全过、整本书
仍然可以是废的——「查了等于没查」我踩过两次。

模板判的是**装配后的整张图**：10 条结构不变量（场景图是否碎成多块、有没有
声明入口、线索是否支撑任何结论、NPC 是否出现在任何场景、节点有没有页码……）
加 16 个上报的度量。判据是**交代，不是内容**：一个字段要么有值，要么显式
声明为空——`_accounted` 只认显式空列表，**绝不读 coverage**（自述不是结构）。
...
`coc_evidence_query.py`：`outline` / `read --pages` / `search` / `verify` /
`coverage`。

它是**被 agent 逼出来的**：给它一个 JSON 证据包，它自己掏上下文把包重写成
`=== span-id ===` 的块再读。那不是模型跑偏，那是工具缺口——它需要的是能翻的
书，不是一坨 JSON。给读者工具，别给它原始 JSON。

### 预读一步

`coc_module_read_ahead.py` 的 `warm_targets`：玩家所在处向外扩一层。
地点型额外预热一个**连通地点**（`adjacent-to` / `route-to` / `located-in`
三种边，双向都跟）。优先级 100 当前地点 / 90 开场 / 80 邻接。
