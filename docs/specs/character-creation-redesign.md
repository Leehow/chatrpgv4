...
## 附录 A2：回放实测（2026-09-17，分支实现后）

同样三段真实输入，用 `tests/play/driver.py` 的 setup 模式回放（starter 鬼屋，zh-Hans，引导包开着）。

| 桌 | 模型 | 回合 | 等待秒 | 调用 | 画卡/修订 | 被拒 | 结果 |
|---|---|---|---|---|---|---|---|
| 韩渡（最终构建） | grok-4.6 | 1 / 2 / 3 | 17 / 52 / 7 | 1 / 5 / 2 | 0 / 1 / 0 | 0 / 0 / 0 | 第 2 回合一次画卡；「确认」一次落地，ready_for_table |
| 龙薇档案 | grok-4.6 | 1 / 2 / 3 / 4 | 63 / 25 / 55 / 35 | 6 / 3 / 2 / 2 | 1 / 3 / 2 / 0 | 2 / 1 / 1 / 0 | 档案一回合成卡（DEX 70、APP 60）；改外貌装备不动数字；「敏捷90…闪避85」一次放宽上限落到 DEX 90 / APP 85 / 闪避 85；确认落地 |
| 戴维·奥尔 | grok-4.6 | 1 / 2 / 3 | 37 / 134 / 9 | 8 / 2 / 2 | 0 / 1 / 0 | 5 / 0 / 0 | 一次画卡（Engineer）；确认落地 |
| 龙薇档案 | deepseek-v4-flash | 1 / 2 / 3 / 4 | 27 / 41 / 47 / 7 | 2 / 2 / 5 / 2 | 0 / 1 / 5 / 0 | 0 / 0 / 3 / 0 | 一句问职业，一次画卡；改外貌那回合 3 次拒绝（顶层 personal_description、单个背景分类、闪避超上限）后落到 DEX 90 / APP 90；确认落地 |

被拒的三类都已在最终构建里消掉：grok 的 `note` 空调用是工具 schema 只声明了 `step` 和 `params`，provider 把未声明键填成布尔和 null（韩渡第一回合 3 次、戴维 5 次），所有参数显式声明后韩渡第一回合 1 次调用 0 拒绝；单个背景分类现在深合并；顶层背景分类现在折进 backstory。超上限的拒绝是设计内的（带 unlock，模型自己放宽）。

对照目标：每回合画卡/修订 ≤1 ✓（超过 1 的都是模型自己接着改数字）；形状类拒绝 0 ✓（最终构建）；确认 1 次且成功 ✓（四桌全部）；手改的数没有被改回的情形 ✓（钉子在四桌所有 revision 里都在）；龙薇首张卡 APP / DEX：两模型都是 60 / 70（目标 ≥70 / ≥70，APP 差 10：两个模型都把「格斗、下肢力量」读成力量第一、敏捷第二、容貌第三，这是模型对档案的读法，玩家一句话就钉到 85–90）；一回合等待 deepseek 7–47 秒 ✓、grok 7–63 秒（戴维第 2 回合 134 秒是 grok 单次生成慢）。
