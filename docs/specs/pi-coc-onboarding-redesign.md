...
1. **引导会话用什么模型。** 它主要做确定性编排，只有源审阅与建卡需要 LLM。
   可以用比 KP 便宜的模型。
2. **建卡留在引导还是进 KP。** 留在引导 = 交接边界更干净；进 KP = 玩家在同一
   个对话里从建卡直接进游戏，体验更连贯。我倾向留在引导，但这是产品手感问题。
3. **旧路径何时删。** Stage 4 是设计的一部分；如果要留回退期，需要明确回退期
   的截止条件，否则就是永久双写。
