...
- **Numbers.** Parent `172b80065`: `test:ext` 2715 / 2713 pass / 2 fail under load (`lanes.test.mjs` #67 header timeout, `npc-preparation-integration`; both 38/38 on rerun); pytest 1645 passed, 1 skipped. After the SL-08 merge: `test:ext` 2744/2744 (0.9.5a baseline 2728 + 16). After the RD-03 merge (`d8efc68cf`, final state `d3e728d58`): `test:ext` 2746 run, 2741 pass, 5 fail under load average 140 (`jev-s0-rpc-lifecycle`, `jev-source-domain`, `lanes` #67, `npc-preparation-integration`), those four files 46/47 then 8/8 twice alone, no failure in a file RD-05 touches (0.9.5a baseline 2730 + 16 = 2746); pytest `tests/kernel tests/play` 1691 passed, 1 skipped (baseline 1688 + 3). Mutation table rerun on `d3e728d58`: 16/16 killed.
