...
_Date: 2026-09-12. Baseline: 0.9.2a at 825f4719d72b16702b28e5a1e5e60404a9f109fa. Status: implementation present; acceptance partial. Authoritative execution plan: [../plans/story-continuity-and-adaptation.md](../plans/story-continuity-and-adaptation.md); planned continuity-review wire direction: `docs/kernel-rpc.md` §35.13._

**Current decision note (planning).** The goal has been clarified by the user and is authoritative (§ Testing Decisions and the plan). Recalling already-delivered history must be faithful; adding a NEW campaign detail may be invented when coherent with settled state and is not a failure merely for lacking a literal module quote; an NPC assertion or player hypothesis retains attribution; kernel-authoritative action/resource state is never invented or reversed. The old rubric that treated a missing source quote as sufficient grounds to fail a turn is superseded in interpretation, though all raw evidence is retained unchanged. The source-consistency audit below (capability `audit.source.v1`, `narration-audit` 1.1.2) is the implemented legacy check; a bounded continuity-review replacement is PLANNED, not implemented, and is specified in `docs/kernel-rpc.md` §35.13. Missing source proof alone is not the new acceptance oracle. Implementation and acceptance of the replacement remain pending; writing this note changes no live behavior.
