...
**分叉不回退工作状态（2026-09-02 实测，当日修复，见下）：** `fork_timeline` 只做
`update-ref` + 写 timeline-state，从不把战役目录的 `save/*.json` 检出回分叉点。
git 拓扑是对的——新分支第一个提交（turn 0052）的父提交确实是 turn 0035——但
那个提交的**内容**是 turn 0051 的状态：`npc-sarah-browne.availability` 已是
`unavailable`，impression 写着「火刑柱上的火焰已吞没她」。

**已修复：** 激活一条世界线现在会把工作区物化成那条线的内容——分叉激活到分叉点，
重新激活与 `set_active_timeline` 到该线自己的 tip；只登记不激活仍然不动工作区。
`set_active_timeline` 是更根本的那一半：它原来只写指针，所以任何切线都不带状态。
confluence 早就在激活时同步，其助手的文档字符串写明了理由（否则下一个结算回合会把
残留内容拍进快照）；fork 与活动指针从来没调过它。

后果不是记账瑕疵，是这个功能的核心承诺没有兑现。玩家显式要的是「回到火刑之前，
莎拉还能被救」，而机械上她从新世界线的第一秒起就是死的。随后十余个真实回合都在
这个矛盾上跑：KP 反复叙述「柱上已空、焦架冒烟」，看着像它在编造，其实它读到的是
真实的 canonical 状态——投影没有骗它，是分叉没有兑现。

修它要先答一个语义问题，所以这里只记录不动手：分叉后的工作状态应当**回退到分叉
点**（真正的重来一次），还是**保留当前状态**（只换一条记账线）？前者是玩家和模组
循环的语义，也是 `resets-to` / `persists-across-loop` 这两种关系存在的理由——那两
条关系恰恰要求「哪些回退、哪些跨循环保留」是逐条声明的，而不是全有或全无。

**另一个留给下一个动世界线的人的坑：** 存在两条 fork 路径。活的那条是
