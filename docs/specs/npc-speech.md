...
_Date: 2026-09-15. Status: **landed on 0.9.3a** (contract §40, §34.16); the 2026-09-15 repackage failed on the Xcode licence gate (clang), so the App stayed on the morning build until the next packaging. Superseded in part by `docs/specs/npc-voice-mask.md` (2026-09-16): `sample_lines` gave way to `voice_mask` + `exchanges` and the capsule's `voices` (§40.7). The branch `claude/npc-speech-20260915` carries the say token, speaker colour, the npc-voice package 1.0.1 and its lane, the host speech steer, the runaway cut, the quotation-mark fix from the A/B/C, and `driver.py start --pregen`. Suites at the head: `npm run test:ext` 1342/1342, kernel pytest 1246 passed with the three failures that fail on the 0.9.3a base commit too (`test_continuation`, `test_facts_warn`, `test_fast_guidance`), `tests/play` 57 passed. Real tables: the twenty-one-turn grok acceptance (`speech-0915`), the twenty-turn deepseek measurement (`speech-0915-ds`), the three-arm A/B/C (`ab-say-{a,b,c}`), and a final five-turn table on the packaged build from a template sheet (`final-0915`: 240–349 characters per turn, every line quoted and wrapped, no runaway). Still open: the second-model wrap rate depends on the host steer now and was not re-measured after it; the colours were never seen as pixels (screen access declined); example exchanges instead of two lines is the next lever (below). User rulings 2026-09-15 under "Rulings"._
...
**Choppy speech (2026-09-15, user report on both models).** Measured: clause length inside 「」 is 8–11 characters on every grok table, base or branch, so the telegraphic rhythm predates this section; but the only instructions in the stack that ask for fragments were this section's ("half sentences, leave the end off" in `npc-voice` 1.0.0 and the lane instruction), and they made a habit into a rule. A web research pass (screenwriting craft: McKee, Sorkin, Pixar; Chinese spoken-dialogue craft with 老舍《茶馆》 as the reference; SillyTavern / Character.AI creator guidance; LLM findings that negative and "be brief" constraints produce mechanical output) gave the replacement, now in `npc-voice` 1.0.1 and the lane instruction: whole connected spoken sentences with connectives, end-particles and address terms; long and short alternating; a fragment is one beat at most; every line wants something and may answer a different question; a person hiding something talks more, not less; one recurring habit per person instead of adjectives. Not yet adopted, recorded as the next lever: replace the two sample lines with three or four example exchanges (question → answer under different pressures), which every roleplay product surveyed found teaches voice better than lines in isolation; it changes `shape: "lines"` and is its own section.
