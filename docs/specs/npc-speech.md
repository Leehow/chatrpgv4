...
- **Trigger.** On `coc:turn-committed`: every person in the committed turn's `present[]`, and every resolved speaker in its `speech[]`, whose dossier has no `sample_lines` (authored or established). On bind of a campaign: a low-priority backfill over the graph's named NPCs behind `PI_COC_NPCVOICE_BACKFILL`, so a table that opens on a crowded scene is covered by its second turn. Graph-only NPCs the table never meets are covered only by backfill; nothing here calls a model for a person the player will never hear.
