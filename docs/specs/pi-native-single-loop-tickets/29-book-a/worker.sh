...
# Usage: worker.sh <action> <input-json>. Events (JSONL, timestamped) append to $E/<action>.events.jsonl.
set -e
W=/Users/haoli/leehow/code/chatrpgv4-wt-pdf-a
E=$W/.coc/playtests/sl29-a-import
cd $W
...
INPUT="$(node -e 'const i=JSON.parse(process.argv[1]); i.home=process.argv[2]; process.stdout.write(JSON.stringify(i))' "$2" "$W")"
