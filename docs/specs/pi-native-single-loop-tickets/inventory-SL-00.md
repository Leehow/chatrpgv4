...
| `extensions/kernel/admission.ts` | `typedAttempt` | 497 | `createDecisionAdapter` | kernel ext: action admission (contract 32.10, 32.11) | app-play | not observed (reviewer: lane) | SL-10 (32.11): every bookkeeping apply batch (move/clue/handout/time) is typed first by default (`PI_COC_ADMISSION_FAST_MIN_CONFIDENCE`, `off` disables); other reviews only with PI_COC_ADMISSION_REVIEWER=jev. Falls back to the lane on every non-verdict. Moved from `reviewAdmissionPrimary` at SL-10 (it was app-play-gated in the SL-00 counts above). |
