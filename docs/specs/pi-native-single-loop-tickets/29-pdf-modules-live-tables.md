...
### 2026-09-25 — book A (血色公路) SL-29A batch-11 on `a9d8f9619` (integration `claude/integ-single-loop-20260923`, batch 10 plus SL-65/66/67 merged on top: the reading lease's input-token floor scales with the reader's context window; a selected name stops at the word; SL-62's name-resolution candidates include the campaign's whole roster): **SL-65 confirmed fixed, live** — zero `budget_input_tokens` refusals, `stage_budget` rows carry the new 8,000,000 floor directly (up from batch-10's fixed 4,000,000); **SL-66 confirmed fixed for exactly what it fixed** (zero leading/trailing punctuation on the stored name) but this table's own live setup exposed a narrower, unfixed edge in the same subsystem — a non-punctuation Han character from the sentence's own scaffolding rode into the name instead; **SL-67 not cleanly re-exercised in its pre-registered shape** (this table's Keeper never attempted a shortened name of a person established elsewhere), though a structurally similar variant-phrasing resolution succeeded for a table-invented identifier; one new P2 finding (a batch introducing three simultaneously-new npc names, with an established roster already non-empty, refused whole-batch citing the same stale-roster candidate shape SL-64 was meant to eliminate) and one new P1 wall-time outlier (a single provider call ran 187 s with no error, driving one turn to 225 s)

Same Keeper as batches 9-10: `opencode-go/deepseek-v4.1-flash`, `--thinking off`. Same book, same reused imported home, same 20-turn script, same investigator identity and backstory (Private Investigator, Drive Auto strong, tracking/investigation his trade, weak with a gun). Only the build changed: batch 10 plus SL-65/66/67.

New worktree `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11` (branch `claude/sl29a-b11-20260925`) at `a9d8f9619`, six node_modules symlinks from `chatrpgv4-wt-pi-coc-v2` per the marker, built via `leehow-pc-tests`' `remote-test.sh build-fetch` (14s remote build, exit 0, clean). No confound: `ps aux` showed no other `driver.py`/opencode-go process running on this Mac at table-open time. `.pi/coc-agent/{auth.json,pipiui-settings.json,grok-build-models.json,models-store.json}` copied from `chatrpgv4-wt-integ-sl`; `models.json` deliberately **not** copied. The imported home was reused: `chatrpgv4-wt-pdf-a/.coc/playtests/sl29-a-run2/home` copied into `.coc/playtests/sl29a-b11/home`; 37 stale `.lock` files found across the same categories every prior batch found (`.coc/locks`, `.coc/modules/{.registry.lock,book-1/*.lock}`, `.coc/campaigns/*/setup.lock`, `.coc/mods/jobs/*/review.lock`, `.coc/module-campaigns/*/modules/{.seed-book-1.lock,book-1/*.lock}`) were removed before use. A NEW campaign, `sl29ab11-xuese-2205`, was created via the onboarding worker's `converse` action (title `血色公路`, no `.pdf` suffix; `module_id: book-1`, `start_scene: scene-prologue`, `guidance_key` read from the copied module's own `module.json`, confirmed byte-identical to batch-10's own values). Harness adapted from `29-book-a-b10/` as `29-book-a-b11/{worker.sh,start.sh,play.py,triage.py,script.md}` (script unmodified), with `29-book-a-b11/preregistration.md` written and committed (`94a7a7767`) before the table opened. **This batch's git work stayed entirely inside its own worktree/branch** (`claude/sl29a-b11-20260925`), unlike batches 9-10 which committed their pre-registration and this ticket's own entry directly onto the integration branch in `chatrpgv4-wt-integ-sl` — this worker's own marker was explicit ("Work ONLY in your worktree: never run git in any other checkout"), so this entry is committed on `claude/sl29a-b11-20260925` and will need a maintainer merge/cherry-pick onto the integration branch to join the ticket's continuous history the way batches 4-10 did in place. `triage.py` extended with three new sections (SL-65: `stage_budget`/`contextWindow` rows and refusal count from `reading-telemetry.jsonl`; SL-66: the investigator card's stored `name` checked against a plain punctuation/space set, plus every `narrate` sample scanned for the name; SL-67: every person-name event cross-referenced against the nearest prior `table.look{focus:"scene"}`'s `present`/`roster` fields).

**Pre-table confirmation (SL-61).** `content/providers/model-corrections.json` present with `opencode-go/deepseek-v4.1-flash` and `deepseek-v4-flash` both `thinkingLevelMap.off: "off"`. After the setup daemon started, its `daemon.json` `model_confirmed.thinkingLevelMap.off` read `"off"`; this worktree's own `.pi/coc-agent/models.json` — absent before the table, never copied — was written by the product at host preparation carrying the `//`-comment provenance note naming the two corrected models; the play daemon's own `daemon.json` confirmed the same `"off"` again before turn 1.

**Setup (live, `driver.py --launcher bin/pi-coc-setup`, run `sl29ab11-xuese-2205-setup-20260925T220613Z`, replaying batch-10's own recorded 5-turn exchange turn-for-turn, read from batch-10's `turn-{1..5}.json` sentences — including turn 4's own sentence with the trailing comma immediately after the stated name, the exact SL-66 test case).** 5 turns, 61.0s total, 9 tool calls. Investigator rendered **雷·卡特** in the Keeper's own prose throughout. Private Investigator, Drive Auto strong (DEX aptitude). **New finding (P3, see below): the confirmed card's `name` field is `"叫雷·卡特"` — SL-66's punctuation trim held (no comma), but the selected range's leading edge swallowed the adjacent verb "叫" ("named/called") instead.**

**Table (`29-book-a-b11/play.py`, driver run `sl29ab11-xuese-2205-20260925T220900Z`, hybrid-v1, `PI_COC_JEV_PRESELECT=1`, `opencode-go/deepseek-v4.1-flash` thinking off).** 20/20 turns settled, 0 stranded, 91 Keeper provider calls (0 errors; 89 across turns 1-20, 2 more from the pre-turn-1 opening), reasoning 0 on all 91 calls (`events.jsonl`: 423×`"reasoning": 0"`, 0 nonzero, 91×`"reasoning_effort": null"`), admission max 13,003 ms (one row, t20, `review_pending` at its own `cap_ms: 13000` — the same marginal-miss shape as batches 8-10), `infer(bind)` = 0, 22 non-source `look`/`lookup` calls (lighter than batch-10's 27), 0 `lookup kind=source` calls this table (unlike batch-10's 3 — this Keeper never explicitly consulted the source text directly, though the background reading pipeline still ran from queued jobs), speech 18 resolved / 7 unresolved, 0 sanity rolls (script does not reach one). Walls `[17, 20, 25, 39, 39, 64, 64, 31, 34, 43, 64, 54, 47, 62, 37, 55, 225, 28, 62, 87]`; median 45.4s, 13/20 ≤ 60s (65%), max 225.0s (t17, the same turn as the wall-time outlier finding below). Model-call ms p50 4836.0, p90 8404.0, **one call over 45s: 187,218 ms** (t17). `npc-ledger.json` entries: **5** (batch-10 had 4; 4 minted together in one batch call at t7 when the roster was still empty, a 5th singly at t20). Reading: 5 jobs raised (`read-1`..`read-5`); `read-1` (index) and `read-2` (detail) both **failed**, but neither on `budget_input_tokens` (`read-1`: `reader_transport`, "the provider failed the round (Request timed out.)"; `read-2`: `runtime_closed`, "the reader reported nothing for 600s and was stopped") — pre-existing, unrelated failure modes, not SL-65's target; `read-3` and `read-4` (both `answer`) **completed** with real book-sourced content (naming 拉斯·威廉姆斯/内特·帕特森/史蒂夫·布朗 at the gas station, the Esso station and Main Street's shop list) — this Keeper's own conversation never asked about these named persons, so the successfully-read material went unused this table; `read-5` was still `running` when the table ended. Fork stayed at generation 2 (no directory advance), same as batch-10.

**Table (Scope 3), keyed to what SL-65/66/67 changed.**

| class | pre-registered line | measured | verdict |
| --- | --- | --- | --- |
| delivery | 20/20 with prose | 20/20 settled, 0 stranded, every turn delivered real Keeper prose | **pass** |
| wall | median ≤45s, ≥80% ≤60s | median 45.4s, 13/20 ≤60s (65%), max 225.0s driven by one 187s provider call (no reading wait) | **miss — a new, severe outlier not seen in batches 9-10 (max there was 88.1s); see Finding 2** |
| routing | declared move lands or narrates honestly | one `apply` move raised all table: t4 `welcome-to-abattoir` (lands on first declaration). No sub-location entered as its own scene this table either | **not a defect on its own — same Keeper-choice/pacing noise as every prior batch** |
| admission | no row >12s | admission max 13.0s (t20, `review_pending`, `cap_ms: 13000`), 0 `review_timeout` | **marginal miss (1.0s over the line) — same shape as batches 8-10's own single capped row** |
| binding | `infer(bind)` = 0 | 0 | pass |
| looks | ≤1/turn after first visit | 22 non-source `look`/`lookup` across 20 turns; 0 `lookup kind=source` this table | **fails the per-turn line on several turns (Keeper-style, not a product mechanism) — same as every prior batch** |
| prescreen | status per read; own allowance | `prepared` every read, 2-7 candidates as the graph's local roster grew, no fallback anywhere | pass |
| drops | every drop has a reason | `speech_steer` and `text_beside_tool_calls` rows seen, each with a reason; no stranded turn | pass |
| fiction/rules | no off-sheet skill; no push without declaration | all rolls (Spot Hidden, Appearance, Persuade, Psychology, Charm, Drive Auto) on 雷·卡特's own sheet; established persons come from the Keeper's own descriptions, not invented stat blocks | pass |
| stalls | none; provider errors listed | 0 provider `error` rows anywhere | pass |
| reading (PDF) | every read has a telemetry row and outcome | 5 reads (`read-1`..`read-5`); 2 failed (transport timeout, runtime-closed — neither `budget_input_tokens`), 2 completed with real book content, 1 still running at table end; 0 `reading_timeout` in any foreground turn | pass on the class; the two non-budget failures are noted, not scored (pre-existing failure modes, out of this batch's scope) |
| **SL-65 (this batch's primary target)** | **zero `budget_input_tokens` refusals** | **0 refusals across all 5 jobs; every `stage_budget` row (`read-1`..`read-5`) carries `inputTokens: 8000000` directly — the exact `max(4,000,000, 8×contextWindow)` result for a 1,000,000-token reader, double batch-10's fixed 4,000,000 floor** | **confirmed fixed, live — clean, definitive confirmation with the exact new floor value observed in the telemetry itself** |
| **SL-66 (this batch's primary target)** | **no stray leading/trailing punctuation on the stored name** | **`party/investigator.json` name: `"叫雷·卡特"` — zero leading/trailing punctuation or whitespace (the punctuation-trim mechanism itself worked: the selected range `input:4:3/u:29`-`u:33` never included the sentence's trailing comma at `u:34` in the first place)** | **confirmed fixed for exactly what SL-66 targets; a different, unfixed defect in the same subsystem surfaced instead — see Finding 3** |
| **SL-67 (this batch's primary target)** | **a shortened name of a person established elsewhere resolves** | **not attempted by this table's Keeper — every established person here was Keeper-invented via a purely descriptive label (never a book proper name), so no established-but-not-present proper-name-shortening scenario arose naturally.** A structurally similar case did occur: `油站的瘦长个子` (t15, dropping `加` from `加油站的瘦长个子`) resolved to the same established person as its longer form | **not cleanly re-exercised in the pre-registered shape; the one analogous variant-phrasing case observed resolved without incident — no regression, but not a repeat of batch-10's own failing case either** |

**Findings, one root cause each**

- **SL-65, confirmed fixed live — the clean confirmation the ticket asked for.** Every `stage_budget` row this table (`read-1` through `read-5`) carries `"inputTokens": 8000000` directly in `reading-telemetry.jsonl`, replacing batch-10's fixed `4000000` with the new `max(4,000,000, 8×contextWindow)` formula's output for this reader's 1,000,000-token context window. Zero `budget_input_tokens` refusals occurred anywhere this table, versus batch-10's 5 across 3 jobs on the identical book/fork lineage. The two reads that did fail this table (`read-1` transport timeout, `read-2` a 600s-silent reader stopped by the runtime) are unrelated failure modes SL-65 was never meant to address — recorded for completeness, not scored against this ticket. Evidence: `.coc/playtests/sl29a-b11/home/.coc/reading-telemetry.jsonl` (5×`event: "stage_budget"` rows, each `inputTokens: 8000000`; the two `state: "failed"` entries in `deepen-queue.json` carry `refusal.reason` values of `"transport"` and `"runtime_closed"`, never `"budget_input_tokens"`).
- **Finding 2 (new, P1, wall time) — a single provider call ran 187,218 ms with no error, retry, or reading wait, driving turn 17 to 225.0 s.** `stop_reason: "toolUse"` on the call itself (not an error, not a timeout abort) — the model simply took over three minutes to return a response, versus this same table's own p50 of 4.8s / p90 of 8.4s and versus every other call in the table (the next-slowest was under 11s). No reading job was in flight for this turn (`reads [] reading_timeouts 0` in the triage), so this is not a reading-pipeline wait; it coincides with (but is not proven to cause, or be caused by) the same turn's whole-batch npc refusal in Finding 4 below. Not investigated further — measurement only, and this looks like raw provider-side latency variance rather than a product defect, but it is a new severe outlier (batches 9-10's own maxima were 71.4s and 88.1s respectively) worth flagging for anyone tracking this Keeper's tail latency. Evidence: `.coc/playtests/sl29ab11-xuese-2205-20260925T220900Z/turn-17.json`; campaign `telemetry.jsonl` (the `lane: "provider-call"` row for turn 17 with `ms: 187218`, `stop_reason: "toolUse"`).
- **Finding 3 (new, P3, cosmetic but systemic — SL-66-adjacent, not SL-66 itself) — the selected name range's leading edge swallowed an adjacent, non-punctuation Han character.** Setup turn 4's sentence was "别人常找我跟丢了的人的线索，追踪失踪人口是我的老本行。**名字叫雷·卡特，**现在就做卡吧。" Counting grapheme units from 0: unit 29 is `叫`, 30 is `雷`, 33 is `特` (the name's own last character), 34 is `，`. This table's `create-investigator` call selected `range: {first: "input:4:3/u:29", last: "input:4:3/u:33"}` — one grapheme earlier on **both** ends than batch-10's own `u:30`-`u:34` for the identical sentence. The `last` boundary correctly stopped before the trailing comma this time (SL-66 doing its job, or possibly the model simply picked a different `last` boundary independent of the fix — the range never reached the comma either way), but the `first` boundary included `叫` (a verb meaning "named/called", part of the sentence's own grammar, not the name and not punctuation/whitespace). SL-66's trim (Unicode category P/Z only) correctly leaves `叫` untouched, because it genuinely is not punctuation or whitespace — this is a boundary-selection defect one level upstream of what SL-66 fixed, not a regression of SL-66 itself. The confirmed card's `name` field is `"叫雷·卡特"` (verified in `party/investigator.json`), and it echoes into every mechanical reference to the investigator for the rest of the table (`look{focus:"investigator", name:"叫雷·卡特"}` at t14/t19, `apply` effects' `to: "叫雷·卡特"` fields throughout). Unlike batch-10's comma bug, it did **not** leak into player-facing `narrate` prose this table (0 of 12 `narrate` samples contain the literal string `叫雷·卡特` — the Keeper's own rendered text consistently said `雷·卡特`), so the player-visible harm is smaller this time, but the mechanical/world-state harm is the same shape. Evidence: `.coc/playtests/sl29ab11-xuese-2205-setup-20260925T220613Z/turn-4.json` (the `create-investigator` tool call's `args.profile.name.range`); `.coc/playtests/sl29a-b11/home/.coc/campaigns/sl29ab11-xuese-2205/party/investigator.json` (`name: "叫雷·卡特"`); `.coc/playtests/sl29ab11-xuese-2205-20260925T220900Z/turn-{14,19}.json` (`tools[].args`/`result_text` carrying the literal string).
- **Finding 4 (new, P2, SL-64-adjacent) — a single `apply` batch introducing three simultaneously brand-new npc names, with an established roster already non-empty, refused whole-batch citing a stale-roster-shaped candidate list.** Turn 17's `apply {effects: [{kind:"npc", name:"柜台前的老男人"}, {kind:"npc", name:"穿工装的年轻人"}, {kind:"npc", name:"饭馆柜台后面的女人"}, ...]}` — all three genuinely new (book-absent and table-absent) names — refused entirely (0 receipts) on `unknown_entity: no npc named '饭馆柜台后面的女人' in the module graph`, with `candidates: [柜台前的老男人, 加油站的瘦长个子, 加油站桌边的矮胖男人, 加油站旁两把折叠椅上的两个男人, 马瑟综合商店门口擦门框的女人, 穿工装的年轻人]` — the campaign's 4 pre-existing established table persons (minted at t7) **plus the batch's own first two sibling effect names** (`柜台前的老男人`, `穿工装的年轻人`), which are not established anywhere and appear only as this same call's own earlier effects. This is precisely the batch-9/pre-SL-64 defect shape (a non-empty candidate list read as "refuse, offer these" instead of "nothing matches, mint"), but it is **not** the shape SL-64's own fix (`candidates(name, kinds, limit, {roster:false})` for the minting gate) was validated against in batch-10 — that table's own confirmation was a *single* new name introduced with the established roster already non-empty, and it minted cleanly. This table's own single-new-name introductions worked the same way every time (t9, t14, t15, t18, t20 each minted or resolved cleanly). The defect here is specific to **multiple brand-new names introduced together in one `apply` call**: the first two sibling names appear to land (or at least register) before the third is checked, and the third effect's `candidates()` call picks up not just the graph's established roster but also those two just-processed sibling names — despite the whole call ultimately reporting 0 receipts, meaning nothing actually committed. Whether the first two effects' names get evaluated with the `{roster:false}` exemption individually and only the third does not, or whether sibling-batch names get folded into a separate candidate pool that never received SL-64's exemption, was not investigated — measurement only, this batch. The turn still delivered (the Keeper narrated the scene descriptively without the mechanical registrations landing — `drops: ['text_beside_tool_calls', 'speech_steer']`), so this is not a stranded turn, but it is an unresolved edge of SL-64's own stated success line ("every distinct table person the Keeper introduces mints"). Evidence: `.coc/playtests/sl29ab11-xuese-2205-20260925T220900Z/turn-17.json` (the second `apply` call, `args.effects` and `result_text` verbatim); `.coc/playtests/sl29a-b11/home/.coc/campaigns/sl29ab11-xuese-2205/world.json` (`table_people` — only 4 entries at this point in the table, all from t7, confirming none of t17's three names landed).
- **SL-67, not cleanly re-exercised in its pre-registered shape.** This table's Keeper never introduced a book proper name at all (unlike batches 9-10, which named 拉斯·威廉姆斯/内特·帕特森/史蒂夫·布朗 at t1) — every established person here is a Keeper-invented descriptive label (`加油站的瘦长个子`, `马瑟综合商店门口擦门框的女人`, etc.), even though `read-3`/`read-4` (both completed successfully this table, unlike batch-10) returned the book's actual named gas-station attendants. The Keeper simply never asked about them by name, so the exact established-but-not-present proper-name-shortening scenario SL-67 targets did not arise. A structurally analogous case did occur and succeeded: `油站的瘦长个子` (t15, a shortened variant dropping `加` from the established `加油站的瘦长个子`) resolved to the same person without incident. `table.look{focus:"scene"}` was only called twice this table (t4, t6), both times before any npc was established (`present: [] roster: []`), so the `roster` field itself was never observed populated — the SL-67 mechanism's own visible signal was not exercised, only its downstream effect (name variant resolution) was, and only for a descriptive label, not a proper name. No regression observed. Evidence: `.coc/playtests/sl29ab11-xuese-2205-20260925T220900Z/turn-{7,8,9,15}.json` (the `加油站那个瘦长个子`/`油站的瘦长个子` naming variants and their outcomes); `.coc/playtests/sl29a-b11/home/.coc/module-campaigns/sl29ab11-xuese-2205/modules/book-1/deepen-queue.json` (`read-3`/`read-4`'s `result.source_answer.answer`, naming 拉斯·威廉姆斯/内特·帕特森/史蒂夫·布朗 verbatim, never referenced by this table's own conversation).
- **SL-64, confirmed still held for its own tested shape (sequential/single-name introduction).** `npc-ledger.json`: 5 entries (4 minted together in one batch call at t7 when the established roster was still empty, a 5th singly at t20 against a 4-person roster). Every single-new-name introduction this table minted or resolved cleanly regardless of roster size — the one failure (Finding 4) is specific to multiple simultaneous new names, a narrower case than batch-10's own confirmation covered. Evidence: `.coc/playtests/sl29a-b11/home/.coc/campaigns/sl29ab11-xuese-2205/npc-ledger.json` (5 top-level handles); `world.json` (`table_people`, 5 entries with `established_at` timestamps at t7 and t20).
- **SL-61, confirmed still fixed live.** 91 provider calls, 0 with nonzero reasoning; `events.jsonl` carries 423×`"reasoning": 0"` and 0 nonzero, 91×`"reasoning_effort": null"`; p50 4836ms, p90 8404ms (the 187,218ms outlier in Finding 2 aside). Evidence: `.coc/playtests/sl29ab11-xuese-2205-{setup-20260925T220613Z,20260925T220900Z}/{events.jsonl,daemon.json}` (both `model_confirmed.thinkingLevelMap.off: "off"`); `.pi/coc-agent/models.json` (product-written, present only after the setup daemon started).
- **SL-63, confirmed still fixed live.** One `refusal_budget` trip this table (t19: a repeated `invalid_params: Adopt needs an existing investigator equipment name...` loop over equipment definitions, unrelated to this batch's own tickets — Keeper tooling confusion, not a resolution/minting issue), the host blocked the fourth identical attempt ("apply has been refused 3 times this turn for the same reason... Stop trying it: close the turn with narrate"), and the turn still delivered via `narrate`. Evidence: `.coc/playtests/sl29ab11-xuese-2205-20260925T220900Z/turn-19.json` (the sequence of `apply` refusals and the final `narrate`); campaign `telemetry.jsonl` (`reason: "refusal_budget"` row, turn 19).
- **SL-58, not exercised.** Zero `source_mode: "prepare"` lookups occurred this table (versus 2 of 3 executing cleanly in batch-10) — a different Keeper conversation pattern, not a regression; no claim either way. Evidence: absence, confirmed by direct scan of every `turn-*.json`'s `tools[]` this table.
- **SL-60, not exercised.** 0 displacements, 0 resumes (lighter/different reading load than any prior batch: 2 of 5 jobs failed outright rather than contending for slots). Evidence: `.coc/playtests/sl29a-b11/home/.coc/reading-telemetry.jsonl` (0 `event: "displaced"` or `"resumed"` rows).
- **Minor, unscored — a documentation gap, not a functional defect.** SL-67's own ticket Comments claim "Full text: docs/kernel-rpc.md §11.5.8", and `extensions/kernel/index.ts`'s own code comments reference "§11.5.8" by name (`scenePersonCandidates`'s doc comment), but no `#### 11.5.8` heading or body text exists anywhere in `docs/kernel-rpc.md` in this worktree (only `#### 11.5.6` SL-62 and `#### 11.5.7` SL-64 are present; confirmed by direct `grep` — zero matches for `11.5.8`). The code itself is live and working (confirmed by this table's roster-merge behavior and by SL-67's own kernel-level test per its Comments), so this is purely a missed contract-doc edit, not a product regression — flagged per the project's own "contract §编号 is a stable identifier" rule, since a future worker searching the doc for §11.5.8 will find nothing. Evidence: `docs/kernel-rpc.md` (no `11.5.8` match); `extensions/kernel/index.ts` lines ~3309-3316 (the doc comment naming "§11.5.8's `roster`").
- **Observations, not scored (model noise / recurring unexplored pattern).** The Keeper hallucinated a misspelled tool call `narrite` at t14 (`{}` args, host correctly replied `Tool narrite not found`, immediately self-corrected with a proper `narrate` call) — model behavior, not a product defect. The `"The host did not execute this call."` skip pattern (first seen in batch-10, not investigated there either) recurred several times at t19 during the equipment-adoption confusion — still not investigated, still out of scope for a measurement-only batch.

**Comparison with batch-10's own findings.**
- **Fixed:** SL-65 (reading-lease floor) — clean, definitive, the exact new 8,000,000 value observed directly in telemetry. SL-66's own stated mechanism (punctuation/whitespace trim) — zero punctuation on the stored name, confirmed.
- **Held:** SL-58/59/61/64 all continue to work for the shapes batch-10 confirmed them against (SL-59: every multi-effect batch this table where names didn't collide with the stale-roster bug landed cleanly, e.g. t7's 4-for-4; SL-64: sequential/single-name minting against a non-empty roster, 5 ledger entries).
- **New:** Finding 2 (a 187s single-provider-call wall-time outlier, the most severe seen across 11 batches) and Finding 4 (a batch of 3+ simultaneously brand-new npc names still hits the stale-roster candidate-list bug SL-64 was meant to eliminate for the single-name case) — both unrelated to each other, both on this table's own turn 17. Finding 3 (a different name-selection defect in the same subsystem SL-66 fixed, one grapheme upstream of what SL-66 trims) is new in root cause though the visible symptom (a malformed investigator name) rhymes with batch-10's own Finding 3.
- **Not cleanly retested:** SL-67's own exact target shape (a shortened name of a person established elsewhere) — this table's Keeper never attempted the scenario, so neither confirmation nor regression can be claimed; the one structurally similar case (a descriptive label's shortened variant) resolved without incident.
- **Newly noticed, not a defect:** the §11.5.8 documentation gap.

Evidence (git-ignored, kept per the marker):
- setup: `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.coc/playtests/sl29ab11-xuese-2205-setup-20260925T220613Z/`
- table: playtest `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.coc/playtests/sl29ab11-xuese-2205-20260925T220900Z/` (turn-{1..20}.json, driver.log, events.jsonl, daemon.json, heartbeat.json)
- campaign: `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.coc/playtests/sl29a-b11/home/.coc/campaigns/sl29ab11-xuese-2205/` (turns/0000-0020.json, telemetry.jsonl, campaign.json, npc-ledger.json, world.json, party/investigator.json)
- campaign fork (kept, not deleted): `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.coc/playtests/sl29a-b11/home/.coc/module-campaigns/sl29ab11-xuese-2205/modules/book-1/` (`deepen-queue.json` read-1..5; `generations/generation-2-8bf9c59356844d25a0f9dd32704b35eb`)
- reading telemetry (stage_budget/refusal rows): `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.coc/playtests/sl29a-b11/home/.coc/reading-telemetry.jsonl`
- agent home model confirmation: `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.pi/coc-agent/models.json` (gitignored, product-written; not printed here beyond what's quoted above)
- triage: `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.coc/playtests/sl29a-b11/triage.txt`
- harness (this branch only, originals under `29-book-a-b10/` untouched): `docs/specs/pi-native-single-loop-tickets/29-book-a-b11/{worker.sh,start.sh,play.py,triage.py,script.md,preregistration.md}`
- pre-registration commit: `94a7a7767` (before the table opened, on `claude/sl29a-b11-20260925`)
- per-turn prose (not committed, carries book-derived text): `/Users/haoli/leehow/code/chatrpgv4-wt-sl29a-b11/.coc/playtests/sl29a-b11/turns.log`
