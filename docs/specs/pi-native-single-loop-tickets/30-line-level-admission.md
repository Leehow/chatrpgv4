...
**A regression found by the suites and fixed (`130a6fb0f`).** The final `ext` run on leehow-pc failed `admission-late`'s "typed_late within cap + 1 s" with `ms` 1499 against the 1500 cap. It reproduced on the Mac in 1 of 5 runs. The cause was mine: the split support measured every review's cap as `cap - (now - start)`, and a Node timer can fire a millisecond early against `Date.now()`. A review that begins at the call now waits its whole cap, as it did before SL-30. Only a remainder resumed from a split waits what is left of the batch's cap (`left()` in `reviewAdmissionPrimary`). After the fix it passed 6 of 6 runs, and M10 is still killed. The replays above ran on the tree before this fix, which changes only a top-level review's cap by at most a millisecond.
