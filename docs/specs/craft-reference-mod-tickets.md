...
状态：**用户已批准；首版及默认开启优化均已实现，当前源码为1.6.0/default jev；旧锁与显式off保留。结构化补读有有限正向信号，真人校准未跑。初版default off的收尾决定已由用户明确纠正。**。无远程 issue 编号。切片状态以本文实施记录与运行时任务列表为准，不能据规格存在推断完成。源码起点为 `0.9.5a@ef8efdf97`，实施时确认最新主线与正在运行的单循环形态。
...
### 默认开启优化与最终验证（2026-09-24）

- 用户明确目标是优化到值得默认on，而不是无净收益就关闭收工。1.6.0新锁默认jev并声明context.craft-reference.v2；v1默认off语义、旧存档显式off和冻结包保持兼容。
- 基础Keeper取消强制重放玩家整句，11条旧指令合为4条正向指导，6个通用轴适用于所有语言。完整措辞保留，brief同4 IDs用短提醒，1536字节旧预算未放宽。对白JSON和renderer没改。
- 首次采用仍核对完整binding；已issued的编辑方法在同一输入/场景/参与者内不会被普通结算或Mod运行状态自毁。现有task_source_revision覆盖真实source/config，换来源、参与者、包或有效设置仍撤回；取消永久失效，单次选择和预算不变。
- 三臂18样稿先发现提示一致化的正向信号；用户提醒后以原21稿、12新读者保留对白结构复核。coherent_on/old_on为9/3，实际带参考四例on/no-reference为5胜3平。作者未改，不能把同文重评当新文学收益。额外标点尾段无稳定收益，未发布；未知答话、事实与拒绝范围的分歧保留。
- `.coc/evaluations/craft-speech-aware-20260924/REPORT.md`为结构复核入口；早先stripMarkers-only的归属负分不再当真实UI证据。Boss以原DeliveryCard核对旧无引号样稿和真实T0/T7，确认颜色、2px边线、speaker title；独立预览不是整App/移动端验收。
- 新战役真实锁1.6.0/default jev，不靠mods.configure启用。7句玩家输入（两run连续同一战役）从委托、现场、档案到主动归还钥匙并正式completed。8次选择各1 attempt，5selected/3NONE，平均338ms（开场后270ms），范围251–817ms。7次全链路等待437.14s；未做同条件live off配对或浏览器首字测量，不把全链路差值归因于文笔模块。
- T3过度具体化了未载明的柜门/圣物目的；来源边界补为通用规则后，真实续桌T6明确钉法不证明用意。原错误观察保留，不宣称完全消除事实幻觉。最终Mod运行态失效修正是在本局之后以真实TS回归验证，没有伪称另跑过整局。完整记录 `.coc/playtests/craft-default-on-live-20260924/REPORT.md`。
- 构建及最终扩展全套2899/2899通过（243.82s）。Python全套1699pass/1旧条数断言failed，修为核对full/brief实际措辞后相关52/52过；runtime又复跑turn_floor+18接缝exit0。旧exit1保留，不冒称完整1700单次命令exit0。最终代码审阅及窄复核均pass。
- 原数据、试验失败和战役证据保留；自有driver/预览终端均停。没有打包、推送、重启用户App。后续测试执行按用户要求交Grok4.7fast worker，主会话只作实现、取舍和必要观察。
