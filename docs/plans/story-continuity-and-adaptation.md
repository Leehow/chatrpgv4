...
Current audit version is **narration-audit 1.2.15** (with enhanced-items 1.1.9; 1.2.9, 1.2.10, 1.2.11, 1.2.12, 1.2.13 and 1.2.14 were installed and exercised in retained live evidence and retain their historical meanings, and 1.2.11's checked `reentry_review` sub-review remains current with the 1.2.12 bridge-authority requirement, the 1.2.13 `bridge_offer` basis, and the 1.2.14 separation of the placement/offer stage from the acquisition/delivery stage). The version moves because the two-stage `bridge_offer` basis was added under `causal_reentry` on the contract section 37 slice, at 1.2.14 because retained live evidence (`midgame-reentry-live-15h-run`) showed the reviewer still applying the old receipt-first rule to an authority-true compliant offer, and at 1.2.15 because retained live evidence (`midgame-reentry-live-17b-run`, turn 6) showed the Keeper opening adaptation on a thread that already carried acquired evidence instead of clarifying it, so the `mode` (`clarify_known | introduce_evidence`) now closes which bases may pass; the causal-reentry pre-delivery deferral rule (1.2.10), the scene-commitment heading 1.2.8 (carried from 1.2.7) and the pending-preparation turn rule remain valid as historical descriptions. Current `story-thread` version is **1.2.5**; 1.2.3 remains its historical predecessor and its action-instruction meanings are preserved unchanged, 1.2.4 adds the creation-time no-clue clarification, and 1.2.5 adds the deterministic reentry mode: `clarify_known` uses one already acquired `known` row and continues the chosen action with no adaptation and no new clue, while `introduce_evidence` keeps the existing concrete bridge/authority/`source_rebinding`/`bridge_offer`/`bridge_receipt` flow; a `new_destination` scene's initial no-clue description is not a permanent prohibition, a later reviewed `source_rebinding` is the authorized way to add an existing clue's delivery relation there, no flag/ruling/note/adaptation may forbid the bridge or freeze the scene clue-free, and player agency is preserved after rebinding (`bridge_offer` when the player has not chosen acquisition, or the minimal receipt path when they have).
...
## P5 (core midgame mainline IMPLEMENTED and GENUINE-PLAY ACCEPTED; extended live gate PENDING)

**Current checkpoint (2026-09-13).** The core midgame causal-logic mainline is **IMPLEMENTED and GENUINE-PLAY ACCEPTED**, and the extended live gate is **PENDING**. Do not read this as P5 wholly complete: only the core mainline is accepted. Verified results, primary acceptance campaign `midgame-reentry-live-18` (runs `midgame-reentry-live-18b-run`, `midgame-reentry-live-18c-run`), static validation and performance are recorded in the [midgame causal re-entry checkpoint](../research/midgame-causal-reentry-2026-09-13.md). In that play a midgame explicit `misframed` frame was stored with its exact excerpt, the next turn projected `clarify_known` with known `globe-unpublished-story` and no bridge and delivered in 15.948 s without adaptation (`narrate` only), and the following explicit frame was stored `aligned` for `house-haunted-by-corbitt` without forcing the house route. The extended `introduce_evidence` → `source_rebinding` → `bridge_offer` chain has not yet completed in live play, so items 11, 14 and 15 below remain unaccepted and the gate stays separate from the now-complete core mainline. Human UI acceptance, integration and packaging remain pending/out of scope. Everything recorded below remains valid and historical and is not rewritten.
...
**Current checkpoint (2026-09-13), appended without rewriting the paragraphs above.** The core midgame causal-logic mainline is now IMPLEMENTED and GENUINE-PLAY ACCEPTED, and the extended live gate is PENDING. Verified in primary acceptance campaign `midgame-reentry-live-18` (runs `midgame-reentry-live-18b-run`, `midgame-reentry-live-18c-run`): turn 2 stored `misframed` for `house-haunted-by-corbitt` with the exact frame; turn 3 projected mode `clarify_known` with known `globe-unpublished-story` and no bridge, the first candidate was revised, and the explicit retry delivered in 15.948 s with only `narrate` (no lookup, no adaptation), audit job `43b4ee5adc70603314f3384d3ea895f687639d27b9d06b8a52d0047d701da872` passing `acquired_clarification`; turn 4, retested with the compact packet, passed `player_discharge` (audit job `3df2c7a86eb4eb5b6529aa57e44b5481c430e4eebfecd146caaa3b770b2b72d6`) and stored `aligned` for that thread, with no forced house route. Turn 3's post-commit memory extraction failed and remains in backlog because the old full continuity packet predated the compaction fix; it is retained as failed evidence. The aligned/library turn took 89.542 s with 10 `lookup` calls, classified as repeated lookup/model behavior rather than dynamic graph adaptation, and remains a material performance limitation; no claim is made that all turns meet the 30 s target. The extended `introduce_evidence` / `source_rebinding` / `bridge_offer` chain has not completed in live play, so items 11, 14 and 15 remain unaccepted and that gate stays pending and separate from the core mainline. Static validation for this checkpoint: `npm run check:kernel` passed; targeted continuity/adaptation/audit/turn suite 52/52; full `npm run test:ext` 883/883 exit 0; targeted Python `tests/kernel/test_memory.py` + `tests/play/test_driver.py` 29/29; runtime build passed; DeepSeek Flash only, no Astra/Grok. Human UI acceptance, integration and packaging remain pending/out of scope. Full verified evidence: [midgame causal re-entry checkpoint](../research/midgame-causal-reentry-2026-09-13.md).
