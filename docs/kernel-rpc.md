...
Player-view correction: PipiCOC renders delivered story, user input and structured mechanics/choices. Keeper thinking, tool traces and source dossiers remain retained evidence but are not rendered in the player transcript. Streaming provisional assistant text is withheld until delivery; the ordinary waiting indicator remains. The backend identifies this player projection from the session's immutable `productProfile.id === "coc-keeper"` or its retained `coc-session` binding, and suppresses live `text`, `thinking`, `tool_call`, `tool_result`, citation and source-detail events, including `message_end` catch-up. Settled history projection applies the same boundary: tool-result rows are omitted, and delivered assistant rows retain only prose/text ordering, never thinking, tool, citation or source activities, so reconciliation cannot resurrect a private summary bar. It still forwards turn status, retry status, terminal provider errors and authoritative `presentation` entries. A displayed `coc-delivery` service notice is one of those authoritative entries and is projected immediately on `entry_appended`; waiting for a later history refresh would recreate the blank turn the notice exists to prevent. The investigator panel no longer lists canonical NPC names, which can disclose an identity before the Keeper introduces it. Other products retain their existing transcript rendering.
