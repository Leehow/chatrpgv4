...
This decision supersedes the live-current-generation consumption in §14.1/§20.7: the shared module library is reusable source material, not a mutable table workspace. A campaign uses a private source workspace under `.coc/module-campaigns/<campaign>/modules/<module_id>/`, seeded once from a verified library publication. The seed retains the source graph bytes, manifest, readiness metadata, source PDF/index and referenced assets; it does not inherit publication leases, running jobs or completion identities. Subsequent graph generations, opening selection, reading metadata and queue writes belong to that workspace. Seeding and publication use cross-process locks and atomic publication; another campaign and the library remain unchanged. No existing campaign, library generation, transcript or reading artifact is deleted. Existing campaigns acquire a private seed on their next access by the updated runtime; already running old runtimes are not restarted or silently migrated.

Internal `module.*` source operations accept an optional `campaign` scope. Omitting it retains the library administration/initial source preparation path and its existing same-module claim/lease exclusion. With a campaign scope, status, opening choice, material checks, read request/claim/finish and assets use the same private workspace, including before `campaign.create` when setup has already selected an opening. Source binding, starter registration and module listing remain library operations. The campaign slug and module identity are validated; a scope cannot escape its storage root or substitute another campaign's module. The host retains one scope for the lifetime of a reading request and all its claim/finish/cancellation calls; it never finishes a library lease in a campaign workspace or vice versa. Initial source preparation may build reusable library material before campaign creation; it must not store a player's opening selection in that library. It passes the selected opening explicitly as the opening request focus, and campaign creation records that selection privately.

Producer: campaign creation/first access establishes the seed; scoped `module.opening.choose` and `module.read.finish` publish only to that campaign. Reader: every campaign-facing graph consumer, setup, material/asset resolver, background queue and adaptation source input uses the scoped workspace. Adoption: ordinary Keeper reads and unchanged retry-after-reading calls consume only their own campaign's publication. The shared source may continue to gain library publications without changing an existing campaign's graph. Adaptation still uses its immutable source snapshot plus worldline-local overlay; this isolation adds no new Keeper verb and no automatic adaptation rebase.
