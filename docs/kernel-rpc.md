...
`prompts/keeper.md` Law 4 names two machine tokens; the Writing paragraph states the say rule beside the marker rule; the floor's voice clause names the token. `content/craft/beat-directives.json` `floor_lines[voice]` names it; `narration-craft` 1.2.1's brief names it (a byte changed is a version bumped, §26). The `narrate` and `ask` tool descriptions gain one sentence. **The §30.7 per-turn brief ceiling rises from 4000 to 5000 bytes for all active briefs** (`tests/kernel/test_mod_director_text.py` pins it); §34.8's "4000" reads as this section's 5000. Old package versions keep their bytes and locks (§26).
