...
| `confidence`? | 0–1 |
| `corrects`? | 仅 `keeper_correction` 可用：最多 12 个 `{subject, statement}`，必须逐字来自任务包的 `correction_targets`；有更早候选时普通抽取也提供该字段（最多 30 个目标） |

`correction_targets` 是内核提供的有界、按相关性排序的较早候选；正常抽取在存在更早候选时提供它，最多 30 个。模型只复制这两个语义字段，不写 opaque id；目标严格早于 correction turn。`valid_from_turn` 是候选生效回合，连续性投影必须将它投影为 `turn`。

校验闭合：未知字段、任何机器键（`commit`、`receipt`、`turn`、`id`）、解析不到的名字、同名歧义（图上两个实体同名且都在场）都报 `invalid_params`，`details.index` 指到那一条，`fix` 写出可用的名字；`corrects` 仅限 `keeper_correction`，必须是任务包中不晚于本回合、非自身的精确 subject/statement；未知、未来、自指及无法唯一绑定的引用使整批原子失败。绑定由内核按精确 pair 绑定到保留候选 id（包括相同 statement 的旧重复），模型不见也不写 id。整批要么全落要么全不落。落盘：每条得 `id: "mem:t<n>-<k>"`，`status: "candidate"`，`source: {turn, commit, episode_id, receipts}`，`valid_from_turn: n`。**确定性接续**：同 `subject` 与同 `entities` 的新 `relationship` 或 `promise` 关闭旧的；显式 correction links 也按 §35.12 supersede 旧报告（旧条加 `valid_until_turn`、`superseded_by`），其余种类只累积。任务文件 `memory/jobs/<job_id>.json` 整文件原子写；同任务同内容重放幂等，内容不同报 `idempotency_conflict`。成功发 `memory-written` 事件。校验失败或扩展报告车道失败（`memory.fail {"campaign", "job_id", "reason", "detail"}`）写 `memory/backlog.jsonl` 一行 `{job_id, turn, reason: invalid|lane_error|model_error, detail, at, status: pending}`；backlog 里的任务 `memory.job` 不再自动派发，重派要显式给 `turn`。

候选不晋升：本切片没有把候选变成状态或规则事实的路径；`apply` 的 `note` 与 `ruling` 仍保留给后续切片。`recall memory` 把候选连同 `status` 一起给守秘人，相关与否由它判断。

### 12.4 `recall` 三路

- **memory**：params `{"what": "memory", "about"?: [名], "turns"?: [from, to], "kinds"?: [...], "include_superseded"?: bool, "limit"?: ≤ 30}`。仅 continuity 的 `query`/`anchors` 与 `recall memory` 的 `about` 是语义实体名（人、场景、线索等），不是自由散文；其他 lookup kind 保留既有搜索语义；命名人物、场景或线索即可参与相关召回。收窄全是确定性的：`about` 里的名字先精确匹配图上名字、别名与 `scene_labels`，不中时取「一个名字里的整词」（`Knott` → Steven Knott），人（调查员、NPC）优先于地点与线索，仍歧义则 `unknown_entity` 且 `fix` 列出候选名；命中按图上名字与别名归一化后精确匹配 `subject`、`knowers`、`entities`；`turns` 落在 `valid_from_turn`；缺省不含已关闭的。排序：与 `about`（缺省取当前在场实体加调查员）重叠数多者先，再按种类权重（#20：`world_event`、`knowledge`、`relationship`、`promise` 先于 `belief`、`player_preference`、`keeper_correction`，`player_assertion` 最后——它多半是玩家输入的复述，守秘人已经读过），再按 `valid_from_turn` 晚者先。result `{"what": "memory", "about": [...], "hits": [{"id", "kind", "subject", "knowers", "entities", "statement", "privacy", "state", "confidence", "status", "turn", "origin", "supersedes"?, "superseded_by"?}]}`。`origin` 明示 `source`、`hypothesis` 或 `keeper_correction`，不计算或臆造 comprehension score。相关 active correction 排在普通报告前；其相关性继承 linked targets 的实体上下文。缺省排除 withdrawn；`include_superseded` 同时返回旧 claim 与 superseding correction 的可读 statement。不接受散文筛选，没有关键词与正则。
- **transcript**：params `{"what": "transcript", "turns"?: [from, to], "role"?: "player"|"keeper", "read"?: {"turn", "role"}}`。`read` 返回 `verification_scope: "record_integrity_only"`；`verified` 仍表示字节记录一致，不表示事实为真。不带 `read` 时返回 `cards: [{"turn", "role", "chars", "head": "<前 80 字>"}]`（区间缺省最近 3 回合，最多 40 张），并在区间 ≤ 3 回合时同时返回切片 0 的 `entries`；带 `read` 时返回 `{"turn", "role", "text", "verified": bool}`，`verified` 表示逐字记录里的文本与 `turns/NNNN.json` 记录（守秘人取 `rendered_text`，玩家取 `player_text`）的 sha256 一致；不一致仍返回文本但 `verified: false`。
...
{"head": "<一句固定的话：以下是本回合开始时的全部场面，已含时钟、未发现线索、在场者秘密与来路；胶囊里有的不必再 look/lookup>",
 ...
 "memory": [{"id", "kind", "subject", "statement", "turn", "status", "state", "authority"}],
...
## 35. Story continuity and adaptation wire contract (implemented, 2026-09-12)
...
### 35.12 Memory repair contract (implemented)
...
### 35.13 PLANNED continuity-review repair (not implemented)
...
### 35.15 Adaptation routing and foreground latency repair (2026-09-12)
