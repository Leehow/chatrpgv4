...
export function createComponent(React) {
  ...
  return function Preparation({api,sessionId}) {
    ...
    async function hide(){setError('');try{const result=await api.invoke('onboarding',{action:'hide',id:job.id});if(!result.ok)throw new Error(result.error.message);await query.refresh();}catch(e){setExpanded(true);setError(e.message);}}
    ...
    return h('aside',{className:'coc-preparation-overlay','aria-label':'Scenario preparation'},
      ...
      h('div',{className:'coc-preparation-head'},
        ...
        ready&&h('button',{className:'coc-preparation-close','aria-label':'Hide preparation status',onClick:()=>void hide()},'✕')),
