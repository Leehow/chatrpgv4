...
export function createKernelRuntime(context: KernelContext): {
    ...
    const handlers = assembleHandlers(context, foundationHandlers(context), readHandlers(context, { ...writer.read, lookupRules: rules.lookup }), writer.handlers, modules.handlers, createSetupHandlers(context, writer), createLibraryHandlers(context, writer), resolver.handlers, createMemoryHandlers(context, writer), mods.handlers(writer), graphHandlers(context), createBranchHandlers(context, writer), createApplyHandlers(context, writer, {mods:mods.apply(writer),worldlines,resources:applyResources,ending:stageEnding,requireMaterial:modules.source.requireMaterial,materialReady:modules.source.materialReady,queueAdjacentReading:modules.source.queueAdjacentReading,asset:(id,name)=>modules.source.store.asset(id,name),weaponCatalog:graph=>moduleWeaponCatalog(context,graph)}));
