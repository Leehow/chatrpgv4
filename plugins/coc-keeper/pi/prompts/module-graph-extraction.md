...
packet 里的 `page_window` 说明这一份覆盖了哪几页,以及这本书在它**前面还有多少页、
后面还有多少页**。`pages_after` 大于 0,就意味着你读到的内容会在页边中断——一个场景
只写了一半,一条线索还没揭晓,一个 NPC 刚出场。

**这时唯一正确的做法是:如实抽你看得见的,把没看完的方面在 `coverage` 里标成
`partial` 或 `unresolved`。**

**绝对不要引用 `evidence_view` 里没有的 span id。** 尤其不要顺着编号往下推——你看到
`span-page-97-block-1` 到 `span-page-101-block-9`,不代表 `span-page-102-block-3`
存在于你的 packet 里。它不存在,机器会逐条打回,你这一轮白写。

这不是假设:记录在案的 1281 条编造引用,**全部**是紧接着本节之后的页码,而且**只出现
在被切成片的书里**。你不知道下一页写了什么——这没关系,说不知道就行。

## 先看 `known_nodes`

packet 里的 `known_nodes` 是**这本书已经建立的名册**——别的节点已经给这些人物、
地点、势力定好了 `node_id`。你这一节再遇到他们时,**必须原样沿用那个 id**,哪怕
书在这一页用了别的叫法(把别的叫法写进 `aliases`)。

这不是风格问题。全书是分节抽的,最后要合成一张图:同一个邪教在一节里叫
`faction-bloody-tongue`、在另一节里叫 `faction-cult-of-the-bloody-tongue-nyc`,
合并出来就是两个邪教,玩家追线索会追丢。

`known_nodes` 里没有的,你照常自己命名;名册是起点,不是上限。
...
- **什么才算场景:玩家能站在里面的地方。** `scene` / `beat` / `event` / `ending`
  是运行时会把队伍放进去的四种东西 —— 守密人可以说「你们现在在这里」。

  书里有大量段落**不是**这个:给守密人的前情与阴谋总览、侧栏式的风土背景、
  设计者笔记、章节导言、属性表附录。它们重要,但玩家进不去。把它们写成 `scene`,
  整张图就会因为"这个场景没有出口"而判不可玩 —— 而书本身没错,是分类错了。

  这类内容写成 `concept`(背景与设定)、`rule`(判定与数值)、`section`(书的分节)、
  或者干脆并进相关场景的 `summary`。判断标准只有一条:**守密人能不能说
  「你们现在在这里」**。
