...
2. 产出薄 schema：地点/场景的 L1 索引和 L2 正文、readaloud、keeper_notes、NPC/线索/handout/map 引用，以及四类 SceneEdge（next/if/timeline/clue/fail_loop）。每个实体带 `source_refs`，秘密默认 `secrecy: keeper_only`。若任务附 `needs_image` 或 map-supply asset refs，必须检查这些页：把可确认的 `Map{id,caption,page_ref,linked_locations,image_ref,source_refs,secrecy}` 写进场景的 `maps_ref`；没有原图或关联不确定时保留页引用与失败/不确定原因，绝不从标题或 OCR 文本臆造地图。若任务含 `source_asset_catalog` 或已落盘的 `coc_source_assets` 目录，必须语义消费它：用 `associate` 把已有资产绑到地点/场景/线索/NPC 并写 reason；禁止按文件名/标题/正则映射。玩家可见地图除 `maps_ref` 外必须再产出 `kind:"map"` handout pack，`asset_id`/`image_ref`/`source_refs` 只复制目录里代码生成的值，绝不自造哈希或 id；KP-only 只走 `maps_ref` + `coc_map_supply.present`。边必须写 `provenance`：目录邻接/同父子/时间线是可召回依据，纯叙事暗线写 `semantic_inference`，但仍带支撑它的 source_refs。
