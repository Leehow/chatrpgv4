...
def _characteristic_half_fifth_gaps(characters: list[dict[str, Any]]) -> list[str]:
    ...
    for character in characters:
        ...
        if not isinstance(characteristics, dict) or not characteristics:
            gaps.append(f"{investigator_id} missing characteristics")
            continue
        if not isinstance(thresholds, dict) or not thresholds:
            gaps.append(f"{investigator_id} missing characteristic_thresholds")
        for key, value in characteristics.items():
            if not isinstance(value, int):
                continue
            expected = {"full": value, "half": value // 2, "fifth": value // 5}
            threshold = thresholds.get(key) if isinstance(thresholds, dict) else None
            if not isinstance(threshold, dict):
                gaps.append(f"{investigator_id} missing characteristic_thresholds.{key}")
            elif any(threshold.get(field) != expected[field] for field in ["full", "half", "fifth"]):
                gaps.append(f"{investigator_id} characteristic_thresholds.{key} mismatch")

        creation = character.get("_creation")
        creation_characteristics = creation.get("characteristics", {}) if isinstance(creation, dict) else {}
        if not isinstance(creation_characteristics, dict) or not creation_characteristics:
            gaps.append(f"{investigator_id} creation missing characteristic half/fifth source")
            continue
        for key, value in creation_characteristics.items():
            if not isinstance(value, dict):
                continue
            final = value.get("final")
            if not isinstance(final, int):
                continue
            if value.get("half") != final // 2 or value.get("fifth") != final // 5:
                gaps.append(f"{investigator_id} creation characteristic {key} missing half/fifth")
    return gaps
...
def _report_skill_name_leaks(battle_report: str, metadata: dict[str, Any], context: dict[str, Any]) -> list[str]:
    ...
    for heading in headings:
        section = _strip_html_comments(_section_text(battle_report, heading))
...
def _haunting_conclusion_reward_gaps(events: list[dict[str, Any]], rolls: list[dict[str, Any]]) -> list[str]:
    ...
    for roll in reward_rolls:
        ...
        if (
            ...
            and len(die_rolls) == 1
            and isinstance(die_rolls[0], int)
            and payload.get("die") == "1D6"
            and 1 <= die_roll <= 6
            and die_rolls[0] == die_roll
    ...
    return ["conclusion reward roll is present but does not prove 1D6 SAN gain, die face, and final_san continuity"]
...
def _sanity_failure_involuntary_action_gaps(rolls: list[dict[str, Any]]) -> list[str]:
    """Rulebook p.166: failing a SAN roll always causes loss of self-control.

    Every ``type: "sanity"`` roll with ``outcome == "failure"`` must carry an
    ``involuntary_action`` block with one of the five rulebook kinds. SAN rolls
    that succeed are out of scope (the rule applies to failures only), and a
    failure that escalates to temporary insanity still records the momentary
    involuntary action separately from the bout of madness.
    """
    gaps: list[str] = []
    for roll in rolls:
        payload = roll.get("payload", {})
        if roll.get("type") != "sanity" and payload.get("skill") != "SAN":
            continue
        if payload.get("outcome") != "failure":
            continue
        goal = str(payload.get("goal") or payload.get("roll_id") or "unknown SAN roll")
        action = payload.get("involuntary_action")
        if not isinstance(action, dict):
            gaps.append(f"{goal} failed SAN roll missing involuntary_action block")
            continue
        kind = action.get("kind")
        if kind not in _INVOLUNTARY_ACTION_KINDS:
            gaps.append(
                f"{goal} involuntary_action.kind '{kind}' not in rulebook kinds "
                f"{sorted(_INVOLUNTARY_ACTION_KINDS)}"
            )
        if not str(action.get("summary") or "").strip():
            gaps.append(f"{goal} involuntary_action.summary missing")
    return gaps
...
# --------------------------------------------------------------------------- #
# Combat state audits (Chapter 6) — read save/combat.json, never trust prose
# --------------------------------------------------------------------------- #
_COMBAT_LVL = {"fumble": 0, "failure": 1, "regular": 2, "hard": 3, "extreme": 4, "critical": 5}


def _combat_dex_order_gaps(combat_state: dict[str, Any]) -> list[str]:
    """Each round's turns must follow initiative_order (DEX desc, ties by
    combat_skill). Per-turn dex overrides (e.g. casting_dominate = 85) are
    honored by re-ranking turns by their recorded dex/combat_skill."""
    gaps: list[str] = []
    rounds = combat_state.get("rounds", [])
    if not isinstance(rounds, list):
        return gaps
    participants = {p.get("actor_id"): p for p in combat_state.get("participants", [])}
    for rnd in rounds:
        turns = rnd.get("turns", []) or []
        if not turns:
            continue
        # Build the expected order from each turn's recorded dex + tie-break
        # by the participant's combat_skill. Then compare to actual order.
        expected = sorted(
            range(len(turns)),
            key=lambda i: (
                -(turns[i].get("dex") or participants.get(turns[i].get("actor_id"), {}).get("dex", 0) or 0),
                -participants.get(turns[i].get("actor_id"), {}).get("combat_skill", 0),
            ),
        )
        actual = list(range(len(turns)))
        if expected != actual:
            names = [t.get("actor_id", "?") for t in turns]
            gaps.append(f"round {rnd.get('round')} turns out of DEX order: {names}")
    return gaps


def _combat_opposed_pairing_gaps(combat_state: dict[str, Any]) -> list[str]:
    """Attack/maneuver turns must carry an opposed_roll_id, unless the
    action is surprise_attack (unopposed per p.106) or defense_kind is none."""
    gaps: list[str] = []
    for rnd in combat_state.get("rounds", []) or []:
        for turn in rnd.get("turns", []) or []:
            action = turn.get("action")
            defense = turn.get("defense_kind")
            if action in ("attack", "maneuver") and defense not in ("none", None):
                if not turn.get("opposed_roll_id"):
                    gaps.append(
                        f"turn {turn.get('turn_id')} ({action}) lacks opposed_roll_id"
                    )
    return gaps


def _combat_damage_chain_gaps(combat_state: dict[str, Any]) -> list[str]:
    """Every damage_chain entry must balance: hp_before + hp_delta == hp_after,
    and armor_absorbed + (-hp_delta) == raw_damage (when raw_damage present)."""
    gaps: list[str] = []
    for d in combat_state.get("damage_chain", []) or []:
        hp_before = d.get("hp_before"); hp_delta = d.get("hp_delta"); hp_after = d.get("hp_after")
        if all(isinstance(x, int) for x in (hp_before, hp_delta, hp_after)):
            if hp_before + hp_delta != hp_after:
                gaps.append(f"damage {d.get('damage_roll_id')} hp imbalance: {hp_before}+{hp_delta}!={hp_after}")
        raw = d.get("raw_damage"); absorbed = d.get("armor_absorbed")
        if all(isinstance(x, int) for x in (raw, absorbed, hp_delta)) and raw is not None:
            if absorbed + (-hp_delta) != raw:
                gaps.append(f"damage {d.get('damage_roll_id')} armor imbalance: {absorbed}+{-hp_delta}!={raw}")
    return gaps


def _combat_pushed_roll_gaps(combat_state: dict[str, Any], rolls: list[dict[str, Any]]) -> list[str]:
    """Combat rolls must not be pushed (p.116). Cross-reference damage_chain
    and turn roll_ids against rolls.jsonl for pushed:true."""
    gaps: list[str] = []
    roll_ids_in_combat = set()
    for d in combat_state.get("damage_chain", []) or []:
        rid = d.get("damage_roll_id")
        if rid:
            roll_ids_in_combat.add(rid)
    for rnd in combat_state.get("rounds", []) or []:
        for turn in rnd.get("turns", []) or []:
            for key in ("roll_id", "opposed_roll_id"):
                rid = turn.get(key)
                if rid:
                    roll_ids_in_combat.add(rid)
    for r in rolls:
        payload = r.get("payload", {})
        if payload.get("roll_id") in roll_ids_in_combat and payload.get("pushed") is True:
            gaps.append(f"combat roll {payload.get('roll_id')} marked pushed (forbidden p.116)")
    return gaps


def _combat_outcome_gaps(combat_state: dict[str, Any]) -> list[str]:
    """Concluded combat must have an outcome; participants with hp_current<=0
    must be reflected (monster side down => investigators_win, etc)."""
    gaps: list[str] = []
    if combat_state.get("status") == "concluded" and not combat_state.get("outcome"):
        gaps.append("combat status=concluded but outcome is null")
    if combat_state.get("outcome"):
        participants = combat_state.get("participants", []) or []
        investigators_down = all(p.get("hp_current", 1) <= 0 for p in participants if p.get("side") == "investigator")
        monsters_down = all(p.get("hp_current", 1) <= 0 for p in participants if p.get("side") == "monster")
        outcome = combat_state.get("outcome")
        if investigators_down and outcome != "monsters_win":
            gaps.append(f"all investigators down but outcome={outcome}")
        if monsters_down and outcome not in ("investigators_win", "fled"):
            gaps.append(f"all monsters down but outcome={outcome}")
    return gaps


def _combat_state_missing_gaps(combat_state: dict[str, Any], events: list[dict[str, Any]]) -> list[str]:
    """If events.jsonl contains combat events but combat.json is empty/missing,
    the run wrote combat narration without structured state."""
    gaps: list[str] = []
    has_combat_events = any(e.get("type") == "combat" for e in events)
    state_empty = (not combat_state) or (not combat_state.get("participants"))
    if has_combat_events and state_empty:
        gaps.append("events contain combat events but save/combat.json missing or empty")
    return gaps
...
def audit_run(run_dir: Path) -> dict[str, Any]:
    ...
    if _haunting_module_required(metadata):
        ...
        if conclusion_reward_gaps:
            findings.append(_finding(
                ...
                "Record the The Haunting conclusion reward as a reward roll with reward_kind=sanity, source=conclusion_rewards, die=1D6, die_rolls, roll_id, san_before, san_delta, and san_after matching final_san.",
            ))
        ...
        involuntary_action_gaps = _sanity_failure_involuntary_action_gaps(context["rolls"])
        if involuntary_action_gaps:
            findings.append(_finding(
                "sanity_failure_involuntary_action_missing",
                "system_gap",
                "high",
                "; ".join(involuntary_action_gaps),
                "Per Keeper Rulebook p.166, failing a SAN roll always causes the investigator to lose self-control; record an involuntary_action block (kind in jump_in_fright/cry_out/involuntary_movement/involuntary_combat_action/freeze, plus a summary) on every failed SAN roll.",
            ))
    ...
    # ----- Combat state audits (Chapter 6) ----- reads save/combat.json
    # Note: combat_state_missing is gated until harness integration migrates
    # the inline Haunting combat blocks to CombatSession. Runs that already
    # produce save/combat.json get the full structural check; runs without it
    # are not penalized yet (the gate flips once the migration lands).
    combat_state = context.get("combat_state") or {}
    if combat_state and combat_state.get("participants"):
        combat_dex_gaps = _combat_dex_order_gaps(combat_state)
        if combat_dex_gaps:
            findings.append(_finding(
                "combat_dex_order_not_proven",
                "system_gap",
                "high",
                "Combat turns do not follow DEX initiative order: " + "; ".join(combat_dex_gaps),
                "Order rounds[].turns[] by DEX desc (with per-turn dex overrides like casting_dominate); break ties by combat_skill (p.114).",
            ))
        combat_pairing_gaps = _combat_opposed_pairing_gaps(combat_state)
        if combat_pairing_gaps:
            findings.append(_finding(
                "combat_opposed_pairing_missing",
                "system_gap",
                "high",
                "Attack/maneuver turns lack opposed_roll_id: " + "; ".join(combat_pairing_gaps),
                "Pair every attack vs defender with an opposed roll (Fighting for fight_back, Dodge for dodge) and link via opposed_roll_id (p.115).",
            ))
        combat_damage_gaps = _combat_damage_chain_gaps(combat_state)
        if combat_damage_gaps:
            findings.append(_finding(
                "combat_damage_chain_broken",
                "system_gap",
                "high",
                "Combat damage chain does not balance (hp or armor): " + "; ".join(combat_damage_gaps),
                "Each damage_chain entry must satisfy hp_before+hp_delta==hp_after and armor_absorbed+(-hp_delta)==raw_damage.",
            ))
        combat_pushed_gaps = _combat_pushed_roll_gaps(combat_state, context["rolls"])
        if combat_pushed_gaps:
            findings.append(_finding(
                "combat_pushed_roll_present",
                "system_gap",
                "high",
                "Combat rolls marked pushed (forbidden by p.116): " + "; ".join(combat_pushed_gaps),
                "Combat rolls (Fighting/Firearms) cannot be pushed; the next attack is the substitute per p.116.",
            ))
        combat_outcome_gaps = _combat_outcome_gaps(combat_state)
        if combat_outcome_gaps:
            findings.append(_finding(
                "combat_outcome_unresolved",
                "system_gap",
                "high",
                "Combat outcome unresolved or inconsistent: " + "; ".join(combat_outcome_gaps),
                "Set outcome (investigators_win/monsters_win/fled/stalemate) when concluding; reflect participant hp in the outcome.",
            ))
    if combat_state and combat_state.get("participants"):
        # Only run the structural checks when a combat.json exists.
        combat_dex_gaps = _combat_dex_order_gaps(combat_state)
        if combat_dex_gaps:
            findings.append(_finding(
                "combat_dex_order_not_proven",
        ...
        # Combat Tracker rendering (parallel to chase_tracker_not_rendered).
        combat_tracker_section = _section_text(battle_report, "Combat Tracker")
        if not combat_tracker_section or "No combat tracker recorded." in combat_tracker_section:
            findings.append(_finding(
                "combat_tracker_not_rendered",
                "report_gap",
                "high",
                "save/combat.json has participants, rounds, and damage chain, but the battle report does not render a populated ## Combat Tracker section.",
                "Render save/combat.json participants, initiative order, turns, opposed pairings, and damage chain in ## Combat Tracker.",
            ))
