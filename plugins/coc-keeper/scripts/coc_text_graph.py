...
OBLIGATION_NAMESPACE_TS = (
    PLUGIN_ROOT / "pi" / "lib" / "obligation-namespace.generated.ts"
...
def project_obligation_namespace() -> str:
    ...
    kinds = coc_text_runtime.vocabulary()["obligation_kinds"]
    prefixes = [kinds[name]["id_prefix"] for name in sorted(kinds)]
    ordered = sorted(prefixes)
    body = ",\n".join(f'  "{value}"' for value in ordered)
    kind_names = ",\n".join(f'  "{name}"' for name in sorted(kinds))
    return (
        "/**\n"
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "project":
        ...
        target = (
            REFERENCES_DIR.parent / "pi" / "lib" / "obligation-namespace.generated.ts"
        )
        if args.write:
