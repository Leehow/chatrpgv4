...
DEFAULT_POLICY_PATH = PLUGIN_ROOT / "pi" / "lib" / "operation-policy.generated.ts"
...
def build_policy_projection(toolbox: Any | None = None) -> dict[str, Any]:
    """Build Pi policy facts from the same canonical operation descriptors."""
    tb = toolbox if toolbox is not None else _load_toolbox()
    registry = getattr(tb, "OPERATION_REGISTRY", None)
    specs = getattr(registry, "specs", None)
    if not isinstance(specs, dict) and not hasattr(specs, "items"):
        raise ContractArchiveError(
            "operation_registry_missing",
            "canonical toolbox has no OperationRegistry specs",
        )
    if set(specs) != set(getattr(tb, "TOOLS", {})):
        raise ContractArchiveError(
            "operation_registry_drift",
            "OperationRegistry specs drift from toolbox.TOOLS",
        )

    policies: dict[str, Any] = {}
    surfaces: dict[str, list[str]] = {
        surface: []
        for surface in sorted(tb.coc_operation_policy.KP_SURFACES - {"none"})
    }
    for name in sorted(specs):
        operation_spec = specs[name]
        policy = operation_spec.policy.public()
    ...
    return {
        "audiences": sorted(tb.coc_operation_policy.AUDIENCES),
        "play_phases": sorted(tb.coc_operation_policy.PHASES),
        "kp_surfaces": sorted(tb.coc_operation_policy.KP_SURFACES),
        "execution_classes": [
            "parallel_read", "serial_campaign", "serial_global",
        ],
        "operation_policy": policies,
        "operations_by_surface": surfaces,
    }


def policy_projection_to_canonical_bytes(projection: dict[str, Any]) -> bytes:
 ...
 */
export const AUDIENCES = {encoded(projection["audiences"])} as const;
export const PLAY_PHASES = {encoded(projection["play_phases"])} as const;
export type PlayPhase = typeof PLAY_PHASES[number];
export const KP_SURFACES = {encoded(projection["kp_surfaces"])} as const;
export type KpSurface = typeof KP_SURFACES[number];
export const EXECUTION_CLASSES = {encoded(projection["execution_classes"])} as const;
...
export const OPERATIONS_BY_SURFACE: Record<Exclude<KpSurface, "none">, readonly string[]> = {encoded(projection["operations_by_surface"])};
'''
    return source.encode("utf-8")


def write_policy_projection(
    path: Path, projection: dict[str, Any] | None = None
) -> dict[str, Any]:
    payload = projection if projection is not None else build_policy_projection()
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(policy_projection_to_canonical_bytes(payload))
    return payload


def validate_policy_projection(
    path: Path, toolbox: Any | None = None
) -> dict[str, Any]:
    expected = build_policy_projection(toolbox)
    expected_bytes = policy_projection_to_canonical_bytes(expected)
    try:
        actual = path.read_bytes()
    except OSError as exc:
        raise ContractArchiveError(
            "policy_projection_missing",
            f"Pi operation policy projection missing or unreadable: {path}: {exc}",
        ) from exc
    if actual != expected_bytes:
        raise ContractArchiveError(
            "policy_projection_stale",
            f"Pi operation policy projection drifts from OperationRegistry: {path}",
        )
    return expected
...
def write_projections(
    ...
    tb = toolbox if toolbox is not None else _load_toolbox()
    archive = write_archive(archive_path, build_archive(tb))
    policy = write_policy_projection(policy_path, build_policy_projection(tb))
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help=f"generated Pi policy path (default: {DEFAULT_POLICY_PATH})",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        help=(
            "write/check both projections below an alternate plugin root; "
            "used by workers and CI for temporary generation"
        ),
    )
    ...
    if args.output_root is not None:
        archive_path = args.output_root / "references" / "mcp-operation-contracts.json"
        policy_path = args.output_root / "pi" / "lib" / "operation-policy.generated.ts"

    try:
        if args.command == "build":
            payload, _policy = write_projections(archive_path, policy_path)
            print(
                json.dumps(
                    {
                        "ok": True,
        ...
        archive = load_and_validate(archive_path)
        validate_policy_projection(policy_path)
        print(
            json.dumps(
                {
                    "ok": True,
