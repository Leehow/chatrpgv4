...
def player_facing_style_contract(language: str = "zh-Hans") -> dict[str, Any]:
    ...
    if language == "zh-Hans":
        return {
            ...
            "avoid": list(coc_text_runtime.craft("zh-Hans")["avoid"]),
            "prefer": list(coc_text_runtime.craft("zh-Hans")["prefer"]),
    ...
    return {
        "language": language,
        "register": "natural_tabletop_narration",
        "avoid": list(coc_text_runtime.craft(language)["avoid"]),
        "prefer": list(coc_text_runtime.craft(language)["prefer"]),
        "repetition_policy": repetition_policy,
        "style_guard": guard,
        "render_contract": crisis_scene_render_contract(language),
    }
