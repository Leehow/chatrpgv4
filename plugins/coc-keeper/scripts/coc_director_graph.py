...
LEGACY_THRESHOLDS = [
    ...
    ("score-precision-digits", "scoring.weighted_score", "eq", 4,
     ...
     None),
    ("time-advance-default-confidence", "director_plan.time_advance.confidence", "eq", 0.7,
     "A time proposal derived from the action's own profile is reported at moderate confidence, so the apply layer treats it as a proposal rather than a settled advance.",
     UNKNOWN,
     "Lanes comparing a low and a high default confidence on an ordinary turn; compare whether the apply layer's handling of the proposal changes.",
     None),
    ("time-advance-exhaustion-confidence", "director_plan.time_advance.confidence", "eq", 0.85,
     "An exhaustion-driven sleep proposal is reported at higher confidence than an ordinary one, because the trigger is a measured elapsed-time signal rather than an action default.",
     UNKNOWN,
     "Lanes on an exhausted investigator; vary the confidence and compare downstream handling.",
     None),
    ("time-advance-deadline-confidence", "director_plan.time_advance.confidence", "eq", 0.6,
     "Under high time pressure the proposal is reported at the lowest confidence of the three, because it is a suppression rather than a positive estimate.",
     UNKNOWN,
     "Lanes with an imminent deadline; vary the confidence and compare downstream handling.",
     None),
    ("time-advance-exhaustion-hours", "time_signals.hours_since_last_rest", "gt", 18,
     "Beyond eighteen hours without rest the Director proposes a sleep downtime instead of the action's own profile, so healing and sanity day-reset triggers can fire.",
     UNKNOWN,
     "Lanes at sixteen, eighteen and twenty hours since rest; compare when the sleep proposal appears.",
     None),
    ("time-advance-exhaustion-delta-minutes", "director_plan.time_advance.delta_minutes", "eq", 480,
     "The exhaustion proposal advances eight hours, one night's sleep.",
     UNKNOWN,
     "Lanes with a six-hour and an eight-hour sleep; compare which day-reset triggers fire.",
     None),
    ("time-advance-deadline-delta-minutes", "director_plan.time_advance.delta_minutes", "eq", 5,
     "With a deadline imminent the proposal collapses to five minutes, the smallest advance that still moves the clock.",
     UNKNOWN,
     "Lanes with an imminent deadline; vary the minimal advance and compare deadline pressure across a scene.",
     None),
    ("time-advance-confidence-digits", "director_plan.time_advance.confidence", "eq", 2,
     "Reported confidence is rounded to two decimals.",
     UNKNOWN,
     "Not independently falsifiable in play; recorded so the value is not an untracked literal.",
     None),
    ("live-affordance-route-cap", "live_scene.affordances", "gte", 3,
     "Route scanning stops once three live affordances are collected, bounding how much of a scene's exit surface one context projection carries.",
     UNKNOWN,
     "Lanes on a scene with six routes; vary the cap and compare whether the Keeper still finds the route the player wanted.",
     None),
    ("live-affordance-minimum", "live_scene.affordances", "lt", 2,
     "Below two affordances the projection backfills a resume line and an investigator angle, so a live scene is never presented with a single thing to do.",
     UNKNOWN,
     "Lanes on a scene with exactly one authored route; vary the minimum and compare the presented options.",
     None),
    ("live-affordance-return-cap", "live_scene.affordances.returned", "lte", 3,
     "At most three affordances survive de-duplication into the projection.",
     UNKNOWN,
     "Lanes on a scene with five distinct affordances; vary the cap and compare Keeper choice quality.",
     None),
    ("live-affordance-merge-cap", "live_scene.affordances.merged", "lte", 6,
     "At most six affordances survive the merge of authored and live scene state, twice the projection cap so de-duplication has room to work.",
     UNKNOWN,
     "Lanes on a scene whose authored and live affordances overlap heavily; vary the merge cap and compare what reaches the projection.",
     None),
    ("clue-policy-lead-count", "clue_policy.leads", "lte", 2,
     "A clue policy offers at most two leads, so the Keeper is pointed rather than handed the whole route ledger.",
     UNKNOWN,
     "Lanes on a scene with five available clues; vary the lead count and compare whether the Keeper's reveal stays focused.",
     None),
    ("storylet-need-stalled-turns", "rule_signals.stalled_turns", "gte", 3,
     "Three stalled turns make the storylet scheduler infer a recovery-shaped story need. NOTE: this is a second copy of the same number as override-stalled-turns, maintained independently in coc_storylets.",
     UNKNOWN,
     "Lanes at two, three and four stalled turns; compare when the inferred story need changes and whether it should move with the Layer-3 override.",
     None),
    ("storylet-recent-window", "storylet_ledger.recent_families", "lte", 8,
     "The anti-repeat 'recent' window holds eight entries, which is what makes a family or trope count as recently used and therefore excluded.",
     UNKNOWN,
     "Lanes over a long session with a four and a sixteen entry window; compare how quickly families become reusable.",
     None),
    ("storylet-used-window", "storylet_ledger.used_families", "lte", 999,
     "The 'used' window is effectively unbounded at 999 entries, so the softer family and trope repeat penalties apply for the whole campaign rather than expiring.",
     UNKNOWN,
     "Lanes over a long session with a bounded used window; compare whether early families ever return to full weight.",
     None),
    ("storylet-used-targets-window", "storylet_ledger.used_targets", "lte", 16,
     "Used targets are remembered for sixteen entries, twice the recent window.",
     UNKNOWN,
     "Lanes over a session revisiting one target; vary the window and compare repetition.",
     None),
    ("memory-callback-candidate-floor", "memory.callback_query.limit", "gte", 20,
     "A memory callback query asks for at least twenty candidates regardless of how few references the turn carries.",
     UNKNOWN,
     "Lanes on a turn with two references; vary the floor and compare whether a relevant callback is found.",
     None),
    ("memory-callback-refs-multiplier", "memory.callback_query.limit", "eq", 5,
     "The callback query scales at five candidates per reference, so a reference-rich turn searches proportionally wider.",
     UNKNOWN,
     "Lanes on a reference-rich turn; vary the multiplier and compare callback quality.",
     None),
    ("memory-callback-overlap-weight", "memory.callback.score", "eq", 4,
     "Each overlapping entity is worth four points of callback score, which is what ranks candidate callbacks against each other.",
     UNKNOWN,
     "Lanes where a one-entity and a three-entity callback compete; vary the weight and compare the pick.",
     None),
    ("memory-callback-score-digits", "memory.callback.score", "eq", 3,
     "Callback scores are rounded to three decimals.",
     UNKNOWN,
     "Not independently falsifiable in play; recorded so the value is not an untracked literal.",
     None),
    ("mythos-signature-sample", "mythos_directive.signature_sample", "lte", 2,
     "At most two signature elements are sampled into a mythos presentation directive, so the Keeper gets a hint rather than the creature's full description.",
     UNKNOWN,
     "Lanes on a mythos encounter; vary the sample size and compare how much the narration gives away.",
     None),
    ("fumble-tick-bound", "exceptional_effect.ticks", "lte", 4,
     "A typed fumble effect may advance a clock by at most four ticks; anything larger is rejected as malformed rather than applied.",
     UNKNOWN,
...
# Storylet-scheduler multipliers (slice D2b). The scheduler is a second scoring
# engine: it multiplies candidate storylet weights rather than scoring the ten
# director actions, so these carry a scope instead of an action_ref.
# (scope, condition_id, value, value_kind, rationale, falsifiable_by)
LEGACY_MULTIPLIERS = [
    ("storylet-selection", "family-repeat-penalty", 0.45, "constant",
     "A storylet whose family has already been used this session keeps under half its weight, so families rotate without a family ever becoming unusable.",
     "Lanes on a checkpoint whose ledger already used one family; vary the penalty and compare how often that family returns."),
    ("storylet-selection", "trope-repeat-penalty", 0.6, "constant",
     "A repeated trope is penalised less than a repeated family, so a family rotates faster than the tropes inside it.",
     "Lanes with a used trope but unused family; vary the penalty and compare trope repetition across a session."),
    ("storylet-selection", "conflict-rank-gap", [0.35, 0.25], "capped-linear",
     "A storylet one conflict level away from the target keeps three quarters of its weight and never drops below about a third, so an off-level beat stays reachable when nothing on-level qualifies.",
     "Lanes where only off-level storylets pass the gates; vary the floor and compare whether an off-level beat is picked or none is."),
    ("storylet-selection", "serves-reveal-clue", 1.25, "constant",
     "A storylet that can carry an available clue is the strongest of the four serves bonuses.",
     "Lanes on a checkpoint with an available clue and both clue-serving and front-serving candidates; compare which is picked."),
    ("storylet-selection", "serves-tick-front", 1.2, "constant",
     "A storylet that can tick a live threat front ranks just below one that can reveal a clue.",
     "Lanes with a near-full front and an available clue; compare the pick."),
    ("storylet-selection", "serves-deepen-npc", 1.15, "constant",
     "A storylet that can deepen a present NPC ranks below front-ticking.",
     "Lanes with an agenda NPC present and a live front; compare the pick."),
    ("storylet-selection", "serves-surface-choice", 1.1, "constant",
     "Surfacing a choice is the weakest of the four serves bonuses.",
     "Lanes with a choice frame and an available clue; compare the pick."),
    ("storylet-selection", "polarity-match", 1.35, "constant",
     "Matching the trigger's polarity outweighs every individual serves bonus, so tone alignment beats mechanical usefulness by one step.",
     "Lanes where a polarity-matched storylet serves nothing and a clue-serving storylet is off-polarity; compare the pick."),
    ("storylet-selection", "scene-tag-summoned-boost", 5.0, "constant",
     "A storylet summoned by a scene tag is boosted far above the ordinary range, because the generic ambient pool's aggregate weight grows with its size and a modest boost leaks.",
     "Lanes on a scene-tag-summoned turn with a large generic pool; lower the boost and count how often a generic still wins."),
    ("storylet-selection", "scene-tag-generic-suppression", 0.01, "constant",
     "While a scene tag has summoned a beat, generic storylets are suppressed to a hundredth rather than zero, so a generic remains a fallback when no summoned candidate passes the other gates.",
     "Lanes where every summoned candidate is gated out; set the suppression to zero and confirm whether the turn loses its beat entirely."),
]
...
def doctrine_shard() -> dict[str, Any]:
    ...
    for (scope, condition_id, value, value_kind, rationale,
         falsifiable_by) in LEGACY_MULTIPLIERS:
        nodes.append(_doctrine_node(
            f"multiplier:{scope}:{condition_id}", "multiplier",
            ...
            rationale=rationale, origin=UNKNOWN,
            falsifiable_by=falsifiable_by, source_refs=None,
        ))
