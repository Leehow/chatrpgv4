...
- `scenes` (object[])：场景数组。每个 scene：
  ...
  - `affordances` (object[], optional)：该场景自然露出的可行动线（diegetic routes，非玩家菜单）。开场与多分叉场景应至少 2 条，让玩家有选择权、不被线性推向单一出口。每条含 `id`（route 标识）、`cue`（可行动的感官/叙事提示）、可选 `route_type`（**固定枚举**，选最贴切的：`tenant_history` 前租客/房史、`reward_scope` 报酬范围、`direct_entry` 直接进入、`npc_question` 向 NPC 追问、`environment` 环境调查、`investigative_lead` 调查线索、`scene_affordance` 场景通用——运行时 focus 提取按此枚举匹配玩家结构化意图）、可选 `status`（`open`/`suggested`/`exhausted`/`locked`，缺省视为 `open`）。引擎据此结构化计算 `is_real_fork`（≥2 条 open route），仅在真分叉时才把选择交给玩家。需要检定的路线可带结构化 `roll_gate`；普通失败后同一目标默认仍属同一次尝试，运行时只给 Push、换方法/目标或有证据的重置建议，不以关键词猜“已经换方法”。只有模组确实允许时间重置时才增加 `retry_policy: {"mode":"elapsed_time_reset","minimum_elapsed_minutes":<正整数>}`；权威游戏时间达到窗口后 `actions.advise` 才生成带 `reset_evidence` 的新尝试卡。该建议始终 `hard_gate:false`，不替 KP 作语义裁决。
    - `requires_discovered_clue_ids` 只能引用在执行该 affordance **之前**即可获得的前置知识或访问凭据。绝不能引用“成功执行该 affordance 后才显现/交付”的结果线索；否则路线会自锁。结果线索应留在场景的 `available_clues` 或成功后果中，前置则引用更早获得的物品识别、入口、关系或其他先行线索。
  ...
  - `npc_ids` (string[])：该场景**无条件实际在场**的 NPC（引用 npc-agendas.json）。相关、历史、未来或条件性登场的人物只能放在结构化 `mentions[]`，不能为了“有关联”塞进 `npc_ids`。运行期即兴登场、离场或换场由 `state.npc_presence` 的 live overlay 表达；不得从交谈记录猜测仍在场，也不得回写模块真相。
...
- `conclusions` (object[])：结论数组。每个 conclusion：
  ...
  - `clues` (object[])：线索路径数组。每条 clue：
    ...
    - `source_refs` (object[]，可选)：溯源引用数组，指向来源 PDF 中的具体位置。每条 `source_ref` 含规范字段：
      ...
      - `grep_anchor` (string)：该页中一句可辨识的短语，可对 host source bundle 的逐页 Markdown 验证。
...
### Time-loop strategy signals

Time-loop scenes may declare `loop_boundary` (boolean) and
`player_retained_memory_ids` (a unique list of non-empty string IDs). Runtime
and both scenario validators fail closed on any other shape; a string is never
treated as an iterable list of character IDs.

---

## 5. npc-agendas.json

NPC 可额外声明 A21 的结构化交谈合同：`known_fact_ids`、
`revealable_fact_ids`、`disclosure_order`、`facts[]`（每项至少含
`fact_id`、`clue_id`，可含 `min_trust`、`required_leverage_ids`、
...
- `fronts` (object[])：前沿数组。每个 front：
  ...
  - `dangers` (object[])：危险源数组。每个 danger：
    ...
    - `attack_profiles` (object[], optional)：结构化危险结算定义。每个 profile 含 `name`、`attack_skill`（攻击方技能，如 "Fighting"）、`attack_target_percent`（攻击方成功率，如 60）、`reaction_kind`（必填语义枚举：`dodge` / `fight_back` / `dodge_or_fight_back` 走 `combat.resolve`；`noncombat` 才可走普通对抗）、`resist_skill`（防御方技能，如 "Dodge"）、`damage`（伤害表达式）、可选 `lethality`（致命度数值或 null）、可选 `lethal`（bool，与 lethality>0 同属致命证据）、可选 `ignores_armor`（布尔）。Director 不会把战斗 reaction 生成为 generic opposed；未提供 attack_profiles 的 danger 仍由 LLM Keeper 手动驱动。
