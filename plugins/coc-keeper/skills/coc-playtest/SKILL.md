...
- `player-view.jsonl`: the player-safe view stream, including public character state and visible transcript turns only. For active localized runs, player-view `text` is player-visible and must render through `play_language`, including system roll summaries, success-level labels, difficulty labels, actor display names, and skill display names. Preserve canonical rule enum values in structured payload fields, stored logs, and Mechanical Log roll text.
