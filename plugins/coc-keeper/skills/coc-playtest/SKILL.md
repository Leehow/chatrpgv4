...
`suite-report.md` should include `## Run Index`, `## Non-Passing Evaluated Runs`, `## Loop Decision`, and a `## Core Coverage Matrix` with `character_dossier`, `kp_player_transcript`, `mechanical_rolls`, `combat`, `chase`, `sanity`, `meta_game`, and `player_feedback`, so the evaluator can see whether the current evaluated run set covers the requested Keeper Rulebook workflows without hiding failed or missing audits. Coverage and quality matrices must use `evaluated_runs`; historical baseline runs remain visible in `ignored_historical_runs` but must not satisfy current coverage or quality gates. `coc_completion_audit.py` must emit `suite_matrix_references_non_evaluated_run` if a coverage or quality matrix entry lists any run id outside `loop_decision.evaluated_runs`.
...
- Serious completion-oriented suites should include at least one multi-profile virtual player pressure run, or equivalent semantic evidence, so the evaluator can see how the Keeper handles careful planning, reckless risk-taking, and meta/rules challenges rather than only one scripted player style. Source `transcript.jsonl` evidence for that run should include each required `player_profile` enum with visible text plus at least one structured `intent` and localized `intent_display` row per required profile, so completion audit can prove the pressure shape without hardcoded natural-language matching.
