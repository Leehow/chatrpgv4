...
- **Ordinary KP path (required):** one semantic `coc_chargen_delegate` / `setup.chargen_run` call with name, occupation, optional eight-key `assignment_priority`, optional occupation/interest skill name lists, and default Luck `auto_roll`. Do not hand-assemble `investigator.create`. Direct `investigator.create` remains a documented contract for imports and tests only — **normal play must not use it**.
