...
- **Cash and assets are not KP numbers.** `setup.chargen_run` materializes
  ...
  snapshot (card lore — label it as creation/baseline, never as live). Play-time
  cash, Assets, living standard, and inclusive Spending Level live on campaign
  investigator-state (`state.cash_query` / `state.finance_query`). Buys are
  `state.purchase`; converting Assets is `state.assets_liquidate` after a
  linked time advance. `setup.chargen_run` create+link seeds that envelope once
  from the same 1920s/modern table, with `localized_reason` 建卡·信用评级换算,
  ...
  `setup.quick_start` still leave the ledger empty. Credit Rating remains a
  social/financial skill in play: loans, hiring, credentials, access, and
  conspicuous status may be a KP-judged Credit Rating check rather than a
  purchase. It never earns ordinary skill improvement ticks, and finance tools
  never auto-change it. During the Investigator Development Phase it may change
  when financial circumstances warrant, by KP judgment and the rules'
  financial-development procedure.
