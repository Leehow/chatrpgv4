...
- **Module-aware suggestions (required, not optional).** Use the player-safe module briefing (era, place, premise) to **offer** parameters: language that the opening actually needs, era-and-place occupations, and carried items that belong on this table. Suggest in Keeper voice; do not quiz. A 1920s American opening cannot leave the investigator unable to speak English. If the character is Chinese-heritage (or another non-local tongue) and `own_language` is 国语/汉语/etc., also put `Other Language` English (canonical skill name as the table uses it) into occupation or interest names at an investigable level. Runtime 1920s/modern default `own_language` to 英语 only when KP omits it; override when the fiction is not Anglophone.
- **`scenario_bound` is a module hook.** It must name this module's concrete opening (e.g. Knott's commission / the letter / the invitation), not a generic 「被卷入一件怪事」. On empty or partial endpoints, KP **offers** one or two module-specific involvements for the player to take or rewrite. The final draft always shows the chosen hook.
