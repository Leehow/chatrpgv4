...
**主线规则：`0.x.xa` 序列里编号最大的那一支就是当前开发线——不写死版本号。** 取最新一支：`git branch --list '0.*a' --format='%(refname:short)' | sort -V | tail -1`；本 worktree `chatrpgv4-wt-pi-coc-v2` 检出的就是它。开发只落在最新这支上，旧版一律快照只读；新版本线一出，主线随之移动，本文不需要跟着改写。

| 分支 | 是什么 | 怎么用 |
| --- | --- | --- |
| **`0.x.xa` 里最新的一支**（按上面的命令确定；本 worktree 检出的就是它） | 当前开发主线：重构后的产品——一个 Pi 包 + 一个编译后的 TypeScript 内核子进程，守秘人只见七个动词；从孤儿分支重建，**从来没有包含过旧树**。 | 只在这一支上开发。开工前先确认它仍是最新一支；找不到就当主线缺失、报出来，不在旧版上开工。 |
| `0.9.2a` | 前任产品线，最新主线自其顶端继续；停在 `7bde28c8f`（其后新提交只落在更新的一支上）。 | **保留，不在上面开发。** 要看那一版用 `git show 0.9.2a:<路径>`。 |
...
本树里的落实：模组读者是每 section 一个子 `pi -p --no-extensions --tools read,write,edit,bash`（`--no-extensions` 必须，否则子进程会再拉一个内核）。**唯一的例外**是记忆抽取、校验、行动准入三条车道：产出是十几条短 JSON 或一个 verdict，规格 #12 定为零工具子会话，实现是一次不带工具的补全——嵌套零工具会话在 0.85.1 里起得了，是权衡后没走（`docs/pi-host-contract.md` 第 5 节）；记忆与校验 2026-09-06 由用户裁定保留，行动准入（契约 §32，前台跑在每个 `resolve`/`apply` 之前，带工具子进程等不起）2026-09-11 由用户裁定加入，**不得推广到任何别的文本工作**。想用别的形状，当前回合先问。
