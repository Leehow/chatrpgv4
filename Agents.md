...
- **内核不解析 PDF**：解析只发生在宿主适配器里（`@firecrawl/pdf-inspector` 本地抽取、OCR 外包），产出永远是可逐字节复核的资料包，清单由我们的打包器签而不是适配器交（契约 §20.1；2026-09-06 由「仓库不解析 PDF」收窄而来）。Python 侧禁止 import PDF 库的两条测试不变。
