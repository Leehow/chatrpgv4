...
明确**没搬、也不打算按旧样子搬**的：旧 web/Electron 前端、steward 子代理车道（职责归图与深读队列）、世界线/时间线分叉与汇流、跨战役记忆、house rules 的提议/确认流程、Director 的 storylet、战报导出与地图供应技能、OCR worker（PDF 由宿主技能读，仓库不解析）。真桌暴露的 #19–#22 已做完并关票；世界线按契约 §15 在票 #23 重做（不是搬旧树）。
...
| PDF 阅读、构图、按需补读或旧解析链路退役 | `docs/specs/visual-pdf-reader.md`、`docs/kernel-rpc.md` §22 与 GitHub #34。目标待实现；先通过新路径验收，再拆旧入口；保留全部证据，Electron 留后。 |
...
- **内核不解析 PDF**：PDF 处理只发生在宿主，Python 侧禁止 import PDF 库的两条测试不变。现有运行时的文字资料包见 §20.1；2026-09-07 的视觉阅读目标契约见 §22：原 PDF 与可追溯页来源替代强制 OCR/Markdown 资料包，按规格验证后切换，不将旧管线当作新实现的约束。
