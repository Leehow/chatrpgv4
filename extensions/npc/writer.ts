...
export async function authorNpc(options:{runtime:HostRuntime;jobId:string;model:string;instruction:string;input:unknown;signal:AbortSignal}):Promise<unknown>{
    ...
    const outcome=await options.runtime.runTask({kind:'mod',request:{cwd,systemPrompt,model:options.model,tools:'read,write,edit,bash',priority:'background',eventLog,
