/**
 * The player-side NPC journal lane (contract §17.10, wiring cloned from §12.8).
 *
...
/** How many turns one session may backfill at most; `PI_COC_NPCJOURNAL_BACKFILL=0` turns the whole backfill off. */
...
function backfillBudget(): number {
	const raw = process.env.PI_COC_NPCJOURNAL_BACKFILL?.trim();
...
/**
 * One journal job. With a `turn` it is the turn just committed; without one it is the default
 * dispatch (backfill), where the kernel picks a turn whose job is unfinished and not in the backlog.
 */
interface Job {
	campaign: string;
	turn?: number;
...
export default function (pi: ExtensionAPI) {
	...
	let stopped = false;
	let running = false;
	/** Freshly committed turns, first come first served; backfill always queues behind this. */
	const queue: Job[] = [];
	...
	// No backfill while a turn is open: an agent running is the player's turn running, and that
	...
	pi.on("session_start", async (_event, sessionCtx) => {
		...
		queue.length = 0;
		// The backfill budget is per session: by now the kernel extension has already emitted the
		// bridge (extension session_start handlers run serially in load order, kernel first).
	...
	pi.on("session_shutdown", async () => {
		// Shutdown does not wait for the lane: queued jobs are dropped, in-flight ones cut off, and whatever was not written comes back on the next default dispatch of `journal.job`.
		stopped = true;
		queue.length = 0;
		backfillLeft = 0;
		backfillDone = true;
