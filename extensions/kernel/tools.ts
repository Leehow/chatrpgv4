...
export const COC_TOOLS: readonly CocToolSpec[] = [
	...
	},
	{
		...
		parameters: Type.Object({
			...
			narrate: Type.Optional(Type.String({ description: "this turn's closing narration, delivered only once every effect above lands (same rules as the narrate tool's text: play_language, {{marker}}/{{kind:handle}} placement, {{say:Name}}…{{/say}} spans); if any effect is refused, or this text is refused on delivery, nothing here is shown to the player and the effects above still stand — call narrate on your next step instead" })),
		}),
	},
	{
		name: "ask",
