...
export const COC_TOOLS: readonly CocToolSpec[] = [
	...
	},
	{
		...
		description:
			"Deliver story text and close the turn. Describe fiction and observable consequences only. Roll values, targets, grades, resource accounting, elapsed time as a figure (minutes, hours) and rule options are exclusively mechanics JSON rendered by the frontend and its clock. Do not repeat them in text or ask how to handle a failed check. Use the campaign play_language. Mark where each mechanic happened: resolve and apply hand back markers, and writing {{that-marker}} at the point in the sentence where it happened lets the frontend draw the roll or the change there instead of after everything. Place only markers you were handed, each at most once; leaving one out is fine and simply groups it at the end. After delivery write no more prose.",
