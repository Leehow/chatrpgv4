...
export function admissionSystemPrompt(): string {
	return [
		...
		"- not_authorized: the Keeper is choosing for the player — a new destination, a new method (picking a lock the player only looked at; bribing or threatening a guard the player only asked), a new target, a cost or commitment, a route the Keeper offered and the player has not taken, or an action the player only showed interest in.",
