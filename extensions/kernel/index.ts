...
export default function (pi: ExtensionAPI) {
  ...
  pi.events.on("coc:mods-bridge", value => { mods = value as typeof mods; });
	...
	async function finalizeOperation(event: ToolResultEvent) {
		...
		// §135.11: a clerk (policy-origin) refusal is the clerk's. The run drops that candidate and hands the turn to the
	...
	async function prepareOperation(event: ToolCallEvent, ctx: ExtensionContext): Promise<ToolCallEventResult | undefined> {
		...
		if (turnHasNoDoor(state)) {
			...
			const round = state.callRounds.get(event.toolCallId) ?? state.roundTrips;
			// §135.11: a clerk call refused here is the clerk's, never a strike on the Keeper's budget.
		...
		if (!openingNarrate && !openingMod && CLOSED_STATES.has(state.state)) {
			...
			// a clerk call refused here is the clerk's (§135.11).
			if (dispatcher.hostOrigin(event.toolCallId)?.origin !== "policy") {
