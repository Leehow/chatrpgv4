...
export function resolveImagesConfig() {
    const settings = settingsSnapshot();
    const baseUrl = process.env.XAI_API_BASE_URL?.trim() ||
        settingString("ext.grok-build-oauth.xaiApiBaseUrl") ||
        DEFAULT_BASE_URL;
    const model = process.env.GROK_IMAGINE_MODEL?.trim() ||
...
export function resolveRelayBase() {
    ...
    try {
        ...
        throw new ImagesError("invalid_params", `invalid PIPIUI_GROK_RELAY: ${raw}`);
    }
    const loopback = url.hostname === "localhost" || url.hostname === "127.0.0.1" || url.hostname === "[::1]" || url.hostname === "::1";
    if (url.protocol !== "http:" || !loopback) {
        throw new ImagesError("invalid_params", `refusing a non-loopback HTTP relay base: ${raw} (the deprecated relay allows only 127.0.0.1/localhost/[::1] and uses Bearer local, carrying no real credential)`);
    }
    return raw.replace(/\/+$/, "");
}
