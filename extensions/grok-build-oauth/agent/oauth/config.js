...
export function assertHttpsIssuer(issuer) {
    ...
    if (protocol !== "https:") {
        throw new OAuthError("invalid_params", `refusing a non-HTTPS OAuth issuer: ${issuer} (device-code/token/refresh endpoints allow HTTPS only; credentials never travel over plain HTTP)`);
    }
    return issuer;
}
