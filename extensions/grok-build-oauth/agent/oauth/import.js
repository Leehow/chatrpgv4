 ...
 * (spec US-09 / D5 "never silently read the global file").
 *
 * Contract:
...
export async function readGlobalGrokAuth(opts) {
    ...
    try {
        ...
        return { ok: false, reason: `${path} not found — log in with the official grok CLI first, or use /login grok-build` };
    }
    let parsed;
    try {
        ...
        return { ok: false, reason: "auth.json is not valid JSON" };
    }
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
        return { ok: false, reason: "auth.json has an unrecognized structure" };
    }
    const rec = parsed;
    ...
    const access = typeof rec.key === "string" && rec.key ? rec.key :
        typeof rec.access === "string" && rec.access ? rec.access :
            typeof rec.access_token === "string" && rec.access_token ? rec.access_token :
                undefined;
    if (!access)
        return { ok: false, reason: "auth.json has no usable access token (missing key/access fields)" };
    ...
    if (expiresAtMs === undefined) {
        return { ok: false, reason: "auth.json is missing expires_at — cannot determine validity; log in again" };
    }
    const nowMs = opts.nowMs ?? Date.now;
    if (expiresAtMs <= nowMs()) {
        return { ok: false, reason: "the imported credential has expired — log in to grok-build again" };
    }
...
export async function importFromGlobalGrok(opts) {
    if (!opts.confirm) {
        return { imported: false, reason: "explicit confirmation required: /grok-build:import --confirm (reads ~/.grok/auth.json exactly once; the source file is not modified)" };
    }
