...
function isOauthShapedEntry(entry) {
...
export function brokerCredentialFromEntry(entry, nowMs) {
    if (!entry)
        return undefined;
    const typedOauth = entry.type === "oauth";
    const typelessOauth = (entry.type === undefined || entry.type === null) && isOauthShapedEntry(entry);
    if (!typedOauth && !typelessOauth)
        return undefined;
    const access = entry.access;
...
export class GrokCredentialBroker {
    ...
    async importCredential(input) {
        if (!input.access || typeof input.access !== "string") {
            throw new OAuthError("invalid_params", "the imported credential is missing an access token");
        }
        const cfg = resolveOAuthConfig();
        if (input.issuer && input.issuer.replace(/\/+$/, "") !== cfg.issuer.replace(/\/+$/, "")) {
            throw new OAuthError("invalid_params", `the imported credential issuer does not match (expected ${cfg.issuer}) — log in to grok-build again`);
        }
        if (input.clientId && input.clientId !== cfg.clientId) {
            throw new OAuthError("invalid_params", `the imported credential client_id does not match (expected ${cfg.clientId}) — log in to grok-build again`);
        }
        if (!Number.isFinite(input.expiresAtMs) || input.expiresAtMs <= this.nowMs()) {
            throw new OAuthError("invalid_params", "the imported credential has expired — log in to grok-build again");
        }
        const cred = {
            type: "oauth",
            access: input.access,
    ...
    /**
     * Non-secret credential status for host/UI surfaces (login state / expiry / credential source / tier).
