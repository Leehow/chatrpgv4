...
export function boundImages(messages: any[], previouslyIncluded = new Set<string>(), byteBudget = 8 * 1024 * 1024, countBudget = 4) {
	...
	for (let i = copy.length - 1; i >= 0; i--) {
		...
		for (let j = message.content.length - 1; j >= 0; j--) {
			...
			if (count === 0 || (count < countBudget && bytes + size <= byteBudget)) {
				bytes += size; count++;
				included.push(key);
				continue;
			}
