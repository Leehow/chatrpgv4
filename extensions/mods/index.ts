...
/**
 * What `prepare` reports back when it let a delivery through that no reviewer ever judged (§NN).
...
export interface ModBridge {
   ...
   * `reviewed` is §NN's: whether a reviewer's own verdict stands behind the retained block. Absent
   * reads as unreviewed, so a store written before §NN never strands a recovered turn on its own.
   */
    ...
    async prepare(method, payload, signal) {
      ...
      if ((method === "narrate" || method === "ask") && payload.text) {
        ...
        try {
          ...
          // Contract §NN. The continuity review is the same gate, and the same rule reaches it here:
        ...
        // `reviewed: true` and keeps refusing; §NN changes nothing here.
