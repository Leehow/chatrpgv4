 ...
 * reviewer ever reached a verdict about this draft (contract §NN).
 *
 ...
 *
 * `reviewed` is the third fact, and it is the one the *delivery* decision reads (§NN). `service`
...
export class AuditBudget {
    ...
    constructor(scope: string, inputToken?: string, limits = AUDIT_LIMITS) {
        ...
        try {
            ...
            // authority of a verdict it never had (§NN).
    ...
    start(): {max_requests: number; max_artifact_repairs: number; timeoutMs: number} {
        ...
        // §NN: an exhausted allowance is a bound on what this table may still spend, not a reading of
    ...
    verdict(job: string, verdict: string) {
        ...
        // §NN: these three are the only ends a reviewer's own reading stands behind, so they are the
